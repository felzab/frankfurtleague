...
def test_every_needed_privilege_is_reported_independently(mongo_url: str):
    ...
    async def body(database: AsyncIOMotorDatabase) -> list[tuple[str, str]]:
        ...
        limited = AsyncIOMotorClient(mongo_url, username=username, password=password, authSource=DATABASE_NAME)
        try:
...
def test_the_identity_report_names_the_user_and_its_roles(mongo_url: str):
    ...
    async def body(database: AsyncIOMotorDatabase) -> tuple[str, list[str]]:
        await database.command("createUser", username, pwd=password, roles=[{"role": "readWrite", "db": DATABASE_NAME}])
        # Keywords over the url's own root credentials, which is what pymongo does with both given.
        named = AsyncIOMotorClient(mongo_url, username=username, password=password, authSource=DATABASE_NAME)
        try:
