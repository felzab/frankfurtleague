...
from typing import Any, AsyncIterator, Coroutine, Iterable, Mapping
...
def on_the_seed_loop(seed: Coroutine[Any, Any, Any]) -> Any:
    """Motor binds to the loop it first ran on, and ONE lasting loop satisfies that as fully as a fresh one per call.
    ...
    if _LOOP is None:
        ...
        # Discoverable from outside a coroutine too, which is where Motor reads it when a client is
        # built there rather than inside a seed.
...
def _shared_client(url: str) -> AsyncIOMotorClient:
