...
class TestARetiredClubStaysOutOfTheBaseTierReads:
    ...
    def test_the_corpus_really_holds_a_retired_club_in_this_seasons_group(self, mongo_url: str):
        ...
        club, row = on_a_league(mongo_url, body)

        assert club["inactive_since"] == RETIRED_ON
        assert row["gruppe"] == "A"

    def test_the_standings_leave_it_out(self, mongo_url: str):
        response = on_a_league(mongo_url, lambda database: read_teams(database, base_filters(in_gruppen=True)))

        assert standing_names(response) == ["Helmholtz"]

    def test_the_flat_list_leaves_it_out_as_well(self, mongo_url: str):
        """The switch left the ENDPOINT rather than one of its two shapes: the flat list is the same base-tier surface."""
        response = on_a_league(mongo_url, lambda database: read_teams(database, base_filters()))

        assert [team.name for team in response.teams] == ["Helmholtz"]

    def test_the_admin_read_still_un_hides_it(self, mongo_url: str):
        ...
        response = on_a_league(mongo_url, lambda database: read_teams_for_admin(database, filters))
...
class TestTheBaseTierReadsWithholdTheJunctionsContactRecords:
    ...
    def test_the_corpus_really_stores_them(self, mongo_url: str):
    ...
    def test_the_aggregation_carries_none_of_it_back(self, mongo_url: str):
        ...
        rows = on_a_league(mongo_url, body)
    ...
    def test_the_list_endpoint_serves_none_of_it(self, mongo_url: str):
        response = on_a_league(mongo_url, lambda database: read_teams(database, base_filters()))

        assert response.teams
        assert KONTAKT_EMAIL not in response.model_dump_json()

    def test_the_single_team_endpoint_serves_none_of_it_either(self, mongo_url: str):
        ...
        response = on_a_league(mongo_url, body)
