...
`acknowledged` on all 18 of its response schemas. These tests pin the contract from the backend side
so a future model added without the envelope fails here rather than at a browser.

The four models in `test_declares_the_envelope_on_every_untyped_route` were bare `JSONResponse`
...
@pytest.mark.parametrize("name,model", list(_response_models()), ids=lambda v: v if isinstance(v, str) else "")
