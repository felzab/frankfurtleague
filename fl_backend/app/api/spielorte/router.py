...
from app.api.spielorte.schemas import FLSpielorteFilterParams, FLSpielorteListAdapter, FLSpielorteListResponse
...
router = APIRouter(prefix=f"/api/v{backend_config.api_version}/spielorte", dependencies=[Depends(verify_access_base)])


@router.get("", response_model=FLSpielorteListResponse)
async def get_spielorte(
    spielorte_collection: SpielorteCollection, filters: FLSpielorteFilterParams = Depends()
