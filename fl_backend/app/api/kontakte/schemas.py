...
class FLKontaktErasurePayload(BaseModel):
    ...
    # A lookup and never a store, so held to no address rule: a seat stored under an older rule would
    # leave its person unerasable (GDPR Art. 17). The fold that matches never raises.
