...
from app.api.spieltage.schemas import FLSpieltageFilterParams, FLSpieltageListResponse, FLSpieltagListAdapter
...
router = APIRouter(prefix=f"/api/v{backend_config.api_version}/spieltage", dependencies=[Depends(verify_access_base)])
...
async def get_spieltage(
    ...
    spieltage_raw = await pull_many_from_db(
        collection=spieltage_collection, db_filter=db_filter, limit=filters.limit, sort_by=db_sort
    )
