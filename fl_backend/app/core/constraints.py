...
_SPIEL_QUELLE = _object(
    nullable=True,
    # `type` alone: the other keys belong to one variant each, and `$jsonSchema` cannot require a
    # field only when a sibling holds a value. What stays catchable here is a `platz` stored as "2".
    required=("type",),
    properties={
