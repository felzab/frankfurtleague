...
class BackendConfig(BaseSettings):
    ...
    api_cors_allowed_origins: str = Field(
        description="The allowed CORS origins for this API"
    )
    ...
    db_server_selection_timeout: int = Field(
        default=15000, description="MongoDB server-selection timeout in ms"
    )
    ...
    internal_api_key_system: SecretStr = Field(
        description="Internal API-key for the system router"
    )
    internal_api_key_admin: SecretStr = Field(
        description="Internal API-key for the admin router"
    )
    ...
    log_format: Literal["console", "json"] = Field(
        default="console", description="The default log format"
    )
