...
import SpielDisplayPreview from "@/features/spiele/components/SpielDisplayPreview";
import ExpandableDescription from "@/shared/components/ui/ExpandableDescription";
import Link from "next/link";
import { formatAddress } from "@/shared/utils";

function SaisonSpieleTimeline({ spiele, teamId, today }: { spiele: FLSpiel[]; teamId: string; today: string }) {
      ...
      {spiele.map((spielData) => {
        ...
        return (
          <div
            ...
            <div
              className={`absolute left-[-11px] top-1 size-[20px] rounded-full ring-4 ${getBadgeColor(result)} flex items-center justify-center text-[10px] font-bold z-10`}>
              {result}
            </div>
