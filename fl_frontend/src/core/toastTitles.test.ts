...
const FAILURE_CALL = /appToast\.failure\(/g;

/** The name `fl_frontend/src/shared/utils/appToast.ts` gives the title a failure of unknown outcome is raised under. */
const OUTCOME_UNKNOWN_CONSTANT = "OUTCOME_UNKNOWN_TITLE";
...
for (const [file, text] of production) {
  ...
  // `appToast.failure` raises a danger with the failure's own sentence as the description, under the
  // site's title or, where the outcome is unknown, under the one neutral title every such site shares.
  for (const call of text.matchAll(FAILURE_CALL)) {
    const argument = argumentText(text, call.index + call[0].length);
    record("danger", argument, ["description"]);
    record("danger", OUTCOME_UNKNOWN_CONSTANT, ["description"]);
  }
  ...
  for (const stray of readable.matchAll(new RegExp(String.raw`appToast\.(` + VARIANTS + String.raw`|failure)`, "g"))) {
...
const TOAST_TITLES: Record<string, RegisteredTitle> = {
  ...
  "Bewerbung schon angekommen": { variant: "danger", identifies: "one site" },
  ...
  "Passkey gelöscht": { variant: "success", identifies: "one site" },
  "Passkey hinzugefügt": { variant: "success", identifies: "one site" },
  "Passkey nicht eingerichtet": { variant: "danger", identifies: "one site" },
  "Passkey nicht gelöscht": { variant: "danger", identifies: "one site" },
  "Passkey nicht hinzugefügt": { variant: "danger", identifies: "one site" },
  "Passkeys nicht geladen": { variant: "danger", identifies: "one site" },
  "Registrierung nicht abgeschickt": { variant: "danger", identifies: "its description" },
  "Registrierung schon angekommen": { variant: "danger", identifies: "one site" },
  ...
  "Registrierungslink nicht gesendet": { variant: "danger", identifies: "one site" },
  "Registrierungslinks gesendet": { variant: "success", identifies: "one site" },
  "Registrierungslinks nicht gesendet": { variant: "danger", identifies: "one site" },
  ...
  "Unklar, ob es gespeichert wurde": { variant: "danger", identifies: "its description" },
  ...
  "Vorschau nicht geladen": { variant: "danger", identifies: "one site" },
