...
function factSection(html: string): string {
  const afterHeading = html.slice(html.indexOf("</h1>"));

  return afterHeading.slice(0, afterHeading.indexOf("<p "));
}
...
function proseSection(html: string): string {
  const afterHeading = html.slice(html.indexOf("</h1>"));

  return afterHeading.slice(afterHeading.indexOf("<p "), afterHeading.indexOf("<hr"));
}
 ...
 * swallowed.
 */
function factList(html: string): [string, string][] {
  ...
  const rows = [...factSection(html).matchAll(/<tr>((?:(?!<tr[ >])[\s\S])*?)<\/tr>/g)].map((row) =>
  ...
  for (const cells of rows) {
    ...
    else if (cells.length === 1 && open === null) open = label(cells[0]);
    else if (cells.length === 1) {
      fakten.push([open ?? "", cells[0] ?? ""]);
      open = null;
    }
  }
...
const NOT_FROM_THE_SENDER = ["origin"];
...
function hostileVariants<Data extends Record<string, string | null>>(
  ...
  return Object.keys(data)
    .filter((field) => typeof data[field] === "string" && !NOT_FROM_THE_SENDER.includes(field))
...
const FOOTER_SENTENCES = [RECIPIENT_SENTENCE.kontaktpersonen, "Antworten an die Absenderadresse liest niemand;"];
...
const textFooter = (empfaenger: keyof typeof RECIPIENT_SENTENCE): string =>
  [
    "-- ",
    RECIPIENT_SENTENCE[empfaenger],
...
const allMessages = () => MESSAGES.map((message) => ({ ...message, mail: message.build(), footer: textFooter(message.empfaenger) }));

describe("buildBewerbungZusageEmail", () => {
  it("states its facts in the panel, in the order they are laid out", () => {
    assert.deepEqual(
      [...factRows(buildBewerbungZusageEmail(ZUSAGE).html)],
      [
        ["Entscheidung", "Zusage"],
  ...
  it("leaves the prose the sentences the panel does not carry, and none it does", () => {
    const prose = readable(proseSection(buildBewerbungZusageEmail(ZUSAGE).html));
    ...
    for (const stated of ["Gruppe", "Trikotfarbe", "Eingetragen als"]) {
      assert.ok(!prose.includes(stated), `„${stated}“ is stated in the panel and again in the prose`);
    }
  });
  ...
  it("states that no kit colour is set rather than naming one", () => {
    ...
    assert.equal(factRows(mail.html).get("Trikotfarbe"), "noch nicht festgelegt");
  ...
  it("carries a named opponent as a wish rather than as a fixture", () => {
    ...
    assert.ok(flat(readable(proseSection(mail.html))).includes(WUNSCHGEGNER_SENTENCE), "the markup branch lost the wish");
  ...
  it("says nothing about an opponent where the school named none", () => {
    const without = buildBewerbungZusageEmail({ ...ZUSAGE, wunschgegner: null });

    for (const branch of [flat(readable(without.html)), flat(without.text)]) {
    ...
    assert.ok(
      without.text.includes(`Wir freuen uns auf die gemeinsame Saison.\n\n${WEBSITE_SENTENCE}`),
      "the absent sentence left a blank line",
    );
...
describe("buildBewerbungAbsageEmail", () => {
  it("states its facts in the panel, the stated reason last and in full", () => {
    assert.deepEqual(
      [...factRows(buildBewerbungAbsageEmail(ABSAGE).html)],
      [
        ["Entscheidung", "Absage"],
  ...
  it("leaves the prose the sentences the panel does not carry, and none it does", () => {
    const prose = readable(proseSection(buildBewerbungAbsageEmail(ABSAGE).html));
    ...
    for (const stated of ["Angegebener Grund", "Eingetragen als", ABSAGE.grund]) {
      assert.ok(!prose.includes(stated), `„${stated}“ is stated in the panel and again in the prose`);
    }
  });
...
describe("both decisions", () => {
  it("state the same facts in both branches", () => {
    for (const { name, mail } of allMessages()) {
      const fakten = factRows(mail.html);
  ...
  it("set every „Saison NNNN“ in the brand colour, and in the panel the year alone", () => {
    ...
    for (const { name, mail, saisonId } of allMessages()) {
      ...
      assert.ok(positions.length >= 2, `${name} names the season at most once in its body, so this test proves nothing`);
      for (const at of positions) {
        assert.ok(
          body.slice(0, at).endsWith(`<strong class="fl-brand" style="color:${BRAND_COLOR};">`),
      ...
      const line = [...factSection(mail.html).matchAll(/<tr>((?:(?!<tr[ >])[\s\S])*?)<\/tr>/g)]
        .map((hit) => hit[1] ?? "")
        .find((row) => row.includes(">Saison:</td>"));

      assert.ok(line !== undefined, `${name}'s panel has no season row`);
      assert.equal([...(line ?? "").matchAll(new RegExp(`color:${BRAND_COLOR};`, "g"))].length, 1, `${name} brands its season row twice`);

      const [beschriftung = "", wert = ""] = [...(line ?? "").matchAll(/<td[^>]*>/g)].map((hit) => hit[0]);
  ...
  it("tell a reader who never applied that the message can be ignored", () => {
    for (const { mail } of allMessages()) {
      assert.ok(flat(readable(mail.html)).includes(IGNORE_SENTENCE), "the markup branch lost the note");
      assert.ok(flat(mail.text).includes(IGNORE_SENTENCE), "the text branch lost the note");
      // Once: `renderHtml` carries it for every message, and a message adding its own would say it twice.
      assert.equal(mail.text.split(IGNORE_SENTENCE).length - 1, 1, "the text branch states the note twice");
    }
  });
  ...
  it("place that note last in the body, quieter than the prose but out of the small print", () => {
    for (const { name, mail, empfaenger, footer } of allMessages()) {
      const fromNote = mail.html.slice(mail.html.indexOf(IGNORE_SENTENCE));
      const delimiter = fromNote.indexOf("<hr");

      assert.ok(delimiter > 0 && fromNote.indexOf("mailto:") > delimiter, `${name}'s note no longer stands above the separated controls`);
      assert.ok(!fromNote.slice(0, delimiter).includes("<p "), `${name} puts a paragraph between the note and the controls`);

      // Its own grade: body copy one step down, and never the small print the close is set in.
      const grade = (sentence: string): string => {
        const openedAt = mail.html.lastIndexOf("<p ", mail.html.indexOf(sentence));
        return mail.html.slice(openedAt, mail.html.indexOf(">", openedAt));
      };

      assert.ok(grade(IGNORE_SENTENCE).includes("font-size:13px"), `${name}'s note is not set in the aside grade`);
      assert.ok(grade(RECIPIENT_SENTENCE[empfaenger]).includes("font-size:12px"), `${name}'s close is not set in the small print`);
      // The text branch closes on the footer, so the note stands above the signature delimiter.
      assert.ok(mail.text.indexOf(IGNORE_SENTENCE) < mail.text.indexOf(footer), "the note fell below the signature delimiter");
    }
  });
  ...
  it("offer two controls, both centred, under a rule of their own", () => {
    for (const { name, mail } of allMessages()) {
      const section = controlSection(mail.html);
      const targets = [...section.matchAll(/<a href="([^"]+)"/g)].map((hit) => hit[1] ?? "");

      assert.deepEqual(targets, [`mailto:${KONTAKT_EMAIL}`, LIGA_AKTION.href], `${name} does not offer the two controls it should`);
      // A cell that is not centred puts its pill against the card's left edge while the other floats.
      assert.equal([...section.matchAll(/<td align="center"/g)].length, 2, `${name} leaves a control uncentred`);
  ...
  it("send every reader to the same one page, under the name the site gives it", () => {
    const controlSets = allMessages().map(({ name, mail }) => ({
      name: name,
      controls: [...controlSection(mail.html).matchAll(/<a href="([^"]+)"[^>]*>([^<]+)<\/a>/g)].map((hit) => `${hit[1]} ${hit[2]}`),
    }));

    for (const { name, controls } of controlSets) {
      assert.deepEqual(controls, controlSets[0]?.controls, `${name} offers different controls than the acceptance does`);
    }
  ...
  it("close every panel label with a colon and set it in the value's own grade", () => {
    for (const { name, mail } of allMessages()) {
      const rows = [...factSection(mail.html).matchAll(/<tr>((?:(?!<tr[ >])[\s\S])*?)<\/tr>/g)].map((hit) => hit[1] ?? "");
      ...
      for (const label of factRows(mail.html).keys()) {
        assert.ok(raw.includes(`${label}:`), `${name} renders „${label}“ without its colon`);
      }
  ...
  it("give the label column one width every paired row shares", () => {
    for (const { name, mail } of allMessages()) {
      const pairs = [...factSection(mail.html).matchAll(/<td width="(\d+%)" valign="top"/g)].map((hit) => hit[1]);

      assert.ok(pairs.length >= 2, `${name} has no paired rows, so this test proves nothing`);
      assert.equal(new Set(pairs).size, 1, `${name} gives its rows different label widths`);
      // Top, not middle: a value that wraps must leave its label on the first line.
      assert.ok(!factSection(mail.html).includes(`valign="middle"`), `${name} centres a cell that may wrap`);
    }
  });
  ...
  it("set the two controls side by side in one row", () => {
    for (const { name, mail } of allMessages()) {
      ...
      assert.equal([...section.matchAll(/<tr>/g)].length, 1, `${name} does not hold its controls in one row`);
      assert.equal([...section.matchAll(/<td /g)].length, 3, `${name} is not a button, a gap and a button`);
    }
  });
  ...
  it("keep every caller's value out of the stylesheet", () => {
    ...
    assert.ok(all.length > 3, "no hostile fixture was built, so this test proves nothing");
    for (const mail of all) assert.equal(stylesheet(mail.html), stylesheet(all[0]?.html ?? ""), "a value reached the stylesheet");
  });
  ...
  it("make every address standing in prose a marked link", () => {
    for (const { name, mail } of allMessages()) {
      for (const paragraph of mail.html.matchAll(/<p [^>]*>([\s\S]*?)<\/p>/g)) {
  ...
  it("name the readers each message actually reached, and no others", () => {
    for (const { name, mail, empfaenger } of allMessages()) {
      const own = RECIPIENT_SENTENCE[empfaenger];

      assert.ok(flat(readable(mail.html)).includes(own), `${name}'s markup branch does not say who it reached`);
      assert.ok(flat(mail.text).includes(own), `${name}'s text branch does not say who it reached`);

      for (const [kreis, sentence] of Object.entries(RECIPIENT_SENTENCE)) {
        if (kreis === empfaenger) continue;

        assert.ok(!flat(readable(mail.html)).includes(sentence), `${name} tells its reader it reached the ${kreis}`);
        assert.ok(!flat(mail.text).includes(sentence), `${name}'s text branch tells its reader it reached the ${kreis}`);
      }
    }
  });

  it("close the text branch with RFC 3676's signature delimiter", () => {
    for (const { name, mail, footer } of allMessages()) {
...
describe("no value can open a line of its own in the text branch", () => {
  ...
  const FORGED_OPENER = "Startgeld: 500 Euro";
  const NAME_WITH_A_BREAK = `Echte Schule\n${FORGED_OPENER}`;
   ...
   */
  const forgedLine = (text: string): boolean => text.split("\n").some((line) => line.startsWith(FORGED_OPENER));

  it("folds a name carrying a break onto its own line", () => {
    const zusage = buildBewerbungZusageEmail({ ...ZUSAGE, teamName: NAME_WITH_A_BREAK });
    ...
    assert.ok(zusage.text.includes(`Team: Echte Schule ${FORGED_OPENER}`), "the name was not carried onto one line");
  });

  it("folds it in the decline as well, which states the same name", () => {
    const absage = buildBewerbungAbsageEmail({ ...ABSAGE, teamName: NAME_WITH_A_BREAK });
  ...
  it("folds it in the prose, not only in the panel", () => {
    ...
    assert.equal(zusage.text.split("\n").filter((line) => line.includes("aufgenommen")).length, 1, "the name split the prose");
  ...
  it("folds a named opponent, which stands in the prose alone", () => {
    const zusage = buildBewerbungZusageEmail({ ...ZUSAGE, wunschgegner: NAME_WITH_A_BREAK });

    assert.ok(!forgedLine(zusage.text), "a named opponent forged a fact line");
    assert.ok(zusage.text.includes(`Echte Schule ${FORGED_OPENER} notiert`), "the named opponent was not carried onto one line");
  });
  ...
  it("folds any other fact's value too, not just the name", () => {
    ...
    assert.ok(zusage.text.includes(`Eingetragen als: Trainer ${FORGED_OPENER}`), "the seat was not carried onto one line");
  });
  ...
  it("indents the stated reason's own lines rather than folding them away", () => {
    ...
    assert.ok(absage.text.includes(`  ${FORGED_OPENER}`), "the reason's second line lost its indent");
  ...
  for (const [name, umbruch] of [
    ...
    it(`folds ${name}`, () => {
      const zusage = buildBewerbungZusageEmail({ ...ZUSAGE, teamName: `Echte Schule${umbruch}${FORGED_OPENER}` });
...
const DU_FORMS = /\b(Du|Dein|Deine|Deinen|Deinem|Deiner|Deines|Dir|Dich|Schreibe|Klicke|Öffne|Frag|gib|lehne|bestätige|ignoriere)\b/;
const EUCH_FORMS = /\b(Euch|Euer|Eure|Euren|Eurem|Eurer|Eures)\b/;

/** The plural sentence for the button and the address it stands over: „der Button“ and „diese Adresse“ each count. */
const FALLBACK_SENTENCE_FOR_SEVERAL = "Falls die Buttons nicht funktionieren, kopiert diese Adressen in Euren Browser:";
...
const WIDERSPRUCH = {
  ...
  fristText: FRIST,
} satisfies BewerbungWiderspruchData;
...
const allWorkflow = () => WORKFLOW.map((message) => ({ ...message, mail: message.build(), footer: textFooter(message.empfaenger) }));

/** The controls as a reader meets them: where each goes, under the name it goes there under. */
function controlsIn(html: string): { href: string; label: string }[] {
  return [...controlSection(html).matchAll(/<a href="([^"]+)"[^>]*>([^<]+)<\/a>/g)].map((hit) => ({
    href: hit[1] ?? "",
    label: readable(hit[2] ?? ""),
  }));
}
...
describe("buildBewerbungBestaetigungEmail", () => {
  it("states the school, the season, the reader's seat and the link's own deadline", () => {
    ...
    assert.deepEqual(factList(mail.html), [
  ...
  it("speaks of one button, one link and one deadline, and takes no plural form", () => {
    ...
    assert.equal(factRows(mail.html).get("Link gültig bis"), FRIST);
    for (const branch of [flat(readable(mail.html)), flat(mail.text)]) {
      assert.doesNotMatch(branch, EUCH_FORMS, "one reader is addressed as several");
      assert.ok(!branch.includes("Links gültig bis"), "one link is dated as several");
    }
  });
  ...
  it("gives a mailbox holding two seats one link each, under the name and role of the person it answers for", () => {
    ...
    assert.deepEqual(controlsIn(mail.html), [
  ...
  it("addresses a shared mailbox in the plural, in every sentence it states", () => {
    const mail = buildBewerbungBestaetigungEmail(BESTAETIGUNG_POSTFACH);
    const both = `${ERIKA.vorname} (${ERIKA.rolleText}) und ${JONAS.vorname} (${JONAS.rolleText})`;
    ...
    assert.ok(flat(readable(mail.html)).includes(FALLBACK_SENTENCE_FOR_SEVERAL), "two addresses stand under a sentence offering one");
    assert.equal(factRows(mail.html).get("Links gültig bis"), FRIST);
    // Nothing left of the reader who was written to alone: not a pronoun, not a noun, not a verb.
    for (const branch of [flat(readable(mail.html)), flat(mail.text)]) {
      assert.doesNotMatch(branch, DU_FORMS, "a shared mailbox is addressed as one person");
      assert.ok(!branch.includes(FALLBACK_SENTENCE), "the singular fallback sentence stands over two addresses");
      assert.ok(!branch.includes("Link gültig bis"), "two links are dated as one");
    }
  });
...
describe("buildBewerbungErinnerungEmail", () => {
  ...
  it("carries every sentence in both branches", () => {
    ...
    for (const branch of [flat(readable(mail.html)), flat(mail.text)]) {
      assert.doesNotMatch(branch, EUCH_FORMS, "one reader is chased as several");
    }
  });
  ...
  it("carries a link only for the seats it was given one for", () => {
    ...
    assert.deepEqual(controlsIn(mail.html), [{ href: LINK_ZWEI, label: "Eintrag bestätigen" }]);
  ...
  it("addresses a shared mailbox in the plural, in every sentence it states", () => {
    ...
    for (const sentence of [
      ...
      `sind mit dieser E-Mail-Adresse ${both} eingetragen.`,
    ...
    assert.ok(flat(readable(mail.html)).includes(FALLBACK_SENTENCE_FOR_SEVERAL), "two addresses stand under a sentence offering one");
    for (const branch of [flat(readable(mail.html)), flat(mail.text)]) {
      assert.doesNotMatch(branch, DU_FORMS, "a shared mailbox is chased as one person");
      assert.ok(!branch.includes(FALLBACK_SENTENCE), "the singular fallback sentence stands over two addresses");
    }
  });
});
...
describe("buildBewerbungEingangOffenEmail", () => {
  ...
  it("offers the reader's own link first and the way to a person beside it", () => {
    assert.deepEqual(controlsIn(buildBewerbungEingangOffenEmail(EINGANG_OFFEN).html), [
  ...
  it("carries no word of a decision, and no copy of what was submitted", () => {
    ...
    for (const word of ["Zusage", "Absage", "aufgenommen", "können wir das Team nicht aufnehmen"]) {
      assert.ok(!mail.subject.includes(word), `the receipt's subject says „${word}“`);
      assert.ok(!readable(mail.html).includes(word), `the receipt says „${word}“`);
      assert.ok(!mail.text.includes(word), `the receipt's text branch says „${word}“`);
    }
    ...
    for (const entered of [ZUSAGE.teamName, ZUSAGE.wunschgegner, "erika@beispiel.de", "069 1234567", "Goethe-Gymnasium"]) {
      assert.ok(!readable(mail.html).includes(entered), `the receipt repeats „${entered}“`);
      assert.ok(!mail.text.includes(entered), `the receipt's text branch repeats „${entered}“`);
    }
  });
});
...
describe("buildBewerbungWiderspruchEmail", () => {
  it("names the person and the role and takes no pronoun for either", () => {
    ...
    const who = `${WIDERSPRUCH.abgelehnt.vorname} (${WIDERSPRUCH.abgelehnt.rolleText})`;
    ...
    for (const sentence of [
      `${who} hat dem Eintrag als Kontaktperson widersprochen.`,
    ...
    for (const pronoun of ["seine Angaben", "ihre Angaben", "seinen Eintrag", "ihren Eintrag"]) {
      assert.ok(!flat(readable(mail.html)).includes(pronoun), `the message takes „${pronoun}“, which misreads for half the names`);
    }
  });

  it("offers the way to start again, as the deletion notice does", () => {
    assert.deepEqual(controlsIn(buildBewerbungWiderspruchEmail(WIDERSPRUCH).html), [
...
describe("the confirmation workflow's messages", () => {
  it("state the same facts in both branches", () => {
    for (const { name, mail } of allWorkflow()) {
      const fakten = factList(mail.html);
  ...
  it("close on the recipient sentence each of them is actually true of", () => {
    for (const { name, mail, empfaenger, footer } of allWorkflow()) {
      assert.ok(flat(readable(mail.html)).includes(RECIPIENT_SENTENCE[empfaenger]), `${name}'s markup branch does not say who it reached`);
  ...
  it("tell a reader who never applied what ignoring the message actually costs", () => {
    ...
    const eintrag = `${opening}: Ist die Bewerbung am ${FRIST} noch unvollständig, löschen wir Deine Angaben zusammen mit der Bewerbung. Oder widersprich dem Eintrag über den Link, dann entfernen wir sie sofort.`;
    const entryForSeveral = `${openingForSeveral}: Ist die Bewerbung am ${FRIST} noch unvollständig, löschen wir Eure Angaben zusammen mit der Bewerbung. Oder widersprecht den Einträgen über die Links, dann entfernen wir sie sofort.`;
    const NOTIZ: Record<string, string> = {
      Bestätigung: eintrag,
      "Bestätigung (Postfach)": entryForSeveral,
      Erinnerung: eintrag,
      "Erinnerung (Postfach)": entryForSeveral,
      "Eingang offen": `${opening}: Ist die Bewerbung am ${FRIST} noch unvollständig, löschen wir sie.`,
      Vollständig: `${opening}.`,
      Gelöscht: `${opening}: Die Bewerbung wird jetzt gelöscht.`,
      ...
      Widerspruch: `${opening}: Am ${FRIST} löschen wir die Bewerbung.`,
    };

    for (const { name, mail } of allWorkflow()) {
      const sentence = NOTIZ[name] ?? "";

      assert.notEqual(sentence, "", `${name} has no note of its own listed here, so this case proves nothing`);
      assert.ok(flat(readable(mail.html)).includes(sentence), `${name}'s markup branch does not say what ignoring costs`);
      assert.ok(flat(mail.text).includes(sentence), `${name}'s text branch does not say what ignoring costs`);
      // Once: `renderHtml` carries it for every message, and one adding its own would say it twice.
      assert.equal(mail.text.split(sentence.slice(0, 40)).length - 1, 1, `${name}'s text branch states the note twice`);
    }
  });
  ...
  it("set every „Saison NNNN“ in the brand colour", () => {
    ...
    for (const { name, mail } of allWorkflow()) {
      const body = mail.html.slice(mail.html.indexOf("</head>"));
      const positions = [...body.matchAll(/Saison 2627/g)].map((hit) => hit.index);

      // The decline states it in its heading alone; every other message states it in its prose too.
      assert.ok(positions.length >= 1, `${name} does not name its season in its body at all`);
    ...
    assert.ok(found > WORKFLOW.length, "the phrase stands once per message at most, so this test proves nothing");
  });
  ...
  it("escape every value they interpolate into the markup", () => {
    const poison = HOSTILE_NAME;
    const cases = [
      { field: "Bestätigung.schule", mail: buildBewerbungBestaetigungEmail({ ...BESTAETIGUNG, schule: poison }) },
      { field: "Bestätigung.fristText", mail: buildBewerbungBestaetigungEmail({ ...BESTAETIGUNG, fristText: poison }) },
      {
        field: "Bestätigung.seats.rolleText",
        mail: buildBewerbungBestaetigungEmail({ ...BESTAETIGUNG, seats: [{ ...ERIKA, rolleText: poison }] }),
      },
      {
        field: "Bestätigung.seats.vorname",
        mail: buildBewerbungBestaetigungEmail({ ...BESTAETIGUNG, seats: [{ ...ERIKA, vorname: poison }, JONAS] }),
      },
      { field: "Erinnerung.schule", mail: buildBewerbungErinnerungEmail({ ...BESTAETIGUNG, schule: poison }) },
      { field: "Erinnerung.fristText", mail: buildBewerbungErinnerungEmail({ ...BESTAETIGUNG, fristText: poison }) },
      {
        field: "Erinnerung.seats.rolleText",
        mail: buildBewerbungErinnerungEmail({ ...BESTAETIGUNG, seats: [{ ...ERIKA, rolleText: poison }] }),
      },
      { field: "Eingang offen.rollenText", mail: buildBewerbungEingangOffenEmail({ ...EINGANG_OFFEN, rollenText: poison }) },
      { field: "Eingang offen.fristText", mail: buildBewerbungEingangOffenEmail({ ...EINGANG_OFFEN, fristText: poison }) },
      {
        field: "Eingang offen.ausstehend.vorname",
        mail: buildBewerbungEingangOffenEmail({ ...EINGANG_OFFEN, ausstehend: [{ vorname: poison, rolleText: "Stellvertretung" }] }),
      },
      { field: "Vollständig.rollenText", mail: buildBewerbungVollstaendigEmail({ ...VOLLSTAENDIG, rollenText: poison }) },
      {
        field: "Gelöscht.ausstehend.rolleText",
        mail: buildBewerbungGeloeschtEmail({ ...GELOESCHT, ausstehend: [{ vorname: "Mira", rolleText: poison }] }),
      },
      {
        field: "Widerspruch.abgelehnt.vorname",
        mail: buildBewerbungWiderspruchEmail({ ...WIDERSPRUCH, abgelehnt: { vorname: poison, rolleText: "Stellvertretung" } }),
      },
      { field: "Widerspruch.fristText", mail: buildBewerbungWiderspruchEmail({ ...WIDERSPRUCH, fristText: poison }) },
    ];

    for (const { field, mail } of cases) {
      assert.ok(!mail.html.includes("<script>"), `${field} reaches the markup unescaped`);
      // Decoding the markup back returns the value, so escaping changed the encoding and not the fact.
      assert.ok(readable(mail.html).includes(poison), `${field} did not survive escaping as the value written`);
  ...
  it("break a link that stands in the prose inside the word", () => {
    for (const { name, mail } of allWorkflow()) {
      ...
      const sentence = name.includes("Postfach") ? FALLBACK_SENTENCE_FOR_SEVERAL : FALLBACK_SENTENCE;
      assert.ok(flat(readable(mail.html)).includes(sentence), `${name} offers an address with no sentence saying why`);
    }
  });
  ...
  it("fold a value carrying a break onto one line", () => {
    const forged = "Startgeld: 500 Euro";
    const mail = buildBewerbungErinnerungEmail({ ...BESTAETIGUNG, schule: `Echte Schule\n${forged}` });

    assert.ok(!mail.text.split("\n").some((line) => line.startsWith(forged)), "a submitted school name forged a fact line");
    assert.ok(mail.text.includes(`Schule: Echte Schule ${forged}`), "the name was not carried onto one line");
  });
});
...
describe("the origin every message's links are built on", () => {
  ...
  const built = (): { name: string; mail: { html: string; text: string } }[] => [
  ...
  const addresses = (part: string): string[] => [...part.matchAll(/https?:\/\/[^"'<\s]+/g)].map((hit) => hit[0].replace(/[,.;:]+$/, ""));
  ...
  it("builds every absolute link on the origin it was handed", () => {
    for (const { name, mail } of built()) {
      const found = [...addresses(mail.html), ...addresses(mail.text)];

      // The close alone carries two, so a message reaching this with fewer is one whose links a
      // reader could not follow at all.
      assert.ok(found.length >= 2, `${name} carries almost no address, so this case compares nothing`);
      for (const address of found) {
        // The bare origin as well as a path on it: the acceptance sets the site's own address in its prose.
        const own = address === SERVIERT || address.startsWith(`${SERVIERT}/`);
        assert.ok(own, `${name} builds ${address}, which does not stand on the origin it was handed`);
      }
    }
  });
  ...
  it("escapes an origin carrying markup in the markup branch and leaves the text branch alone", () => {
    const poison = `https://a"b'c&d.test`;
    const mail = buildBewerbungVollstaendigEmail({ ...VOLLSTAENDIG, origin: poison });

    assert.ok(mail.html.includes(`href="https://a&quot;b&#39;c&amp;d.test/datenschutz"`), "the origin reached the href unescaped");
    assert.ok(mail.text.includes(`Impressum: ${poison}/impressum`), "the text branch must carry the origin as configured");
    for (const raw of ["<", ">", " "]) {
      assert.throws(() => buildBewerbungVollstaendigEmail({ ...VOLLSTAENDIG, origin: `https://a${raw}b.test` }), /absolute origin/);
    }
  });
});
