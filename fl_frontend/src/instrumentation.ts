 ...
 * (https://nextjs.org/docs/app/guides/instrumentation). The type-only import above is erased at
 * compile, so it cannot drag the module back into the Edge graph.
 */
 ...
 * container's healthcheck hits `/favicon.ico` and never touches config, so without this gate a
 * truncated API key or a typo'd admin email gives a green build, a green image and a
 * healthy-looking container that 500s on real traffic.
 *
 * **Three things have to be true for this to work, and each was measured against a built image.**
 ...
 *     silently disables `onRequestError` above, i.e. all server error logging in production.
 *  2. The runtime guard must exclude Edge rather than require Node. `NEXT_RUNTIME` is **unset**
 *     in the standalone server, so `!== "nodejs"` returns early and validates nothing.
 ...
 *     within a second of boot, which is the property this gate exists to give.
 *
