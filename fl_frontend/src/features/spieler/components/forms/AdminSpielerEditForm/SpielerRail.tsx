...
export type SpielerRailBanner = { severity: "info" | "warning" | "danger"; title: string; body: string };

/**
 * The squad editor's summary rail — the club editor's rail over a player's draft, and identical in
 * structure for the same reason: a player has no preview card and nothing waits on a player field,
 * so what remains is every inline warning mirrored into one place plus the unsaved changes by
 * section.
 */
export function SpielerRail({ banners }: { banners: readonly SpielerRailBanner[] }) {
  const status = useSpielerDraftStatus();

  const SEVERITY_RANK: Record<SpielerRailBanner["severity"], number> = { danger: 0, warning: 1, info: 2 };
  ...
  return (
    <div className="flex w-full flex-col gap-y-4">
      <RailSection
        ...
        info={<InfoHint label="Was die Hinweise bedeuten">Alle Warnungen zu diesem Spieler an einem Ort, auch die aus dem Formular.</InfoHint>}>
        {bannerCount === 0 ? (
