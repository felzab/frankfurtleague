...
export function AdminDeleteSpielortModal({ ortData, isOpen, onClose }: { ortData: FLSpielort | null; isOpen: boolean; onClose: () => void }) {
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const res = await deleteSpielortAction({ id: ortData.id });
      ...
      toast.success(res.message || "Spielort erfolgreich gelöscht");
      onClose();
    });
  };
        ...
        <Modal.Dialog className="bg-background border-border text-foreground flex w-full max-w-md flex-col rounded-2xl border p-4 shadow-2xl outline-none">
          ...
          <Modal.Header className="shrink-0 pt-2 pb-4">
            <div className="flex items-center gap-4">
              ...
              <Modal.Heading className="text-fluid-lg text-foreground font-extrabold tracking-tight">Spielort löschen</Modal.Heading>
            </div>
          </Modal.Header>
            ...
            <div className="flex min-h-[80px] flex-col justify-center gap-4 px-3 pt-2">
              {confirmStep === 1 ? (
                ...
                <div className="animate-appearance-in bg-danger/5 border-danger/20 flex flex-col gap-2 rounded-xl border p-4 shadow-sm">
                  ...
                  <p className="text-fluid-sm text-foreground-muted leading-relaxed">
                    Diese Aktion kann <strong className="text-foreground">nicht</strong> rückgängig gemacht werden. Bereits eingetragene
                    Spiele behalten den hier hinterlegten Ort samt Maps-Link — er steht künftig nur nicht mehr zur Auswahl.
                  </p>
                </div>
              )}
            </div>
