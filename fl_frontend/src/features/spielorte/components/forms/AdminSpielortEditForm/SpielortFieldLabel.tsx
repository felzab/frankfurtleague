...
import { useSpielortFieldStatus } from "./SpielortDraftStatusContext";
...
/**
 * A venue has no action-required categories, so the only marker is Geändert, with the previous
 * value a hover away rather than a line that shifts the layout. The wrapper carries the `feld-`
 * anchor id and scroll margin.
 */
export function SpielortFieldLabel({ path, children }: { path: string; children: ReactNode }) {
  const status = useSpielortFieldStatus(path);

  return (
    <div
      id={`feld-${path}`}
      className="flex w-full scroll-mt-28 flex-col gap-y-1">
      {/* `min-h-5` reserves the marker's height, so its arrival never grows the row. */}
