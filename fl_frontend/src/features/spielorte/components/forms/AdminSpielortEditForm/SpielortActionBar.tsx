...
import { useSpielortDraftStatus } from "./SpielortDraftStatusContext";

/** One editor per page, so the id can be a constant rather than threaded through a hook. */
const SAVE_HINT_ID = "spielort-speichern-hinweis";

/**
 * Save, cancel and the running state of the venue draft — the match editor's action bar over this
 * editor's own status context. The reasoning lives on `FormActionBar`, unchanged here: the bar is the
 * scroll container's STATIC sibling, save is disabled only on "nothing changed" and never on a client
 * verdict, and the unsaved count sits where the eye already goes.
 */
export function SpielortActionBar({ isPending, onCancel }: { isPending: boolean; onCancel: () => void }) {
  const status = useSpielortDraftStatus();
