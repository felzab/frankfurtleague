 ...
 *
 * **The id goes in the PATH and never in the body** (ADR-0034). The patch payload schema still carries
...
// matchday is picked up by every consumer on the next read. `saison_id` is not on the payload —
// moving a matchday between seasons would strand its matches, which carry their own.
