...
import { useSpieltagFieldStatus } from "./SpieltagDraftStatusContext";
...
/**
 * A field's label plus what the page knows about that field — `SpielerFieldLabel` over a matchday's
 * draft, with the same single marker: a matchday has no action-required categories, so the only thing
 ...
 */
export function SpieltagFieldLabel({ path, children }: { path: string; children: ReactNode }) {
  const status = useSpieltagFieldStatus(path);
