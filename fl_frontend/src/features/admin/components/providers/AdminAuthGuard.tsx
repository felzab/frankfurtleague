 ...
 *
 * Split out of `admin/layout.tsx` for NEW-SC11. The layout used to `await` this before returning any
 ...
 * the guard returning.
 *
 * `connection()` stays first, per CLAUDE.md §9 A1 — the builder stage has no reachable Mongo, and a
 * session lookup resolved at build time would fail `docker compose build`.
 *
 * **Known trade, accepted when this moved (see the Wave 7 report).** `proxy.ts` matches
