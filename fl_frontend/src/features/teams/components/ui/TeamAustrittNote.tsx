...
export function TeamAustrittNote({ austritt }: { austritt: FLAustritt | null }) {
  ...
  return (
    ...
    <section
      aria-label={zustand}
      className="flex w-full flex-col gap-y-2 rounded-2xl border border-danger/40 bg-danger/15 p-4 sm:p-6">
      {/* `-strong` on a `/15` tint, the pairing the accent tokens were measured at. */}
      <strong className="fluid-xs font-extrabold tracking-tight text-danger-strong">
        {zustand} seit {formatSpielDatum(austritt.datum)}
      </strong>

      <p className="fluid-xs font-medium text-pretty text-foreground">{austritt.grund}</p>
    </section>
  );
}
