 ...
 *   `ausstehend`, unlike the server — a filter selects, a label partitions.
 ...
 *   bracket takes a winner from it.
 *
 * See:
 ...
 * table counts it as a draw, so a card that showed `4:3` where `2:2` belongs would state the
 ...
 *
 * The label is DERIVED, never stored: `quelle` is a reference and carries no German, so this
 ...
 *
 * A `quelle` and a team are independent and all four combinations are stored states, but
 ...
 * that is the admin's AND empty is filled by nobody at all.
 *
 ...
 * bracket slot is fed only by a knockout match of an earlier round. The form derives its
 ...
 * would then compile with that round ranked nowhere.
 */
 ...
 * check against its draft. The write-path refusal is the backend's half; this
 ...
 * advanced from the standings is exactly what the client never does, so this reads only
 ...
 * fed from, and therefore the recommendation the feeder picker marks. The picker's list
 ...
 * strictly earlier round, in bracket order.
 *
 ...
 * Structural typing accepts that object wherever the stored shape is asked for, so
 ...
 *
 * **What the undo toast is built from.** A save that resolves the bracket can delete a
...
/**
 * The requests that put a season back the way it was before one save.
 *
 ...
 * preview is worth requesting at all.
 *
 ...
 * `ausgang` is not compared, because either outcome of this fixture moves the slot.
 *
 ...
 * Empty for a knockout fixture, where the group route cannot apply.
 *
 * **This states the wiring; it does not predict the loss.** Whether a save actually voids a stored
 * result is the dry run's answer — this list only decides whether a preview request is
 ...
 * fixture's slot — and correcting one can empty a slot that should never have been filled.
 ...
 *
 * **A destroyed result gets its own sentence.** A moved `Paarung` and a deleted scoreline
...
export const formatSpielUpdateMessage = (
  ...
  // A sentence of its own, and the whole point of the shape change. A cleared result is a
  ...
  // same Spieltag. Named per fixture rather than counted, because the admin has to know
...
/**
 * Why one derived fault needs a person, in a sentence an admin can act on.
 *
 ...
 * because a placing that is not decided yet needs nobody's attention.
 *
