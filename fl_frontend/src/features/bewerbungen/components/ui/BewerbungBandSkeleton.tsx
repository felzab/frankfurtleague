 ...
 */
export function BewerbungBandSkeleton() {
  const styles = band();
    ...
    <div
      ...
      <div className="flex w-full flex-row items-center gap-3 lg:w-auto">
        ...
        <span className={`${styles.text()} relative block w-full min-w-0`}>
          ...
          <span className={`${skeletonBlock()} absolute inset-y-0 left-0 w-full max-w-[34ch] rounded-md`} />
        </span>
      </div>
      ...
      <span className="relative block w-full shrink-0 lg:w-56">
        ...
        <span className={`${skeletonBlock()} absolute inset-0 rounded-xl`} />
      </span>
    </div>
  );
}
