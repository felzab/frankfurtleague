...
describe("what one delivery event says about a seat", () => {
  ...
  it("reads every subscribed event as the state it leaves", () => {
    ...
    ].map(([type, data]) => leseZustellEreignis(eventFor(String(type), data as Record<string, unknown>))?.stand);
  ...
  it("reads a temporary bounce as trouble rather than as a refusal for good", () => {
    const provisional = leseZustellEreignis(eventFor("email.bounced", { bounce: { type: "Temporary", subType: "MailboxFull" } }));

    assert.equal(provisional?.stand, "verzoegert");
    assert.equal(istDauerhaftUnzustellbar(provisional === null ? null : { stand: provisional.stand }), false);
  });
  ...
  it("does not read a failed send as an address that refuses", () => {
    const failedSend = leseZustellEreignis(eventFor("email.failed", { failed: { reason: "reached_daily_quota" } }));

    assert.equal(istDauerhaftUnzustellbar(failedSend === null ? null : { stand: failedSend.stand }), false);
    assert.equal(failedSend?.grund, "reached_daily_quota");
  });

  it("takes the provider's own token as the reason, never its prose", () => {
    const turnedAway = leseZustellEreignis(
  ...
  it("keeps the state and drops a reason the endpoint would refuse", () => {
    const prose = leseZustellEreignis(eventFor("email.failed", { failed: { reason: "Q".repeat(129) } }));
    const multiLine = leseZustellEreignis(eventFor("email.failed", { failed: { reason: "quota\nexceeded" } }));

    assert.equal(prose?.stand, "verzoegert");
    assert.equal(prose?.grund, null);
    assert.equal(multiLine?.grund, null);
    assert.equal(leseZustellEreignis(eventFor("email.failed", { failed: { reason: "Q".repeat(128) } }))?.grund, "Q".repeat(128));
  });
  ...
  it("takes an offset the provider spells as hours and minutes", () => {
    const shifted = leseZustellEreignis({ ...eventFor("email.delivered"), created_at: "2026-09-08T12:15:00+02:00" });
  ...
  it("carries the event's own message id, which is what tells a superseded message apart", () => {
    assert.equal(leseZustellEreignis(eventFor("email.delivered"))?.nachricht_id, MESSAGE_ID);
  });
  ...
  it("orders on the envelope's instant rather than the message's", () => {
    assert.equal(leseZustellEreignis(eventFor("email.delivered"))?.am, EVENT_AT);
  });

  it("names every seat one message covered, so a mirrored pair moves together", () => {
    const pairedSeat = leseZustellEreignis(
      eventFor("email.bounced", { tags: { bewerbung_id: BEWERBUNG_ID, rollen: "ansprechperson-trainer" } }),
    );
