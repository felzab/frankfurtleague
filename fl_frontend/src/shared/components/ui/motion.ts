 ...
 *
 * For any grid or list of `SpielCard`s. The keyframes, the stagger and the reasoning live in
 * `globals.css`; the short version is that a block fade reads as the content *mutating in place*
 * when each card lands exactly where the previous set's card was, and only separating them in time
 * fixes it. That is why this is not interchangeable with `PAGE_RISE` even though both are 300-400ms
 * fades.
 *
 * Applies to descendants carrying `role="listitem"`, so it goes on the grid container and the cards
