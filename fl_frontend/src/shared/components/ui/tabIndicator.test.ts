...
import { TAB_INDICATOR, TAB_TRACK } from "./formFieldStyles.ts";
...
describe("the mark on the selected tab", () => {
  ...
  it("rings the indicator rather than resting on its fill alone", () => {
    assert.ok(tokensOf(TAB_INDICATOR).includes("ring-1"), `the indicator is ${TAB_INDICATOR}, which draws no ring`);
    assert.equal(paletteToken(TAB_INDICATOR, "ring"), "--accent-brand");
  });
