 ...
 */
const CLIPPED_CLASSES = "overflow-clip";
 ...
 */
const INK_ROOM_CLASSES = "-m-1 p-1";
 ...
 */
export const COVER_LAYER_CLASSES = `${IN_THE_CELL_CLASSES} ${CLIPPED_CLASSES} ${INK_ROOM_CLASSES} max-h-[calc(100000px*var(--admin-region-held))]`;
 ...
 */
export const CONTENT_LAYER_CLASSES = `${COLUMN_CLASSES} ${IN_THE_CELL_CLASSES} ${CLIPPED_CLASSES} ${INK_ROOM_CLASSES} max-h-[calc(100000px*(1-var(--admin-region-held)))]`;
 ...
 */
const AWAITS_ROWS_CLASSES = "[--admin-region-held:1] has-[tbody]:[--admin-region-held:var(--admin-placeholder-hold)]";

/** Nothing to wait for: mapped markup renders whole on its first pass, leaving the 500ms clock alone to hold it. */
const AWAITS_THE_CLOCK_CLASSES = "[--admin-region-held:var(--admin-placeholder-hold)]";
 ...
 */
export const PLACEHOLDER_BOX_CLASSES: Record<AdminCrudShape, string> = {
  table: `${AWAITS_ROWS_CLASSES} ${COLUMN_CLASSES}`,
  cards: `${AWAITS_THE_CLOCK_CLASSES} ${COLUMN_CLASSES}`,
  sections: `${AWAITS_THE_CLOCK_CLASSES} ${COLUMN_CLASSES}`,
};
