...
| `scripts/tests/`                    | any           | pytest over the documentation gate and its copy corpus, the branch checks, the exit contract, the worker handoff, the message gates, the scope classifier, the kernel's floors, the compose mirror, conflict markers and the unit pool's exit statuses |
