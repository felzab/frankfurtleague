...
Both are lifted out of the script and driven behind a stand-in `docker`, so no daemon, no compose
file and no environment file of this machine.
...
BEFORE: Final = 'copy_streams "" refuse "NOTHING has been recreated." ; printf "copied=%s\\n" "$COPIED_STREAMS"'
FAILED: Final = 'copy_streams "-failed" warn "The rollback goes on without it." ; printf "copied=%s\\n" "$COPIED_STREAMS"'
...
def test_the_age_sweep_bounds_the_directory_rather_than_a_name_the_suffix_leaves() -> None:
    """The `-failed` names are only bounded while `docs/ops/runbooks.md` §7's tmpfiles line has no glob in it; a `*.log` pattern would leave them forever."""
...
def test_the_two_call_sites_differ_in_the_suffix_and_in_the_verb_alone() -> None:
    """The pair is the decision: a refusal where nothing has been recreated, an advisory where the site is already down. The rollback's comes first in the file."""
...
def test_any_other_answer_is_an_advisory_the_deploy_survives() -> None:
    """A check that could not be made leaves the deploy where it stood before the check existed; a refusal would stop a deploy over the checker."""
...
def test_the_file_is_mounted_read_only_where_the_image_puts_its_working_directory() -> None:
    ...
    code, output, fixture = _run(NAMES)
