...
**The owner** sends this the way [USAGE.md](USAGE.md) says; **a coordinator reading this file starts
at the block.**

```
Resume this session. Do not continue any work until you have finished this protocol.

1. RECONSTRUCT. Read the register, its resume point first, then the handoff its header names if a
...
5. VERIFY, DO NOT TRUST. Re-establish the last verification result from the register's `Last gate
   run`, and confirm the commit it names is still the tip. Run the gate yourself only once step 2
   ...
   nothing -- least of all a reason to restore the file it names, which is step 3's question and
