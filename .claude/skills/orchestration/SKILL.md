...
- **A fresh session reads the starter prompt's files in the order the starter gives before any work;
  a resumed one runs [resume-prompt.md](resume-prompt.md) to its end before continuing anything.**
- **A quota stop costs exactly what the register does not hold.** Every edit on disk survives it, and
  ...
  incremental for this reason. A resume by the name the register records is documented, not driven,
  so message none to checkpoint and rely on the files. What dies is your judgement since the last
  register edit: the report read and not banked, the commit about to land. So the register's **resume
  point** is rewritten in the same edit as whatever moved it, and an announced stop gets one action:
  bring it current and reply with it. **The resume point is this session continuing; the handoff (§7)
  is the next session's work** — never write one for a pause.
...
carried the session, and one agent resumed by its name kept its context and finished.
...
1. **A commit lands in the turn its report is judged** — a leaf when its agent reports, a hub when
   ...
   action, which fills that row's `Audit dispatched to` cell: a row landed with that cell empty is
   the incident below, and a cycle that ends on a commit has skipped its last step. An audit of
   committed work starts when the commit lands, never with the next wave, and nothing waits for the
   wave boundary, where the tree is reconciled and pushed. Agents propose messages; you write the
   commit.
...
6. **At the boundary, reconcile the ownership map against the whole tree** with
   ...
   measurement and invites a re-plant, and nothing in a tree separates a ten-second plant from an
   abandoned one. Run it mechanically, never by happening to read `git status`.
...
inside six minutes, every SHA different and every subject intact, which is why the handoff and the
resume prompt cite subjects.
...
5. **Dispatch a judging auditor as `cold-auditor`** — no `Edit`, `Bash` or `Agent`, and a hook
   refuses its `Write` inside the repository, while its report still reaches the scratch path. **So
   put no shell command, no plant and no exit code in its brief**: send the template's **cold form**,
   which replaces the read rule too — an agent told to read committed state with no shell substitutes
   the working tree. A re-auditor that must plant needs a shell and goes as `general-purpose` under
   the **driving form**, read-only by prose alone. Pick the type that allows the instrument the
   question needs.
...
- **Every reply to the owner ends with one fleet line: live agents against the cap, what went out
  this turn, and what stays queued with the condition holding each.** Write it last; where it would
  read below the cap with something dispatchable, dispatch first and reply after — the reply is not
  the work. The owner asking about parallelism is a defect report, never a question.
- **Dispatch before you read.** A landed report frees a slot, and the queue's next standing action
  fills it before the report is opened. Then judge the report and land its commit (§5) in the same
  turn.
...
Every false green in this repository's record was a status-carrying bug, which reading cannot see
and driving finds at once.
...
- **A document or plan re-audit walks the previous audit's findings one at a time**, closed or open,
  with the evidence — for code the driven re-audit does this, for a document nothing does.

Incidents. A plan reached a fourth version where the cycle ended at the third, each extra audit
looking justified with a fresh list of findings in hand. A chain of plan audits each read the plan
afresh, none confirming the previous findings had closed.
...
6. **Write the register ([register-template.md](register-template.md)) before the first agent runs,
   naming the session's scratch path** — outside the repository, a subdirectory per agent named for
   it, since agents sharing one directory overwrite each other. You own its top level. **The session's
   one branch exists before the first agent runs** (CLAUDE.md §2); the register names it. Then send
   the owner batch.
