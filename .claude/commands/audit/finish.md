...
6. Present the final report to the owner and ask for explicit confirmation before anything is
   deleted. **The delete is the owner's, run by hand** on a clear yes: `rm -rf docs/audit/programme`
   from the repository root, that folder and nothing else. Nothing in it is recoverable
...
7. Commit the final report and any doc updates, run the bare `./scripts/gate/verify.sh` before
   the push (CLAUDE.md's gate section; a named scope is only for iterating), push, open the draft pull request, print its link, and name the
