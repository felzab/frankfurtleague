...
**One item per session, and this command never starts another.** A session that opens several items
builds the later fixes on decisions the earlier ones have not taken yet.

## Preconditions

Read `docs/_roadmap/items.md` and `docs/_roadmap/protocol.md` in full, find the entry, and work down
this table. **The first row that matches decides.**
...
**The second row is `git log --all --grep="Closes: <ID>"`.** It reaches every entry closed under
that trailer and nothing before the convention started, so an id that resolves to nothing there is
not proof the subject was never concluded — search the log for the subject too where the entry reads
like settled ground.
...
5. **Conclude the item in one commit** —
   [Closing an entry](../../../docs/_roadmap/protocol.md#3-closing-an-entry-one-commit-carrying-a-closes-trailer)
   fixes what that commit holds. What that page leaves to this one:

   - **The commit message carries `Closes: <ID>` as a trailer.** That trailer is the only index of
   ...
   - **A decision's destination is picked by how it fails**: a silent failure to a comment at the
     ...
     commit with none.
   - **`git grep -n "<ID>"` enumerates every reference the commit must update**, which CLAUDE.md's
...
7. **Hand over.** State the mode you were in, what was concluded, whether the entry was deleted or
   ...
   closing commit, because the entry carrying the reasoning is gone by then:
   ...
   If nothing is unblocked, say so.
