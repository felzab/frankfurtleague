...
| Soft deletion is `inactive_since`, a nullable `YYYY-MM-DD` **date, never a boolean** — a flag beside a date can contradict itself and no validator can catch that. **Creating never revives a retired row**; `POST /{resource}/{id}/reactivate` does, and a natural-key collision is a **409**, which is correct rather than a bug. `saisons`, `saison_teams` and `spiele` deliberately have no such field.                                | [ADR-0032](../docs/_decisions/0032-soft-deletion-is-a-date-not-a-flag.md)                    |
| `POST /saisons/{id}/activate` is the **only** code path that writes `status`, in one transaction; `status` is on no payload and there is no `DELETE /saisons/{id}`. **`saison_teams` has a POST and a PATCH and no DELETE** — a team leaves a season only by disqualification. Neither gap is an incomplete CRUD surface to finish. No date guard on activate: that precondition is FB-6's UI, not the endpoint's.                         | [ADR-0033](../docs/_decisions/0033-one-active-season-and-one-path-to-it.md)                  |
| **Two routers per slice**: `router.py` reads under `verify_access_base`, `admin_router.py` writes under `verify_access_admin`, both guards at ROUTER level. Never move a guard onto an endpoint and never merge the files. **`GET /{id}` exists on all seven resources and six have no caller — that is deliberate, not dead code.** Junctions nest under the entity and are addressed by their natural key. One team shape, no `compact`. | [ADR-0034](../docs/_decisions/0034-the-write-path-is-resource-first-in-a-second-router.md)   |
...
- **Documentation names only what exists — comments AND `/docs` alike (owner, 2026-08-02; widened to `/docs` 2026-08-03, [DS14](../docs/_standard/4-decisions.md#ds14--documentation-names-only-what-exists)).** No "used to", "previously", "there used to be", "this was reverted", no narration of a past edit, and nothing documenting the absence of something that is not there. The diff is in git and the reasoning belongs in an ADR; a history note ages badly, lengthens the file without clarifying it, and quietly becomes wrong — and a spec sheet naming a deleted endpoint reads exactly like one naming a live endpoint. Rejected alternatives ARE worth recording — phrase them in the present as a constraint ("never X, because Y"), not as a story about what changed. A **measurement with a date is not history** and stays. Three places are exempt because recording what changed is their job: an ADR's **Context** section, `roadmap/closed-items.md`, and an ADR's `Superseded by` line. Greppable — run this over a branch diff before committing and READ the hits, because "the former … the latter" is ordinary English:

  ```bash
  git diff main...HEAD -U0 | grep -niE "former|used to|was removed|no longer|previously|moved here"
  ```

- **A module header POINTS at the ADR; it never restates it ([DS15](../docs/_standard/4-decisions.md#ds15--a-module-header-points-at-the-adr-it-does-not-restate-it)).** A header section running past about five lines, **or repeated in a second file, is an ADR that has not been written yet** — write it, then reduce the header to the rule plus the number. The **claim** is stated in full; the **argument** is cited. A reader who never opens the ADR must still know not to violate the rule; what they lose is _why_, never _what_.
