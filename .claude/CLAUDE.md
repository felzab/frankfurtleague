...
- **Adding a HeroUI component is a two-part change, and the second part fails silently.** `globals.css` imports HeroUI's CSS **per component**, so a component that is imported in TSX but missing from that block renders unstyled — `tsc`, `next build` and ESLint all pass. **Whenever a HeroUI component is added, or the owner is weighing adding one, restate the checklist in [`docs/frontend/overview.md` § Adding a HeroUI component](../docs/frontend/overview.md#adding-a-heroui-component) before writing code**: add the import at HeroUI's own position in `node_modules/@heroui/styles/dist/components/index.css`, import the sub-components it renders underneath (a picker is a popover + listbox + button), and verify computed styles in the browser rather than reading the diff. Why: [ADR-0019](../docs/_decisions/0019-per-component-heroui-css.md).
