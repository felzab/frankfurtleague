...
exceptions are the pool's alone, so a one-scope run still starts its checks together. **CI's
`images` job installs no interpreter at all** — it creates no backend virtualenv, so whether its two
`docker buildx build` invocations run side by side is left to whichever `python3` the runner image
ships (§4). Each build exports its own `type=gha` cache either way. A step joined after its work ran
beside its neighbours is re-dated to that work's own length (`scripts/_lib.sh :: step_took_ms`),
without which the first step joined absorbs the whole stretch and every step after it reads as free.
...
| CI's images job leaves its own concurrency to the runner image        | Open — the two image builds start together only where an interpreter at the checkers' floor is on PATH (§1.6), and `.github/workflows/verify.yml :: images` creates no backend virtualenv and installs no python, so the job silently builds one image after the other on any runner whose `python3` is below the floor. The scope is correct either way; what varies is the wall clock, and nothing reports which of the two happened.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
