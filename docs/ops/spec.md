...
tag that puts them back on one build (§3), or where the edge answers the liveness probe with
anything but 200. That last read is the only line in the report not taken from a container: nginx
resolves its upstreams once as it loads, so a healthy pair says nothing about whether the edge is
still pointed at it (I9). A `curl` that reached no HTTP status at all is an advisory instead — this
host's own DNS or egress sits between the two.
...
log for, and a truncated host is what the OPS-2 incident was diagnosed from. The match is bounded at
the delimiters userinfo cannot carry unencoded, so a second URI or an option value further along the
line falls outside it, and `scripts/selfcheck.sh :: redact_case` pins both directions against
fixtures. What no filter of this shape reaches is a credential arriving with no URI around it, which
[`docs/logging/spec.md`](../logging/spec.md) section 4 records as accepted.
...
| INC-9 measures none of a renamed file's comment blocks                | Open — `scripts/docs_gate/structure.py :: check_comment_length` reads a block only where every one of its lines sits in the branch's added set, and a detected rename leaves a carried block's lines as context, so no length is measured under the new path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| A citation continuing a file already named is checked by nothing      | Open — `scripts/docs_gate/references.py :: CITATION_RE` needs a file part before the separator, so a continuation resolves to nothing: its anchor is never proved.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
