...
| I7   | Neither `:latest` tag moves until both packages hold the build under its `sha-` tag                                                                                           | `.github/workflows/publish.yml`, whose last step moves both; and `scripts/ops/deploy.sh`, which refuses a pulled `:latest` pair whose `version` labels differ or are absent, before recreating anything                                                                            |
