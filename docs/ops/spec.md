...
| A guard the database tier stays green without                     | Narrowed — dropping `session=` at `judge_and_write_the_rules` turns `--db` (§1.6) red; the other four callbacks in that file are held by the source sweep alone                                    |
