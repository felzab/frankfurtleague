...
| `--docs`     | `check_conflict_markers.py` over every tracked file; `check_docs.py`; `check_commits.py`; `check_public_routes.py`; `check_regenerate_spelling.py`; and `fl_backend/tests/openapi_document.py` in `--check` mode, the published document against the docstrings it is composed from | the backend venv                                                                                                                             |
...
| I11  | The three API keys are 64 characters and match on both sides                                                                                                                  | `fl_frontend/src/core/config.ts` (`length(64)`) and `fl_backend/app/core/config.py :: BackendConfig`, each an equality, so neither side accepts a length the other refuses                                                                           |
...
| I179 | A startup refusal names the failing variables and never their values, at both tiers: a value in a container log is the whole exposure                                         | `fl_backend/app/core/config.py :: get_config` and `fl_frontend/src/core/config.ts :: frontend_config`; `fl_backend/tests/core/test_config.py :: TestTheNamesOnlyErrorPath` and `:: TestTheStartupPing` assert no value appears, on the backend alone |
