...
Both maps were verified by parsing their bodies out of the `.conf` files and driving them through a port of
`ngx_inet6_ntop`: no /64 or /48 prefix split across two keys, no two prefixes sharing one key, no address
reaching the fail-open `default`, and the same result from `nginx/prod.conf` and `nginx/local.conf`
independently. A hierarchy check over separately generated pairs found no two addresses sharing a /64 while
differing in /48, so the wide key never cuts across the narrow one (measured 2026-08-30).
...
one location overriding that server-level value.

**None of these zones has been observed refusing anything.** No request has been issued through a running
edge, so every rate here is reasoned rather than measured, and the `limit_conn` figure in particular is
derived from HTTP/2 semantics rather than from a real page load. What the maps underneath them do IS
measured, above.
...
| `--docs`     | `check_docs.py`, whose checks are CUR-5's table; then `check_commits.py`; then `fl_backend/tests/openapi_document.py` in `--check` mode, the published document against the docstrings it is composed from | the backend venv, plus node and an `fl_frontend` install for full-fidelity branch impact |
...
is the one currency defence that does not depend on somebody remembering, and its checks are listed in
CUR-5's table and nowhere else.
...
| I2  | Security headers are repeated in every `location` that sets any header                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `nginx/prod.conf :: location /_next/static/` and `:: location = /api/v0/system/is_live`, each restating the set its own `add_header` replaced                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| OPS-86 | A change to a root-level file asks no page to be re-verified          | Open — `scripts/docs_gate/kernel.py :: repo_path` discards a token carrying no slash, so neither compose file, `.gitignore`, `.prettierignore` nor `.gitattributes` can enter a stamped page's watch list, and CUR-4 never arms on one however many pages cite it. This page cites all four. Tracked in [`docs/_roadmap/tooling-items.md`](../_roadmap/tooling-items.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| OPS-88 | The documentation corpus is the git index                             | Open — `scripts/docs_gate/kernel.py :: tracked_files` enumerates through `git ls-files` with no `--others`, so a file the branch has written but not staged is in no check that reads the corpus, and the run that clears a branch before its commit reads less than CI's does over the committed tree. The branch-scoped checks are half-sighted rather than blind: `scripts/docs_gate/branch.py :: check_branch_impact` takes its diff from `scripts/check_scope.py :: changed_files`, which lists untracked files too, while enumerating pages from the corpus, and `scripts/check_scope.py :: is_comment_only` finds no version of an untracked file at the fork and so calls it material. The second consequence inverts the failure mode: `scripts/docs_gate/references.py :: _resolve` reaches the corpus for a bare filename alone — a citation carrying a path resolves against the filesystem — so a comment naming an untracked file by name alone fails as `cited file not found`, a false finding in a true one's shape, and the repair it invites is repointing the comment at a similarly named file, which `scripts/docs_gate/references.py :: _check_citation` then passes because it asks only that the anchor text appear inside whichever file the citation names. Tracked in [`docs/_roadmap/tooling-items.md`](../_roadmap/tooling-items.md)                                                                      |
| OPS-89 | A form dispatched from a route handler is swept by nothing            | Open — `fl_frontend/src/core/refusalPaths.test.ts` reaches a route-parsed payload through its `ROUTE_FORMS` table, so the public application form's parse is now inside the sweep that holds [`docs/frontend/spec.md`](../frontend/spec.md) I34; what stays outside it is the hand-written half, no `map*Refusal` being read by the sweep at all, so a mapped path reaching no input is held by review alone. Tracked in [`docs/_roadmap/tooling-items.md`](../_roadmap/tooling-items.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| OPS-91 | A citation continuing a file already named is checked by nothing      | Open — `scripts/docs_gate/references.py :: CITATION_RE` needs a file part before the separator, so a continuation resolves to nothing: its anchor is never proved and the file it means is never armed. Tracked in [`docs/_roadmap/tooling-items.md`](../_roadmap/tooling-items.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
