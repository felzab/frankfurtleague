...
| Scope        | Runs                                                                                                                                                                                                                                                                                        | Needs                                                                                                                                        |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `--scripts`  | `selfcheck.sh`, `ruff` and `pyright` over the python in `scripts/`, and the pytest suite in `scripts/tests/` (§1.5), itself distributed a file at a time — the argument is at `scripts/gate/verify.sh :: do_pytest`                                                                         | the backend venv, `pytest` included; shellcheck and actionlint from PATH, else Docker                                                        |
| `--docs`     | `check_conflict_markers.py` over every tracked file; `check_docs.py`, whose registry is `scripts/checks/docs_gate/kernel.py :: CHECKS`; `check_commits.py`; and `fl_backend/tests/openapi_document.py` in `--check` mode, the published document against the docstrings it is composed from | the backend venv                                                                                                                             |
...
| `--db`       | `pytest -m db -n auto --dist loadfile`, capped at `scripts/gate/verify.sh :: GATE_WIDTH_DB_PYTEST`, against the two real `mongod`s the xdist controller starts and every worker shares (`docs/backend/spec.md` §1.6)                                                                        | venv + Docker                                                                                                                                |
...
`scripts/gate/verify.sh :: run_checker`'s `annotate` mode, which prints a passing run's capture, its
...
log for, and a truncated host is what the OPS-2 incident was diagnosed from. **The match ends at the
...
| I15 | Every platform-conditional branch in the Python and the shell `scripts/checks/docs_gate/platform.py` reaches is a named module constant or an allowlist row carrying its reason, and every such constant is driven to both values by a test (§1.6)                                                                                                                                                                                                                                                                                              | gate check `platform-branch`, over `scripts/checks/docs_gate/platform.py :: PLATFORM_ALLOW`; the effect a branch selects is proven by the `verify` workflow's Linux run alone                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| I16 | No Python in those same scopes opens a text-mode writer without `newline=""`, so no file the gate or a test writes reaches a Linux shell carrying CRLF (§1.6)                                                                                                                                                                                                                                                                                                                                                                                   | gate check `crlf-write`, over `scripts/checks/docs_gate/platform.py :: TEXT_WRITE_ALLOW`; a shell redirect of a program's stdout carries no call to read and stays the reader's                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| I17 | No job of the `verify` workflow spans longer than the budget its row in `.github/gate-wall-clock.tsv` gives it, no job runs without a row, and no budget or reference there rises without a new measurement stamp (§1.6)                                                                                                                                                                                                                                                                                                                        | `scripts/checks/check_gate_budget.py`: `--jobs` in the aggregate `verify` job on every event, `--base` in the `commits` job on every pull request; `scripts/tests/test_check_gate_budget.py` drives the committed table red and green and holds the workflow's job keys to its rows. A slowdown under the ceiling is the advisory report's to name and the `gate` clause's to forbid                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `./scripts/ops/deploy.sh --status` exits 1 over a pair it has just called healthy | The edge is not serving them — nginx resolved its upstreams as it loaded, and nothing has re-resolved them since those containers were replaced              | Reload the edge, then re-run `--status`. `scripts/ops/deploy.sh :: serve_through_nginx` is what a deploy reloads it with, and the report's own detail line names the command to run by hand                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `./scripts/ops/local.sh` reports `mongo` unhealthy                                | The local database has not elected itself primary, so no transaction opens and no validator applies                                                          | `docker compose -f docker-compose.local.yml logs mongo`. The script waits on `mongo` by name so this reports as itself rather than as two services that never started                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| The local stack's data disagrees with production, in either direction             | Working as intended — `--seed` reuses the copy already on disk however old it is, so it holds rows production has dropped and lacks rows it has since gained | `./scripts/ops/local.sh --refresh-db` takes a fresh one (§1.5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Item                                                                  | State                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The API version is spelled outside the code                           | Open — every spelling works today and an API version bump has to reach all of them. The tracked files one commit moves together are the backend healthcheck in each compose file (`docker-compose.yml :: backend`, `docker-compose.local.yml :: backend`), the liveness location in each nginx config (`nginx/prod.conf :: location = /api/v0/system/is_live`, `nginx/local.conf :: location = /api/v0/system/is_live`) and the deploy script's probe URL (`scripts/ops/deploy.sh :: PROBE_URL`), which two steps read — the deploy's `The liveness probe, through the edge` and `--status`'s `The site, as the internet reaches it` — and which the deploy step spells a second time, in the message it fails with. The healthcheck and the location each break silently, and §3 carries both symptoms; the probe breaks loudly instead — left at the old version it asks the edge for a path no location names, and both readers grade that answer with `fail` rather than `warn`, so either run counts a finding and exits 1: the deploy of a healthy site withholds its closing `The pulled build is live.` on top of that, and `--status` reports the same edge as a finding of its own. The remaining site is the frontend's `API_VERSION` environment value (`fl_frontend/src/core/config.ts :: frontend_config`, which `fl_frontend/src/core/api.ts` builds every call's base URL from, constrained by [`docs/frontend/spec.md`](../frontend/spec.md) §1.7) — a deployed per-environment value rather than a tracked file, so no commit can carry it and the deploy has to. Left behind, it breaks loudly and all at once: every frontend fetch targets the old version, matches `location /` and collects Next's HTML 404 (I13) |
| A rollback moves nothing in the registry                              | Accepted — `scripts/ops/deploy.sh :: roll_back` re-tags this host's local `:latest` at the image ids preflight recorded, and nothing in the deploy path reaches ghcr, which would need a token the server deliberately does not hold (§1.5). So the registry's `:latest` still resolves to the build that failed: a bare `./scripts/ops/deploy.sh` afterwards pulls it back, fails again and costs a second full outage, and `--status` reports the rolled-back build while the registry disagrees. The rollback says so where it lands, naming the tag to deploy instead, and [`runbooks.md`](runbooks.md) §1 carries the procedure                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| INC-9 measures none of a renamed file's comment blocks                | Open — `scripts/checks/docs_gate/branch.py :: check_comment_length` reads every block the branch added a line to, and a detected rename adds none: a carried block's lines arrive as context, so no length is measured under the new path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| No gate scope COMPARES `nginx/local.conf` against production's        | Open — its header claims production's routing, rate limits and security headers, and nothing reads that claim: `nginx -t` runs against `prod.conf` alone and `scripts/checks/check_compose_mirror.py` compares the compose pair (§1.6). **The parse half is closed**: `nginx/redaction_test.sh` serves `local.conf` itself in the pinned image inside the ops scope, and a config nginx cannot load never answers the readiness probe that file starts on, so the step fails. What stays open is the comparison itself, and §1.3 records one directive the two files spell identically and HTTP/2 makes behave differently.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
