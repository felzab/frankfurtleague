...
| I7   | Neither `:latest` tag moves until both packages hold the build under its `sha-` tag                                                                                           | `.github/workflows/publish.yml`, whose last step moves both; and `deploy.sh`, which compares the pulled `:latest` builds' `version` labels before recreating anything                                                                                                              |
