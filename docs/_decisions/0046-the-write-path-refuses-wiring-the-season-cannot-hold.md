# ADR-0046 — The write path refuses wiring the season cannot hold, and the form offers only what it can
...
ADR-0042 made the bracket resolve itself from `teamN_quelle` and, deliberately, enforced no
...
- **A team was picked onto a side a source maintains.** The resolution reverted it inside the same
  request — ADR-0042's rule working as specified — so the admin saw a success toast and no effect.
...
2. **A `spiel` source names a knockout match of the same season in a strictly earlier round.** A
   ...
   (ADR-0042: the first knockout round is seeded from the standings).
...
no source is the admin's own and is maintained by nobody (ADR-0042), so it is a seventh
...
**ADR-0042's "referential integrity is not enforced" consequence is narrowed, not reversed.** It
remains true of the resolution and of hand-edited data; it is no longer true of the endpoint. The
decision that ADR records — the reference model, the resolution, the containment — stands, which is
why this ADR complements rather than supersedes it.

**A stale form is now an explicit 409 where it used to self-heal silently.** Editing a venue on a
fixture whose occupant the resolution moved since the page loaded submits the old occupant beside
the source, and rule 4 refuses it. Before, the write succeeded and the resolution re-reverted the
side in the same transaction — invisible, and indistinguishable from the reproduction above. The
...
why the dialog finally holds its season — the cost ADR-0042 recorded as the reason the form took a
typed number.
