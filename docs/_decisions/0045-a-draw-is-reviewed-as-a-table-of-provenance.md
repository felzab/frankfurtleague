# ADR-0045 — A draw is reviewed as a table of provenance, on a page that writes nothing
...
([ADR-0034](0034-a-result-entry-resolves-the-whole-bracket.md)) and **no surface showed it**. Two
...
- **The triage list reports faults, and a wrong draw is not a fault.** All five stored bracket faults
  ...
  ([ADR-0039](0039-a-bracket-fault-is-derived-on-demand.md)), and the write path refuses wiring the
  season cannot hold ([ADR-0038](0038-the-write-path-refuses-wiring-the-season-cannot-hold.md)). What
...
- **`SpielCard` is the match card on every surface, admin routes included**
  ([ADR-0044](0044-a-triage-list-is-ordered-by-what-blocks-play.md), and I rejected a
...
- **A fixture is edited in one place**, `/admin/spiele/[spiel_id]`
  ([ADR-0040](0040-a-form-that-outgrows-a-dialog-becomes-a-page.md)).
...
a match card and ADR-0044's rule is not engaged rather than worked around. It also puts the surface in
...
write surface to keep in step with the endpoint's four refusal rules (ADR-0038), and Next's E592 stays
out of reach ([ADR-0049](0049-every-page-owned-editors-undo-is-a-route-handler.md)).
...
admin-authorized and [ADR-0009](0009-admin-scoped-reads-are-never-cached.md)'s no-cache rule does not reach
...
from the schedule and the write path refuses wiring on one (ADR-0038, rule 1), so there is nothing
...
over whole seasons by the admin route and already have a durable home in the triage list (ADR-0039),
...
([ADR-0028](0028-reference-data-staleness-is-bounded-by-cache-lifetime.md)).
...
twice (ADR-0044). The topology is also the half that is already correct and already visible; what was
...
**Put a wiring summary beside the existing card grid**, in the shape ADR-0044 prescribes for anything
