...
the dry run replaces a static warning shipped the same day (retired number 0048), reversed after
...
ADR-0042 and ADR-0044 both require it — goals scored by a team no longer in the fixture are not that
...
The first answer to that principle (retired as 0048) had two halves. The response half was right
...
value ADR-0042 bought is that a corrected group result flows to the final with no manual
...
[ADR-0043](0043-a-group-placing-is-ranked-by-one-chain-and-seeded-only-when-final.md) keeps off the
