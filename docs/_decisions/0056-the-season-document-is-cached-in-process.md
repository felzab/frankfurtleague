# ADR-0056 — The season document is cached in-process, dropped by its own write path, bounded by a TTL
...
[ADR-0019](0019-team-statistics-are-derived-from-spiele.md) then widened the cost: `GET /teams`
...
[ADR-0050](0050-a-matchday-list-is-the-seasons-skeleton.md) gave `PATCH /saisons/{saison_id}` and
...
[ADR-0028](0028-reference-data-staleness-is-bounded-by-cache-lifetime.md) refuses. With the TTL,
every layer's staleness is bounded by a cache lifetime: at most ten minutes here, at most a day in
the frontend's reference caches, and the remedies ADR-0028 documents (recreate the container, or
...
ADR-0028's refusal of one stands untouched.
...
"bounded by cache lifetime" — the property ADR-0028 built the operational story on — would silently
...
ADR-0050) says writes observe their effects immediately. The drop is three one-line calls.
