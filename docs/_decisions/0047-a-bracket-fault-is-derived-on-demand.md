# ADR-0047 — A bracket fault is derived on demand, and all five reach the list an admin already reads
...
than it reports (ADR-0042). Containment is the right behaviour and is not in question here.
...
(ADR-0043) — were reported in the response to the write that computed them and stored nowhere, so
...
The first two are unreachable through the write path since ADR-0046 and are therefore hand edits. The
...
ADR-0046 had already answered the "where" question for the sibling fault: an unwired knockout slot is
...
**The `spiele` read on that route is no longer season-scoped.** Its 1024-document cap now bounds the
whole archive rather than one season — about thirty seasons at today's size. An unread fixture would
make every reference to it read as dangling, so the boundary is named at the read.
...
`fl_backend/openapi.json` move with it (ADR-0040).
...
**Nothing is reported that waiting fixes.** A placing merely undecided stays invisible, exactly as
before (ADR-0043, invariant I24c) — surfacing it would raise a notice on every group-seeded slot for
...
precisely what ADR-0026 exists to avoid.
...
the morning and refused in the afternoon, because a match was played in between. ADR-0046's rules are
