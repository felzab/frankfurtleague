# ADR-0066 — The domain model is declared as data and conformance-checked, not evaluated at runtime
...
([ADR-0042](0042-a-result-entry-resolves-the-whole-bracket.md)). `teams` looks like it belongs to a season
...
([ADR-0028](0028-store-what-was-true-then-derive-what-is-true-now.md)); a matchday retirement touches no
...
test is what [ADR-0031](0031-the-third-copy-of-the-schema-is-checked-not-generated.md) ratified for the
database validators and [ADR-0040](0040-the-zod-mirror-is-checked-against-the-published-document.md) for the
...
Rejected for ADR-0031's reason: a generated declaration agrees with its source by construction, so it
...
emptying it ([ADR-0047](0047-a-bracket-fault-is-derived-on-demand.md)). A generic layer would have to be
