# ADR-0058 — One bar across the viewport owns every page's title, and the navigation structure declares it
...
**`AdminCrudShell` no longer takes a title or a description**, and the two CRUD copy modules keep only
their search strings. A reader looking for those pages' headings will find them in
...
`ThemeSwitch` is reachable from `TopNav` on every public route (ADR-0023, and the checklist in
