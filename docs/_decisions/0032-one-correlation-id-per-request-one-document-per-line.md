# ADR-0032 — One correlation id per request, and one JSON document per line on every surface
...
No line of a successful request was correlated on any surface. The frontend minted a trace id **per
fetch call** — one page view produced eight distinct ids — and the id appeared in a log only when a
request failed. The backend had a real request scope but uvicorn's access log never saw it. nginx had
no logging configuration at all: no edge latency record, no id, nothing to join to either application
log.
...
the frontend) and the full field reference live in [`docs/logging/spec.md`](../logging/spec.md),
which is the maintained convention this ADR only argues for.
...
- A client-supplied `X-Correlation-ID` is replaced at every surface. Correlation with an external
  caller's own ids is deliberately traded away for log-injection safety.
