# ADR-0032 — Soft deletion is a date, not a flag, and creating never revives
...
validator can express that they must not**. ADR-0027 put cross-field rules out of the validators' scope
...
- **`saisons`** — `status: past` is what "gone" means for a season, and a season is never deleted at
  all. See [ADR-0033](0033-one-active-season-and-one-path-to-it.md).
- **`saison_teams`** — a team never leaves a season once squads are settled; the only way out is
  disqualification ([ADR-0033](0033-one-active-season-and-one-path-to-it.md)). The junction has a POST
...
unrecoverability argument above. It is worth stating what makes this one different from an ordinary
convenience: the failure is silent, it is discovered by reading historical match cards that now name
the wrong club, and there is no record of what the rows used to point at. A 409 that an admin has to
think about is the correct cost.
