# ADR-0048 — A player's position and stufe are closed sets
...
inspection that made the question unavoidable is in the Context below.
...
are already nullable, and a squad filled in over time is exactly what ADR-0025 records as normal.
...
directions of the change pull against each other exactly as ADR-0034's did: the validator refuses
...
rather than an overwrite. No migration tooling is added: ADR-0025 settled that a one-off ships as a
runbook rather than as a permanent file with one day's purpose.

## Consequences

**A fourth spelling of an existing position cannot be entered**, by the API or by hand. That is the
whole point, and it is also the cost: a genuinely new value needs a code change and a deploy rather
than a keystroke. Six values across two fields, in a league of school teams, is a rate
...
[ADR-0020](0020-the-database-enforces-its-own-invariants.md) is why types, presence and enums
are the validator's scope and ranges and formats are not;
[ADR-0024](0024-the-third-copy-of-the-schema-is-checked-not-generated.md) is why the validator
is transcribed and checked rather than generated;
[ADR-0025](0025-soft-deletion-is-a-date-not-a-flag.md) is the runbook convention, and why a
...
the shape ADR-0034 and ADR-0047 both declined for the same reason: the data is ordered against the
