# ADR-0051 — A matchday's position and name are derived, not stored
...
the stored name by the same argument, which a decision since retired recorded separately.
...
[ADR-0034](0034-a-result-entry-resolves-the-whole-bracket.md) says so outright — "`spieltage.order_val`
...
documents ([ADR-0020](0020-the-database-enforces-its-own-invariants.md)), and a unique index would forbid a
...
**`spieltage.name` had the same shape**: a required string, set by whoever created the matchday,
restating two fields the document already held — a group-phase matchday's name is its place in the
order this decision derives, and a knockout matchday's name is its `saison_phase`. Both ways of
disagreeing were reachable, and neither is a hypothetical class of typo, because the name was free text
on a create form and the phase was a select beside it:
...
[ADR-0034](0034-a-result-entry-resolves-the-whole-bracket.md) set the precedent: a bracket reference carries
...
**Neither a shared name nor a name contradicting its phase is expressible** — not because a rule
refuses either, but because there is no field to hold one.
...
declare the same field set ([ADR-0024](0024-the-third-copy-of-the-schema-is-checked-not-generated.md)), so
...
accepts is a season whose columns order correctly, which a hand-set number could not guarantee.
...
([ADR-0052](0052-a-seasons-schedule-is-derived-from-its-rules.md)) and the name (here) all left the
...
**A bulk reorder endpoint, so positions could be dragged.** ADR-0050 declined this while the field existed;
...
**Derive the name on the backend and serve it as a read-only response field.** This is the shape ADR-0052
...
a wording change is made. ADR-0034 already drew that line for `quelle`, and drawing it differently here
