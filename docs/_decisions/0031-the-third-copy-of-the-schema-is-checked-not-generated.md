# ADR-0031 — The third copy of the schema is checked by a test, not generated
...
**Supersedes:** —
**Superseded by:** —
**Source:** Open item DB-2, implementing [ADR-0027](0027-the-database-enforces-its-own-invariants.md).

## Context

[ADR-0027](0027-the-database-enforces-its-own-invariants.md) accepted a third copy of the schema —
...
| --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `FLSpielTeamField.model_json_schema()["properties"]["team_id"]` is `{"type": "string"}` | That every ObjectId reference **is a string** — blessing `"Lessing-Gymnasium"`, the exact defect ADR-0027 exists to refuse |
| `tore` emits `{"anyOf": [{"minimum": 0, "type": "integer"}, {"type": "null"}]}`         | A range ADR-0027 puts out of scope, plus an `anyOf` where `bsonType` is the BSON-aware spelling                            |
| `FLTeam.model_fields` includes `gruppe`, `is_disqualified` and `statistik`              | Three fields that are on no `teams` document: two come from the junction, one is derived and stored nowhere (ADR-0026)     |
...
`test_no_validator_constrains_a_range_or_a_format` enforces ADR-0027's scope in the same file. Widening
...
about is, for this pair, no longer a risk that depends on anybody remembering. Note that this is
strictly stronger than generation in one respect: a generator makes the copies agree by construction
and therefore cannot tell you the model itself is wrong, whereas the test makes a person look at the
field twice — once as an API shape, once as a stored shape. For `FLTeam` and `FLSpieler` the difference
between those two is real information, and it now has a home.
...
real `mongod` (ADR-0030) — a slower net, in the deselected tier, but an exact one.
