# ADR-0047 — A disqualification is a record, and its absence is the null
...
disqualification is the **only** way out: [ADR-0026](0026-one-active-season-and-one-path-to-it.md)
...
- **Nothing may hold a second copy.**
  [ADR-0021](0021-store-what-was-true-then-derive-what-is-true-now.md) rule 4 makes this field joined
...
- **The database can assert types, presence and enums, and nothing else**
  ([ADR-0020](0020-the-database-enforces-its-own-invariants.md)). An enum is the only lever a
...
[ADR-0025](0025-soft-deletion-is-a-date-not-a-flag.md)'s shape applied to a second question and it is
...
[ADR-0036](0036-a-shoot-out-is-its-own-scoreline.md) both used:
...
3. **After the deploy**, `$unset` `is_disqualified`. It is last because the running image reads it
   until the moment it stops running, and `additionalProperties` is never `false` (ADR-0020), so the
...
that is still live. No migration tooling is added, for the reason ADR-0025 settled: a one-off that
...
meaningful, and `min_length=1` on `grund` stays Pydantic's, which is the line ADR-0020 draws and
...
one. ADR-0025 rejected exactly this arrangement for `inactive_since` and the argument transfers without
modification.
