# ADR-0036 — A shoot-out is its own scoreline, read by the bracket and by nothing else
...
[ADR-0034](0034-a-result-entry-resolves-the-whole-bracket.md) and
[ADR-0035](0035-a-group-placing-is-ranked-by-one-chain-and-seeded-only-when-final.md) built.

## Context

The bracket maintains itself in both halves. The first knockout round seeds from the group standings
once a placing is beyond doubt (ADR-0035), and every round after it is filled from the round before
(ADR-0034). One shape defeats all of it: **a knockout that finishes level.**
...
slot's `quelle` hands it to the admin, who types a side in by hand (ADR-0034). What it cost was the
...
- **`statistik` is derived from the match documents on every read** and stored nowhere
  ([ADR-0019](0019-team-statistics-are-derived-from-spiele.md)), so whatever is stored here is read by
...
- **A `$jsonSchema` validator may assert types, presence and enums and nothing else**
  ([ADR-0020](0020-the-database-enforces-its-own-invariants.md)), so no rule relating this field to
...
- **Two fields that can contradict each other are refused on principle.** An `is_manual` flag beside
  `quelle` was rejected for it (ADR-0034) and `inactive_since` is a date rather than a boolean beside
  one ([ADR-0025](0025-soft-deletion-is-a-date-not-a-flag.md)).
...
**`ergebnis` does not absorb it**, for the reason the first rejected alternative gives.
...
taken by a side the fixture does not hold, so `advance_bracket_winners` clears it alongside `ergebnis`.
Leaving it would hand the slot below a winner derived from a match neither side played.
...
**A third production data change is owed before the next deploy**, in ADR-0034's shape and for its
...
No migration tooling is added, for the reason ADR-0025 settled: a one-off ships as a runbook rather than
as a permanent file with one day's purpose. It is independent of ADR-0034's and ADR-0035's own changes,
...
counts must differ (ADR-0020) and Pydantic is therefore the only place the rule can live. That is the
...
expressible: a cancelled match carrying a result is a forfeit, it counts in the table (ADR-0019), and
...
copies maintained by the rename fan-out (ADR-0021, rule 3), `tore` is already the one exception, and a
...
are out of its scope entirely (ADR-0022), and a rule written for the knockout rounds would be a scoring
...
merely contradictory, which is the trap ADR-0034 records for pairing `quelle` with its team field.
