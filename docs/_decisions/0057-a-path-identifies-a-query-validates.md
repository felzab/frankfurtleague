# ADR-0057 — A malformed id is a 404 in a path and a 422 in a query, and that split is the rule
...
([ADR-0027](0027-the-write-path-is-resource-first-in-a-second-router.md)) — and with an
...
reads that way: `docs/backend/spec.md` §1.4 states it beside the other failure contracts.
...
(`docs/backend/spec.md` §4 tracked it as such, and that row leaves with this decision).
