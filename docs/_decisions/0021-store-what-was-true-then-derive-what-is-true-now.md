# ADR-0021 — Store what was true then; derive what is true now
...
[ADR-0019](0019-team-statistics-are-derived-from-spiele.md): if statistics should be derived rather
...
Read as a list, every row is the same defect ADR-0019 just removed. It is not, and treating it as one
...
1. **A derived aggregate is never stored.** A pure function of other documents' current state has one
   ...
   settled by ADR-0019; `spieltage.anzahl_spiele` is the same shape and is carried in DB-2.
...
be normalised?" now has an answer that does not require re-deriving ADR-0019's argument.
