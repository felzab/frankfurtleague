# ADR-0028 — Reference-data staleness is bounded by cache lifetime, not by an endpoint
...
**Supersedes:** —\
**Superseded by:** —\
**Source:** Decision of 2026-08-04, removing the interim revalidation route a retired decision had introduced.
...
An earlier decision, since retired, answered that with `POST /api/revalidate` — an in-network
...
- Removing the route removes the only cache control the frontend exposed over HTTP. The Next-side
  handlers nginx's routing table carves out of `/api` — `/api/auth`, the client-error ingest, and the
  page-owned editors' undo handlers
  ([ADR-0049](0049-every-page-owned-editors-undo-is-a-route-handler.md)) — invalidate nothing, so
  that table protects no revalidation surface.
...
- `INTERNAL_API_KEY_SYSTEM` stays: the frontend still uses it to call the backend's system
  endpoints ([ADR-0010](0010-keep-the-system-endpoints.md)); the route was one consumer, not the
  reason it exists.
