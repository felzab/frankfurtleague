# ADR-0030 — The gate refuses a run that skips the image build, and only reports the rest
...
CLAUDE.md's gate section, `docs/ops/spec.md` and `docs/_git/spec.md` — including its
...
- `verify.sh` ends with a warning that names the file asking for the scope, not a blanket one. A
  blanket warning fires on every scoped invocation regardless of the diff, which is the definition of
