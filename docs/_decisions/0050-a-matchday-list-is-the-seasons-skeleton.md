...
([ADR-0040](0040-a-form-that-outgrows-a-dialog-becomes-a-page.md)). Copying them is what the entry
...
- **Does `anzahl_spiele` match what is attached?** It is a hand-maintained count of something countable,
  ...
  [ADR-0019](0019-team-statistics-are-derived-from-spiele.md)'s derivation pointedly does not extend to
...
`status`, and [ADR-0026](0026-one-active-season-and-one-path-to-it.md) deliberately refused a backend
...
([ADR-0051](0051-a-matchdays-position-is-derived-not-stored.md)), so the order arrives correct and a second
...
**The matchday editor is a dialog, and the season editor is a page.** ADR-0040's threshold is a form that
...
inverts [ADR-0041](0041-a-voided-result-is-named-before-it-is-lost.md)'s preference on purpose. That
...
**Every soft delete in the admin now says so** (my call, 2026-08-07). Every `DELETE` endpoint stamps
`inactive_since` and keeps the document ([ADR-0025](0025-soft-deletion-is-a-date-not-a-flag.md)), and the
...
hand-maintained count in the system with a check. It is still not derived, and ADR-0019 still does not
extend to it.

**Two pages end ADR-0028's staleness window for `saisons` and `spieltage`.** Every write invalidates its own
tags as it saves, which is the durable fix
[ADR-0028](0028-reference-data-staleness-is-bounded-by-cache-lifetime.md) deferred to. A hand edit made
...
from `rules.win_points` and `draw_points` on every read (ADR-0019).
...
([ADR-0035](0035-a-group-placing-is-ranked-by-one-chain-and-seeded-only-when-final.md)), and
...
against a season's list ([ADR-0048](0048-position-and-stufe-are-closed-sets.md) holds it against the
...
**A fourth undo route handler exists.** ADR-0049 replaced a count with a pattern precisely so this would not
...
option and declined, then made moot: ADR-0051 removed the stored position a reorder would have written, so
...
the shape ADR-0026 assigned this UI to avoid.
...
**Refuse the rollover while the outgoing season has unfinished matches.** Already refused by ADR-0026 for
...
ADR-0040's threshold is a measurement rather than a preference: five scalar fields do not produce the
