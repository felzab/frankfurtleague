# ADR-0044 — A triage list is ordered by what blocks play, and its section lives in the URL
...
`/admin/spiele/[spiel_id]` ([ADR-0040](0040-a-form-that-outgrows-a-dialog-becomes-a-page.md)), addressed
by fixture id alone.
...
deliberately uncached ([ADR-0009](0009-admin-scoped-reads-are-never-cached.md)). `replaceState` and not
...
the tab cannot state it, and ADR-0039 already refused splitting it into five mostly-empty sections.
...
Neither strip carries those classes.
...
`docs/frontend/spec.md` §1.10 records against the match editor's draft, and the reason that editor needs
...
([ADR-0049](0049-every-page-owned-editors-undo-is-a-route-handler.md)). Every action on this page is a
...
[ADR-0039](0039-a-bracket-fault-is-derived-on-demand.md) is the derivation this renders and
the fault panel's placement, both unchanged;
[ADR-0040](0040-a-form-that-outgrows-a-dialog-becomes-a-page.md) is the page every card links
into; [ADR-0009](0009-admin-scoped-reads-are-never-cached.md) is why the read is uncached,
which decides the transport; [ADR-0005](0005-three-spiel-cards-stay-separate.md) is why this
page renders an existing card rather than a fourth. The URL-state rule and the sibling hazard
are `docs/frontend/spec.md`'s, invariant I24 and section 1.10.
...
category set that ADR-0038 and ADR-0039 build on and that the match editor reads through
