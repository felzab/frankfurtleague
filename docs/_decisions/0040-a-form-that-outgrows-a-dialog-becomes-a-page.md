# ADR-0040 — A form that outgrows a dialog becomes a page, and it judges a field when the field is left
...
action. What ran at render time was prevention — the source pickers offer only legal options (ADR-0038),
...
by ADR-0041) and FB-9's eligibility feedback (now ADR-0042, implementation deferred at the time).
...
and a reload returns to it. `GET /spiele/{spiel_id}` — kept by ADR-0027 for uniform addressability and
...
**Two admin routes stop serialising four lookup lists.** `AdminContextWrapper` is mounted by the
editor's own route rather than by the list routes, so `/admin/action_required` and
`/admin/spielsuche` send the client no referee, venue, team or fixture list for cards that never
used them.
...
bracket and rewrites fixtures the request never named (ADR-0034), so nothing narrower describes what it
...
**ADR-0004's constraint holds and its supplier moves.** The form still takes its three lookup lists as
props and still must not read `useAdmin()`; what hands them over is `AdminSpielEditView`, the
aggregator view this page renders, rather than the admin card list that owned the modal's state. The
invariant is I12 in `docs/frontend/spec.md`, which names the current supplier — ADR-0004's own text
is left as written, because this folder does not edit an accepted ADR's reasoning.

**ADR-0016's membership decision is unaffected and its illustration is now historical.** The nine
stylesheets in `admin.css` are still reachable from no public route; what changed is that the graph now
reaches them statically from this route rather than through the `next/dynamic` import that ADR-0016 cites
...
**FB-9's frontend half found its home here.** ADR-0042's disabled picker entries and eligibility
...
by a decision since retired; [ADR-0041](0041-a-voided-result-is-named-before-it-is-lost.md) later reversed
exactly this — its dry run is that simulation, run read-only through the write path itself.

**A confirmation step before a save that would advance something.** Refused by ADR-0041 (as by the
