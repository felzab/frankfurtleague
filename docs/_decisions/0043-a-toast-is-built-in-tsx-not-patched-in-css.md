# ADR-0043 — A toast is built in TSX, and its duration is derived from what it says
...
[ADR-0041](0041-a-voided-result-is-named-before-it-is-lost.md) had already shipped the shape the rest
...
class this repository already fights in ADR-0013's per-component CSS imports.
...
**The raw error string in the dispatch-failure toast stays** (ADR-0041 raised it; this reviewed it).
...
**One thing this could not verify.** The check ran in a hidden browser pane
(`document.visibilityState === "hidden"`), where `document.timeline.currentTime` never advances, so no
CSS animation progresses. The timer bar's configuration was verified (`15s`, `linear`, `forwards`,
`origin: left`, `running`) and its dismissal fires, but its visual drain needs one glance in a real
browser.

[ADR-0041](0041-a-voided-result-is-named-before-it-is-lost.md) is the undo toast whose
structure this brings the other producers to; [ADR-0016](0016-admin-only-css-split.md) is why
`toast.css` stays in `globals.css` and `admin.css` must not gain a second copy;
[ADR-0013](0013-per-component-heroui-css.md) is the per-component import rule and the
