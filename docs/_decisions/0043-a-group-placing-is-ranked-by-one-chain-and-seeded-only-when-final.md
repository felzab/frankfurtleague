# ADR-0043 — A group placing is ranked by one chain, and seeded into the bracket only once it is final
...
[ADR-0042](0042-a-result-entry-resolves-the-whole-bracket.md) built the storage half of a bracket slot's
...
- **Nothing recorded how many teams advance.** `FLSaisonRules` carried `win_points` and `draw_points`,
  ...
  [ADR-0026](0026-team-statistics-are-derived-from-spiele.md) refused for 3/1/0.
...
read and stored nowhere (ADR-0026), so any second implementation of the counting rule is a second answer
to how many points a team has. And a `$jsonSchema` validator may assert types, presence and enums and
nothing else ([ADR-0027](0027-the-database-enforces-its-own-invariants.md)), so no rule here can be
...
ADR-0042 applies to a `spiel_nr` naming no match. A tie that survived the whole chain is a real state of
...
(ADR-0042's runbook). This changes nothing about the 2026 bracket.
...
--check` reports exactly which do not. It is independent of ADR-0042's own unrun `spiele` change, which
...
**The displayed table's order changes where it used to be alphabetical.** Two teams level on points and
goal difference were previously rendered in name order; they are now separated on goals scored and then
on their own match. This is the point — that order was an artefact of a stable sort — but it is a
visible change to a public page that no test of the old behaviour would have flagged.
...
edit — the same class of exposure ADR-0042 already accepted when the endpoint started reading the whole
season.

**`FLGruppen` can no longer be built from a list of teams.** The construction moved to
`fl_backend/app/api/teams/services.py :: build_gruppen`, because the last criterion reads matches a model holding
only teams cannot see. A model method that ordered on two of four criteria would be the ordering the
bracket must not disagree with, one import away from any caller.
...
**A `verlierer` reference now resolves to the losing side.** It previously advanced the winner for
either spelling. Nothing writes `verlierer` yet and the 2026 bracket has no third-place play-off
(`docs/glossary.md`, §`Ausgang`), so no stored document changes behaviour — but the branch was wrong and
this decision's rewrite of the resolution is where it was found.
...
to "how many points does this team have", and ADR-0026 exists to have exactly one. The Python side adds
...
Rejected: there is one team shape ([ADR-0034](0034-the-write-path-is-resource-first-in-a-second-router.md)),
