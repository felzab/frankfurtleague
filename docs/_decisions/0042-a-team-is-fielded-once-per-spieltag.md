# ADR-0042 — A team is fielded once per Spieltag, and an ineligible one is refused where it is fielded
...
eligibility tiers were first recorded the same day under a decision since retired; this decision carries
...
(ADR-0034), and every layer declined it in turn: the picker mapped every team of the season without
reading the `is_disqualified` each one carries; `find_wiring_refusal`'s four rules each begin by
skipping a side with no source (ADR-0038); the handler `$set` the side as submitted; the resolution
leaves a source-less slot alone by design; and `FLBracketFault` had no variant for an occupant,
since ADR-0039's five are contradictions between references. So a disqualified team that never
...
**That moment does not exist here.** ADR-0026 settled that a team never leaves a season: there is
...
applies can express it: a `$jsonSchema` validator sees exactly one document (ADR-0020), and a
...
(ADR-0039). Checked on 2026-08-06 against the live database before any of this shipped: 58 occupied
...
fielding such a team contradicts the season _as it stands at the write_ — ADR-0038's class of rule,
...
**Both occupant rules apply only to a team the payload NEWLY fields** — O2 for O1's reason, and the
fixture the clause protects is the one an admin most needs to open.
...
field the rename fan-out maintains (ADR-0021). Derived over the _effective_ sides of knockout
...
replacement team is the manual mode's intended use (ADR-0034). The form states that the system does
...
- **Both sides of the payload name one club** — there is nothing to move it to, since the only side
  ...
  see: they key a source by identity, and two hand-set sides carry no source at all (ADR-0038).

Every side emptied this way is named in `released_sides`, for the reason
[ADR-0041](0041-a-voided-result-is-named-before-it-is-lost.md) names a voided result: a write the
...
fired.** A failure body is `{error_code, correlation_id}` and nothing else (ADR-0032), so the code
...
the team leaves loses its `ergebnis` and `elfmeterschiessen`, because they were scored against a team
that fixture does not hold. The dry run of ADR-0041 names it before the save, and the undo toast
covers it after.
...
costs a second per-season junction read on the action-required route, which ADR-0039 already
...
everywhere else, and it contradicts ADR-0026 head-on: no delete exists on `saison_teams`, a team's
...
**Report all tiers as derived faults and refuse nothing.** Consistent with ADR-0039's "derive, do
