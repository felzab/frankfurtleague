# ADR-0023 — Pipelines are tested against a real `mongod`, behind a marker the default suite deselects
...
[ADR-0019](0019-team-statistics-are-derived-from-spiele.md): "whoever implements this should add
...
[ADR-0019](0019-team-statistics-are-derived-from-spiele.md) made the league table derived rather than
stored, and [ADR-0022](0022-the-league-table-counts-the-gruppenphase.md) gave it a `statistik_scope`,
...
- The forfeit rule, the scope rule, the season filter and the goal orientation are now executed
  rather than described. The corpus reproduces ADR-0022's live measurement exactly — Helmholtz at 3
...
- A stored `statistik` on the junction row is now proven to be ignored, which is the claim
  [ADR-0019](0019-team-statistics-are-derived-from-spiele.md) rests on and DB-3 is about to act on.
