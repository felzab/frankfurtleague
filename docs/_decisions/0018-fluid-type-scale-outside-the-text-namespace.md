# ADR-0018 — The fluid type scale lives outside Tailwind's `--text-*` namespace
...
`tailwind-variants` at module scope, and every HeroUI component accepting a `className` runs it
through one — `descriptionVariants({ className })`, `labelVariants({ className })`. Each recipe
captures its merge configuration when it is created. Two escape routes were tried against the real
package and both failed:
...
**A stale `text-fluid-*` styles nothing, silently.** No such utility exists, so a class copied out of
an old commit, an old audit report, or an LLM's memory applies no font size and inherits one instead.
This is the sharpest edge this ADR creates, mitigated only by there being zero occurrences left in
`src/` and by the token block in `globals.css` saying so in bold — it will not fail a build.
