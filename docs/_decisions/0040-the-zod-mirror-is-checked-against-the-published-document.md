# ADR-0040 — The Zod mirror is checked against a committed OpenAPI document, not generated from it
...
[ADR-0031](0031-the-third-copy-of-the-schema-is-checked-not-generated.md) settled the sibling question
...
runs from `create_app(build_test_config())` — no server, no database, no `.env`. The harness ADR-0031
...
frontend scope a Python prerequisite that `scripts/README.md` says it does not have.
...
scheme restriction on a value rendered into an `href`, and every German message is gone. ADR-0031
...
**Compare only field names, exactly as ADR-0031 does.** Rejected because it does not catch the change
