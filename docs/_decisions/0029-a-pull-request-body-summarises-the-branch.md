# ADR-0029 — A pull request body summarises the branch; the Commits tab indexes the commits
...
and why `docs/_git/templates.md` spends most of its length on them. That raises a fair
...
The forty-five merged pull requests were read in the browser on 2026-08-05. Four facts came out of
that reading and together they decide the question.
...
- **Link an individual commit only where that commit is the thing a reader must find.** The standing
  ...
  `Closed in` column of `docs/_roadmap/closed-items.md` points at, and the removal commit is what
...
- `docs/_git/templates.md` states the rule in the same words as this decision. That
  page already has to change with the claims it carries (CLAUDE.md, documentation); this adds one
  more claim to keep in step.
- The pull-request section of `docs/_git/spec.md` stops being marked unverifiable. It
