# ADR-0055 — A `?saison_id=` naming no season is stripped from the URL, before the page renders
...
**Why this does not contradict [ADR-0002](0002-omitted-season-means-current.md).** It reads as a
reversal, so the boundary is worth stating exactly.
