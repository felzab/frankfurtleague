# ADR-0019 — Team statistics are derived from `spiele`, never stored
...
and had not since commit `50aa75b` introduced the junction. The junction's figures were correct only
...
- **A recomputation from the `spiele` documents reproduced every stored figure exactly** — all seven
  fields, all seventeen teams, season 2026. Stored numbers and derivation are identical on the real
  data, so switching between them can be proved to change nothing before it is done.
- **The database is tiny and will stay tiny.** A whole season is 31 matches, 17 teams and 362
  players; all nine collections together hold about 130 KB, less than their own `_id` indexes. At a
  season a year, read cost never becomes the binding constraint. Correctness is the only scarce
  resource.
...
- The arithmetic moves from a pure Python function into aggregation stages, which cannot be
  unit-tested without a database — the suite loses the _ability_ to test cheaply what it was not
  testing anyway. **The integration coverage this asked for was built on 2026-08-02**
  ([ADR-0023](0023-a-real-mongod-behind-a-deselected-marker.md)): a real `mongod` behind a marker the
  fast suite deselects, which also demonstrates that a stale `statistik` on the junction row is
  ignored rather than merely unread.
- `GET /teams` fails if a `spiel` document is malformed, where the two were independent before.
...
is the obvious second candidate. Roadmap item DB-2 carried it and is closed;
[ADR-0052](0052-a-seasons-schedule-is-derived-from-its-rules.md) settles it the same way.
