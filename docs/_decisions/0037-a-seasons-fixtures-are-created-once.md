# ADR-0037 — A season's fixtures are created once, so `/spiele` has no POST and no DELETE
...
The admin router therefore carries exactly two operations: `PATCH /spiele/{spiel_id}` and the read
`GET /spiele/action_required`. `/spiele` is the only resource in the API without a create and without a
delete, and **nothing said why.**
...
the rule with its reasoning and cites [ADR-0026](0026-one-active-season-and-one-path-to-it.md).
...
table if a result was awarded ([ADR-0019](0019-team-statistics-are-derived-from-spiele.md)).
...
drawn before the documents exist ([ADR-0034](0034-a-result-entry-resolves-the-whole-bracket.md)).
...
and leaves the slot alone (ADR-0034), which is the correct answer to a mistyped number and the wrong one
...
that field because their rows can be retired ([ADR-0025](0025-soft-deletion-is-a-date-not-a-flag.md)).
...
[ADR-0027](0027-the-write-path-is-resource-first-in-a-second-router.md) settled — every resource
...
a position to disagree, which is the shape ADR-0025 refused for a boolean beside a date.
