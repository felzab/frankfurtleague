...
**Verified against:** `bbd05129`, 2026-08-26\
...
docs scope. This table is the one place its checks are listed; the gate's own `--help` names this
page.

| Check               | A failure means                                                                                                                                                                                                                           | Verdict |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `link`              | A relative markdown link points at nothing                                                                                                                                                                                                | Fail    |
| `anchor`            | A `#anchor` link matches no heading in the page it opens — its own, or the page it links to                                                                                                                                               | Fail    |
...
| `copy-dash`         | A dash is punctuation in a rendered string, or stands as a rendered value of its own, outside §1.12's date-range exception                                                                                                                | Fail    |
| `copy-term`         | A rendered string spells a concept with a word §1.12 retired — a club is a `Team`, never a `Mannschaft`                                                                                                                                   | Fail    |
| `copy-formal`       | A rendered German string uses `Sie` or `Ihr` where no sentence opens, which only the formal address does (§1.12)                                                                                                                          | Fail    |
| `copy-informal`     | A rendered string lower-cases a `Du`, `Dein`, `Dir` or `Dich`, which §1.12 capitalises everywhere                                                                                                                                         | Fail    |
| `copy-corpus`       | The copy sweep read nothing, could not read a file to its end, or holds a date-formatter exemption that matches nothing — each leaves the copy checks above passing over an empty corpus                                                  | Fail    |
| `stamp`             | A stamped SHA is no ancestor of `HEAD`, or a page this branch changed still carries the `Verified against` line it had at the fork point                                                                                                  | Fail    |
| `stamp-format`      | A stamp line deviates from CUR-3's exact shape, or from its line                                                                                                                                                                          | Fail    |
| `branch-impact`     | The branch materially changed a file a stamped page cites, and the page kept the fork point's stamp line (CUR-4)                                                                                                                          | Fail    |
...
| `unreadable`        | A tracked file cannot be read as UTF-8 text                                                                                                                                                                                               | Fail    |
...
| `sha`               | A commit named in prose or in a comment resolves to nothing in this clone — the hits must be read                                                                                                                                         | Report  |
