...
| `copy-dash`         | A dash is punctuation inside a rendered German string, outside §1.12's date-range exception                                                                                                                                               | Fail    |
| `copy-term`         | A rendered string spells a concept with a word §1.12 retired — a club is a `Team`, never a `Mannschaft`                                                                                                                                   | Fail    |
| `copy-formal`       | A rendered string uses `Sie` or `Ihr` where no sentence opens, which only the formal address does (§1.12)                                                                                                                                 | Fail    |
| `copy-informal`     | A rendered string lower-cases a `Du`, `Dein`, `Dir` or `Dich`, which §1.12 capitalises everywhere                                                                                                                                         | Fail    |
| `copy-corpus`       | The copy sweep read nothing, could not read a file to its end, or holds a date-formatter exemption that matches nothing — each leaves the four checks above passing over an empty corpus                                                  | Fail    |
