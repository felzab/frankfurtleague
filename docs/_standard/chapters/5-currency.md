...
| `copy-corpus`       | The copy sweep read nothing, could not read a file to its end, or holds a date-formatter exemption that matches nothing — each leaves the three checks above passing over an empty corpus                                                 | Fail    |
