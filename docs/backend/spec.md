...
| --- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| I1  | Team statistics are **derived from `spiele`**, never stored ([ADR-0026](../_decisions/0026-team-statistics-are-derived-from-spiele.md)) | Not yet — see F4                                                       | **Currently violated on both sides**: they are stored on `saison_teams` and `$inc`-ed onto `teams`, so a result edit moves neither. F4 implements the derivation |
...
| ------------------------------------------------ | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| League table does not change after a result edit | Statistics written to `teams`, read from `saison_teams` | Expected today — F4, confirmed and unfixed. The fix is ADR-0026's derivation, not a redirected write |
...
| #    | Item                                                            | State                                                                                                                                                                                                                                                                                                |
| ---- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F4   | Team statistics write/read mismatch                             | **Confirmed against the live database 2026-08-02; fix open, and its shape is now decided.** [ADR-0026](../_decisions/0026-team-statistics-are-derived-from-spiele.md): derive from `spiele`, delete the stored field and the `$inc` path. [`../roadmap/open-items.md`](../roadmap/open-items.md) §F4 |
| —    | Season `rules.win_points` / `draw_points` are stored but unused | `get_stats_contribution` hardcodes 3 and 1. They agree today; they are not wired together. ADR-0026 wires them when the derivation lands                                                                                                                                                             |
| —    | No collection has a validator, and no index exists beyond `_id` | Measured 2026-08-02. [ADR-0027](../_decisions/0027-the-database-enforces-its-own-invariants.md) decides both; the work is DB-2 in [`../roadmap/open-items.md`](../roadmap/open-items.md). Two `saison_spieler` rows are already malformed as a result                                                |
