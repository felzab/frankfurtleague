...
| A `git bisect` over `fl_frontend/` fails at a commit for a reason unrelated to what is being hunted | One commit's test file imports `fl_frontend/src/core/refusalRegister.ts` before the commit adding it, which `git log --diff-filter=A` on that path names | Return 125 from the bisect script there; its only frontend change outside comments is that test file, so the skip hides nothing |
