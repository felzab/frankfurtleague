...
**Verified against:** `8af00d5c`, 2026-08-27\
**Scope:** `fl_frontend/`

A Next.js application on the App Router, with React, HeroUI and Tailwind. It is both the website and, in
effect, the system's application server: **the browser never talks to FastAPI directly.** Every application
read is a server-side fetch made from this container, which is why the entire caching design lives here
