...
| I76  | **The one exception is structural: a confirmation or a spinner may be identified by the press that raised it**, `appToast.pending` taking no options                                                                                       | `fl_frontend/src/core/toastTitles.test.ts`, whose register admits a shared failure claiming the press and an assertion refuses it, no type doing so                                                                                                                                                                                                                                                                                                                                     |
...
| I250 | **A failure toast names the thing rather than the act**: its success title negated — `Adresse nicht kopiert` — bar an unknown outcome (I325)                                                                                               | `fl_frontend/src/core/toastTitles.test.ts :: NAMES_THE_OPERATION`, which refuses a nominalised, impossible or passive ending in the register; review for every other wording                                                                                                                                                                                                                                                                                                            |
