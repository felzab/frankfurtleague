...
the undraw beside it and the rollover's close of the outgoing season
...
that is `Es gibt in der Verwaltung keinen Weg zurück.`, and it covers the replace, the undraw, the
junction replacement and the rollover. Where the same transaction empties those log rows as it empties
the values (`fl_backend/app/core/recording.py :: build_redaction_update`), nothing holds them
afterwards: that is `Zurückholen lässt sich das nicht.`, and it covers a pupil's erasure and a
referee's anonymisation alone. **Each is a whole sentence of its own**, because folding the qualifier
into the object beside it is what let one promise grow into a family of wordings nobody could sort.
