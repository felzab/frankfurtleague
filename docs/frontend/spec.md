...
| I261 | **A passkey is enrolled only from a sign-in under two hours old**: a person's by either factor, an administrator's by passkey, bar their first                                                                                                     | `fl_frontend/src/core/auth.ts :: refuseEnrolment` over `:: isFreshlySignedIn`, raised in `hooks.before` and in `registration.afterVerification`; `fl_frontend/src/core/auth.test.ts :: "which sessions may enrol a passkey, and how many rows they may leave"`                                                                                                                                                                                                                      |
