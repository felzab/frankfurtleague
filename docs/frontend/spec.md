...
- **A link inside text is `textLink`** (I43, I78, I79). A standalone action is a `ctaButton` link
  ...
  (`fl_frontend/src/shared/components/ui/textLink.ts :: BRAND_INK_OUTSIDE_PROSE`); a whole box is pressable only as
...
| I238 | **A control outside prose takes its brand grade from one recipe, without the underline that names a link**: a re-spelled grade loses its dark arm                                                                                          | `fl_frontend/src/shared/components/ui/textLink.ts :: BRAND_INK_OUTSIDE_PROSE`; `fl_frontend/src/shared/components/ui/textLink.test.ts` sweeps every `.ts` and `.tsx` under `src` for a hand-spelled `hover:text-brand-solid`, sparing that module by name                                                                                                                                                                                                                               |
