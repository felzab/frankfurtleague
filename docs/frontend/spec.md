...
error page ([`docs/logging/error-codes.md`](../logging/error-codes.md)). **Each call declares whether
its action only reads**, required rather than defaulted: a throw of the action's own code after a
write may leave the row standing and answers as of unknown outcome (I326), while a read's answers as
the failure it is; and only a write's success refreshes the page (I233). A route handler's write
...
showing their old values. **Only the shared form's type sees it**: `action` is a valid prop on
HeroUI's form and on a plain `<form>`, the build is clean, and a save that succeeds navigates away
before the emptied fields are on screen, which is why the class stayed invisible until a dialog held
the page open.
...
| I32  | **A form whose fields are React state submits through the shared `Form`, never through a function `action`.**                                                                                                                              | `fl_frontend/src/shared/components/ui/Form.tsx :: Form`, whose props carry no `action`, typechecked by `fl_frontend/src/shared/components/ui/Form.test.ts`                                                                                                                                                                                                                                                                                                                          |
...
| I35  | **Every `path` a field label is given is a path its editor's descriptor table carries**                                                                                                                                                    | `fl_frontend/src/shared/components/ui/DraftStatusContext.tsx :: FieldPath`, which types a label's `path` by that table's literals, under tsc                                                                                                                                                                                                                                                                                                                                        |
...
| I37  | **A press confirmed in place escalates through the shared two-press control, and a panel spelling its own armed state is the violation**                                                                                                   | `fl_frontend/src/shared/hooks/useTwoPressConfirm.ts`; review for a panel spelling its own armed state                                                                                                                                                                                                                                                                                                                                                                               |
...
| I64  | **`useFieldStatus` answers `undefined` for a declared path whose descriptor does not apply to the draft**; a path no descriptor declares fails the typecheck (I35)                                                                         | `fl_frontend/src/shared/components/ui/DraftStatusContext.tsx :: useFieldStatus`                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| I75  | **A second `I18nProvider` anywhere below is the same defect narrowed to a subtree**, react-aria reading the nearest one                                                                                                                    | `fl_frontend/src/app/documentLocale.test.ts`, which finds the mount by sweeping the tree rather than by name and fails on a second one                                                                                                                                                                                                                                                                                                                                              |
...
| I186 | **A message's links stand on `AUTH_URL`'s origin, never `fl_frontend/src/core/brand.ts :: SITE_URL`**: one variable behind both puts an environment-read origin before a crawler                                                           | `fl_frontend/src/core/emailShell.ts :: mailOrigin`; `fl_frontend/src/core/emailShell.test.ts` sweeps every builder's close, each minter's own test its link's: `fl_frontend/src/app/api/bewerbung/route.test.ts`, `fl_frontend/src/features/bewerbungen/sweep.test.ts` and `fl_frontend/src/features/bewerbungen/actions.test.ts`; `fl_frontend/eslint.config.mjs :: SITE_ORIGIN` refuses the published one                                                                         |
...
| I233 | **Every admin write's success refreshes the page**, whatever tags it also moves: `runAdminMutation` calls `refresh()`, never the action                                                                                                    | `fl_frontend/src/shared/utils/adminMutation.test.ts :: "the refresh an admin write owes the page"`, and I7's sweep for every action running inside it                                                                                                                                                                                                                                                                                                                               |
...
| ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| An admin write saves and toasts, and the list keeps the old rows until a manual reload                                             | The action runs outside `runAdminMutation`, or declares a write `readOnly`, so its response carries no re-rendered payload                                                              | I233 — run it inside the spine and declare a write a write; a tag's own re-render is Next's bookkeeping rather than the action's guarantee                               |
