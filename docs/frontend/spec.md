...
| `react/forbid-component-props` and `react/jsx-props-no-spreading` on the segmented date controls    | A bound goes on the Calendar offering the days, never on the control judging each keystroke; a spread would carry one there unseen                                                 |
| `no-restricted-imports` on `core`/`shared`                                                          | Layer boundaries, scoped deliberately narrowly — see the overview                                                                                                                  |
| `no-restricted-imports` of Next's private contexts, the segmented date controls and HeroUI's `Form` | Each has one home: `fl_frontend/src/shared/testing/nextContexts.ts`, `fl_frontend/src/shared/components/ui/DateTimeFields.tsx` and `fl_frontend/src/shared/components/ui/Form.tsx` |
