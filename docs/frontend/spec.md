...
| `spieltage`      |   ✅    |    ✅     |   ✅    |   ✅    | Create and edit — no delete; `constants.ts`, `facets.ts`, `resolvers.ts`, `spieltagDraftStatus.ts`, `types.ts`, `utils.ts`, tests                     |
...
(I25). **Every spieltage action clears `spieltage` alone**: `GET /spiele` never joins `spieltage`, so a
matchday write changes no fixture a match read serves.
...
the one endpoint that may; and a matchday is created and edited but never retired, `spieltage`
carrying no `inactive_since` to stamp.
...
`fl_frontend/src/features/spiele/draftStatus.ts :: isLevelKnockout`
...
sentence.
`fl_frontend/src/features/spieltage/components/forms/AdminSpieltagEditForm/banners.ts :: describeMitwandernd`
is the last of the three. Two parallel maps for one noun set drift the moment a value is added.
...
| I25 | **A season write invalidates every read its answer reaches, and the two writes reach different sets.** An edit clears `saisons` + `teams`; the rollover clears `saisons` + `spiele` + `spieltage` + `teams`                                                                                                                                                                                                                                                                      | `fl_frontend/src/features/saisons/actions.ts :: invalidateSaisonAndTable` and `:: invalidateRollover`                                                                                                                                                                                                                                             |
...
| I27 | **A matchday's order is the API's and no frontend surface re-sorts it, and its ordinal is the SERVED `position` rather than a place in the arrival order.** So a filtered or reordered list labels each matchday the same way, and the editor moves one by writing `position` on the ordinary PATCH                                                                                                                                                                              | `fl_frontend/src/features/spieltage/utils.ts :: spieltagLabels` takes the ordinal from `position`, `:: buildSpieltagPositionOffer` offers the phase's free slots with the taken ones disabled, and `:: orderRoundsByWiring` consumes the arrival order                                                                                            |
...
| A matchday sits in the wrong place in the list                                                                                                                                   | Its stored `position` is wrong, or it sits in the wrong `saison_phase`                                                                                                         | Pick another slot in the matchday editor; a slot another matchday of that phase holds is offered disabled                                                                                                                                                                                                                                                                  |
