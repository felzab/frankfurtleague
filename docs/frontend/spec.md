...
([`docs/logging/error-codes.md`](../logging/error-codes.md)). **Each call declares whether its
action only reads**, required rather than defaulted: a throw of the action's own code after a write
may leave the row standing and answers as of unknown outcome (I326), while a read's answers as the
failure it is. `fl_frontend/src/core/adminWriteRefresh.test.ts` holds each declaration to the roster
of writes and reads.
...
| I8   | `getAdminSession()`'s return value must be checked                                                                                                                                                                                         | `fl_frontend/src/core/adminSessionGuard.test.ts`, whose reader takes the call only inside a negated `if`; `fl_frontend/src/features/admin/components/providers/AdminAuthGuard.tsx` and `fl_frontend/src/shared/utils/undoRoute.ts` sit outside that population                                                                                                                                                                                                                          |
...
| I13  | Before deleting a `"use client"` directive, check for render props                                                                                                                                                                         | `fl_frontend/eslint.config.mjs :: SCOPED_BANS` and `:: SOURCE_BANS` for the facets shape, over the views `fl_frontend/src/shared/utils/facets.test.ts :: VIEWS_GLOB` finds; review for every other render prop (§4)                                                                                                                                                                                                                                                                     |
...
| I233 | **Every admin write action calls `refresh()` on its success path**, whatever tags it also moves                                                                                                                                            | `fl_frontend/src/core/adminWriteRefresh.test.ts :: "every slice's admin writes, and the refresh each one owes"`, its roster held to every admin action module and export                                                                                                                                                                                                                                                                                                                |
...
| The render-prop rule I13 states is checked for the facets shape alone, and reviewed elsewhere                                                        | Accepted — `fl_frontend/eslint.config.mjs :: SCOPED_BANS` covers `fl_frontend/src/app/`, `:: SOURCE_BANS` the admin views `fl_frontend/src/shared/utils/facets.test.ts :: VIEWS_GLOB` finds; a server-render harness is refused |
