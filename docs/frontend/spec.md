...
still see it. The uncached reads that resolve `null` the same way — `getAdminSpiel`,
`getAdminSpieltagById` and `getBewerbungById` — are under no such constraint, and catch where they do
only because that is where the id is known.
...
**The queue MARKS a colliding application; the write refuses none.** Two applications of one season
naming the same club, or proposing the same Kürzel, are both flagged for the administrator to decide
between (`fl_frontend/src/features/bewerbungen/duplicates.ts :: findBewerbungDubletten`), which is
where the argument against enforcing that uniqueness at an unauthenticated write is recorded.
...
a reader are all in — the sign-in messages in `fl_frontend/src/core/authEmail.ts` and the triage's
decisions in `fl_frontend/src/core/bewerbungEmail.ts`. German inside `/docs` and in code comments addresses
