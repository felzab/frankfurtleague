...
| `passkeys`        |    —    |     —     |   ✅    |    —    | A list and a removal of this administrator's own passkeys — never a rename, and the enrolment is the browser's ceremony                                                                            |
| `funktionen`      |    —    |     —     |    —    |    —    | The person and team areas' guard, shells and landings, the team's forbidden panel, and every signed-in shell's Funktion switcher; what a Funktion is, `fl_frontend/src/core/funktionen.ts` answers |
| `dashboard`       |    —    |     —     |    —    |    —    | —                                                                                                                                                                                                  |
| `meta`            |    —    |     —     |    —    |    —    | —                                                                                                                                                                                                  |
...
holds every editor to it. **A redraw's moved shape is the season editor's unsaved typing, though its
save never sends it**: the leave prompt and the discard dialog ask over it, a save standing beside
one ends on that dialog rather than leaving, and
`fl_frontend/src/features/saisons/components/views/AdminSaisonEditView.tsx` holds it above the key
the save re-keys the editor by, so only the stored shape moving drops it. The draw alone refuses
over the draft without it, being made from it;
`fl_frontend/src/features/admin/editorWiring.test.ts :: "a season page holding a redraw's moved shape"`
holds the page to it. A press writing no typed entry reads the page again, a reload after its
...
season's rollover. On an application they are both
...
and
`fl_frontend/src/features/bewerbungen/components/views/AdminBewerbungView.test.ts :: "a press on the application page beside another panel's typing"`
hold each press. A route handler's write
...
**Every request runs under one deadline** (I366),
`fl_frontend/src/core/requestScope.ts :: REQUEST_DEADLINE_MS`, and in one scope: an action's or a
...
(`docs/backend/spec.md :: I321`). **A ban's notice is the one exception (I372)**: the ban is written
before its notice, so a deadline cutting the notice leaves the ban certain and only the notice in
doubt, and the create answers the ban saved with its notice unclear. Both mail fan-outs count a send that broke off unanswered as neither
...
(`fl_frontend/src/core/api.ts :: BASE_FETCH_TIMEOUT_MS`,
`fl_frontend/src/core/mail.ts :: MAIL_TIMEOUT_MS`), that under the request's deadline, and the
deadline under `nginx/shared/site.conf :: proxy_read_timeout`.
`fl_frontend/src/core/requestScope.test.ts` holds the last three in that order and
`fl_backend/tests/core/test_request_deadline.py` the first pair. The backend keeps its own deadline
rather than taking what is left of this one.

**A payload the API refuses is marked on the field it names** (I343):
...
link opened that link again, and never a retry.
The answer carries that sentence beside the map as `unplacedError`: an admin action's from
`fl_frontend/src/shared/utils/actionError.ts :: toActionErrorResult`, a public slice's own from
`:: refusedPayloadAnswer`, and a public route's own parse from `:: refusedDraftAnswer`.

- **One owner raises the press's toast on every form whose write reaches the API**:
  `fl_frontend/src/shared/hooks/useServerFieldErrors.ts :: useServerFieldErrors`, handed the whole
  failure, raises the answer's sentence where any path of the map reached no control and nothing
  where controls took all of it, unless the failure says more than the marks, as a two-part press
  does where one half saved or failed with no map. A public page, and a control on the application's confirmation strip, raises
  its own toast only for an answer carrying no map: an unread or rejected one, or one of unknown
  outcome. The sign-in form answers from Better Auth rather than the API, and keeps its own toast.
- **A toast marks nothing**, so `fl_frontend/src/shared/utils/appToast.ts :: appToast` speaks a
...
`auth.api.signInMagicLink` — the verification-token write, the send gate (the allowlist, then one
read of what the address holds and whether a ban bars it; I390, I391) and the send — is
...
(`fl_frontend/src/core/auth.ts :: isAdminSession`), not the allowlist alone: a control gated on it
admits exactly the sessions `getAdminSession()` admits, so a code-borne session of an allowlisted
address and one past the administrator's window are both `false`. It answers `null` where no
readable session stands or the lookup marks its subject `gesperrt` (I406), and throws on every backend failure, which is the fail-closed answer for a
panel — `null` would offer a sign-in to somebody already signed in, and an empty subject would tell
a seat holder the league holds nothing of theirs.
...
(`fl_frontend/src/app/api/bestaetigung/kontakt/route.ts`) authorize nobody, so none can borrow
`runAdminMutation`, whose guard turns away every caller without an admin session (I7);
...
`fl_frontend/src/shared/utils/undoRoute.ts :: handleUndoRequest` is where
`fl_frontend/src/shared/utils/adminMutation.ts :: runAdminRouteWrite` and its guard are reached — each of the eight `route.ts` files supplies a name, a schema and
...
| `readPasskeysAction`              | passkeys       | **nothing** — it writes nothing                                                                |
...
| `ALLOWED_ADMIN_EMAILS`                         | comma-separated; each entry folded through `fl_frontend/src/core/emailAddress.ts :: asSignInIdentifier`, deliverable, inside `:: KONTAKT_EMAIL_MAX_LENGTH`, and one the sign-in library itself accepts |
...
The `AUTH_URL` https rule exists because `fl_frontend/src/core/auth.ts` reads the scheme of the value it
hands the library and gives the session cookie the `__Host-` name prefix and the `secure` attribute
together or neither (I401), so a stray `http://` value would ship a session cookie in plaintext; it is gated on hostname rather than `NODE_ENV`, because the local stack runs
...
| `better-tailwindcss/no-unknown-classes: error`                                                                                                                               | Sees a class resolving to nothing, which TypeScript, Prettier and the browser accept silently; it reads a constant only by its `*_CLASSES` name                                                                                                  |
...
| `better-tailwindcss/enforce-consistent-class-order: error`, over the `*_CLASSES` constants alone                                                                             | The Prettier plugin sorts a `className` and a `tv()` call and never a constant; each list answers to one tool, so the two never fight                                                                                                            |
| `no-restricted-syntax` over `fl_frontend/eslint.config.mjs :: SOURCE_BANS`                                                                                                   | Each refuses a spelling node by node, so a comment naming it never trips one                                                                                                                                                                     |
| `local/admin-link`, defined in `fl_frontend/eslint.config.mjs :: LOCAL_RULES`                                                                                                | An admin link carries `?saison_id=`; an excused site names this rule alone, so every other ban still reaches its line                                                                                                                            |
| `no-restricted-syntax`: a constant reaching a class list unsuffixed                                                                                                          | A class-list constant not named `*_CLASSES` is one `no-unknown-classes` never reads                                                                                                                                                              |
| `no-restricted-properties`: `back`                                                                                                                                           | A bare history back is a silent no-op on a cold entry (I225); the property form refuses a destructured `back` too                                                                                                                                |
| `react/forbid-component-props` on the judging date controls, `react/jsx-props-no-spreading` on all                                                                           | A bound goes on the Calendar offering the days, never on the control judging each keystroke; a spread hides one from the ban                                                                                                                     |
| `no-restricted-imports` on `core`/`shared`, and their production `import()`s                                                                                                 | Layer boundaries over the `@/` alias and relative paths, scoped deliberately narrowly — see the overview                                                                                                                                         |
| `no-restricted-imports` of `fl_frontend/eslint.config.mjs :: TEST_ONLY` outside the tests and `:: TEST_SUPPORT`, and its production `import()`s                              | A module only the suite reads never ships; the harness and those modules may import one another, as a test may                                                                                                                                   |
| `no-restricted-imports` of `node:module` outside the tests and the harness, in `fl_frontend/eslint.config.mjs :: SUITE_IMPORTS`                                              | A `createRequire` held in a name loads past every load ban, and production code has no use for the loader's hooks                                                                                                                                |
...
`fl_frontend/eslint.config.mjs :: VENDOR_ROOTS` refuses either root, `useOverlayState` excepted,
which HeroUI publishes at the root alone.
...
the first thing it finds. `fl_frontend/src/core/schemaGerman.test.ts` runs its reader over such a
sample and keeps its floor over the tree as well: the two answer different questions.

**A spelling refused wherever it is written is held by a lint ban (§1.8) where a selector states it**; a sweep holds what
...
on a ban the config states that no plant marks. **Every block restating a ban is one more place to
drop it**, so the same file fails on a block no plant is linted in, a plant being linted in a block
whose whole entry, severity included, eslint resolves for its path. An import or property block
needs each of its bans marked there; a syntax block needs one, and each of its bans marked by a
plant of its own population, production or test.
...
`fl_frontend/src/app/bereich/admin/spiele/[spiel_id]/page.tsx` keys `AdminSpielEditView` with
...
`fl_frontend/src/shared/components/ui/Form.tsx :: Form`, which runs its `onSubmit` handler after
`preventDefault` and whose props carry no `action` (I32).
...
showing their old values. **Only the shared form's type and the lint ban on a native `<form>` see
it**: `action` is a valid prop on HeroUI's form and on a plain `<form>`, the build is clean, and a
save that succeeds navigates away before the emptied fields are on screen, which is why the class
stayed invisible until a dialog held the page open.
...
`fl_frontend/src/app/bereich/admin/admin.css` loads only under `/bereich/admin`. A third stylesheet in that tree takes
...
comment at `fl_frontend/src/app/bereich/admin/admin.css :: @reference` records that nothing else does.
...
(I57). Where a stylesheet is genuinely the only route, **a test holds the rule against the installed
`@heroui/styles` sheet**, never a version written beside it, which nothing compares with what is
installed. `fl_frontend/src/app/toastStack.test.ts` is the standing example: it compiles
`fl_frontend/src/app/globals.css` with HeroUI's own sheets and fails when the toast block stops
overriding what `toast.css` declares.
...
FORM_SECTION_HEADING_CLASSES`, an eyebrow, a table header, a footer column title; the display voice is Anton
at the display step.
...
| `control`                                                   | A field's border at rest, from `fl_frontend/src/shared/components/ui/formFieldStyles.ts :: FIELD_INPUT_CLASSES` and `:: FIELD_GROUP_CLASSES` | A box, a divider, or a button whose own text identifies it                                         |
...
`fl_frontend/src/shared/components/ui/hintSurface.ts :: HINT_SURFACE_CLASSES`), `shadow-2xl` a modal or the
...
- `rounded-md` — a pill (`fl_frontend/src/shared/components/ui/badges.ts :: PILL_RADIUS_CLASSES`)
...
- **A picker always carries the value the record holds** — at its place in the order and closed with
  ...
  swept by
  `fl_frontend/src/features/spiele/components/schiedsrichterAnzeige.test.ts :: "names the held referee by its own id even though the list offers nobody"`
  and
...
- **Every row in a picker's list is composed through
  ...
  compact rows an `Autocomplete` or a `ComboBox` popover holds are a second family and take none of it,
  and a filter surface marks the current row in its list from
  `fl_frontend/src/shared/components/ui/pickedOption.ts :: PICKED_OPTION_CLASSES` where a picker's trigger
...
- **A link inside text is `textLink`** (I43, I78, I79). A standalone action is a `ctaButton` link
  ...
  it sits on), never a text link; a control that is neither — a toggle under a clamped paragraph, a
  row's own way in — wears the brand grade without the underline
  (`fl_frontend/src/shared/components/ui/textLink.ts :: BRAND_INK_OUTSIDE_PROSE_CLASSES`); a whole box is pressable only as
  `card({ interactive: true })` (`fl_frontend/src/shared/components/ui/card.ts :: card`).
- **One pill composed through `fl_frontend/src/shared/components/ui/badges.ts :: labelBadge`, and
  ...
  takes `PILL_RADIUS_CLASSES` and the app's classes, never `variant` or `color`
...
- **A decorative glyph never stands alone** (I169). It is `aria-hidden` beside the words carrying the
  ...
  (`fl_frontend/src/shared/components/ui/RowActions.tsx :: ACTION_LINK_CLASSES`), the `bg-brand` dot
...
- **An admin list row built on `fl_frontend/src/shared/components/ui/adminTable.ts` leads with one
  ...
  (`fl_frontend/src/shared/components/ui/adminTable.ts :: IDENTITY_ROW_CLASSES`), the same block the phone
...
- **A collection whose row is a line is a table; one whose row is a RECORD is a card list at every
  ...
  sectioned shape the other two follow. Three seats
...
- **Every field-shaped control resolves to one height**
  (`fl_frontend/src/shared/components/ui/formFieldStyles.ts :: FIELD_HEIGHT_CLASSES`). Refusal is the danger
...
| `4`   | Fields in a grid (`fl_frontend/src/shared/components/ui/formFieldStyles.ts :: FIELD_PAIR_CLASSES`); a card grid below `sm` |
...
**The gap is the whole of the distance between two siblings** (I240): a margin on the same axis
at the call site composes with it, so the class list declares one rung while the reader sees
another — the failure this ladder's census is blind to by construction. A margin against the
container's own inset stands, being between a child and the box rather than between two siblings:
the first child's leading side, the last child's trailing side, and a bleed widening a child past
that inset. **A negative margin cancelling the child's own padding stands too, for a different reason:** the
margin box does not move, so no distance the ladder measures changes and only the painted box grows
...
| I7   | Every admin server action runs inside `runAdminMutation`, whose guard turns a caller with no admin session away before its body runs; §1.3's `auth` pair excepted                                                                                  | `fl_frontend/src/shared/utils/adminActionSpine.test.ts`, which calls every export of every slice's actions module signed out                                                                                                                                                                                                                                                                                                                                                        |
| I8   | `getAdminSession()`'s return value must be checked                                                                                                                                                                                                 | `fl_frontend/src/shared/utils/adminMutation.test.ts :: "the session guard every admin write runs behind"`; `fl_frontend/src/features/admin/components/providers/AdminAuthGuard.tsx` sits outside it                                                                                                                                                                                                                                                                                 |
| I9   | `core` imports neither `shared` nor `features`; `shared` does not import `features`                                                                                                                                                                | `fl_frontend/eslint.config.mjs :: LAYER_BOUNDARY` through `no-restricted-imports`, and `fl_frontend/eslint.config.mjs :: SCOPED_BANS` for a production `import()`                                                                                                                                                                                                                                                                                                                   |
...
| I13  | Before deleting a `"use client"` directive, check for render props                                                                                                                                                                                 | `fl_frontend/eslint.config.mjs :: SCOPED_BANS` for the facets shape, in `fl_frontend/src/app/` and in every admin view's own file; review for every other render prop (§4)                                                                                                                                                                                                                                                                                                          |
| I14  | `revalidateTag(tag, profile)` in a route handler and `updateTag(tag)` in a server action, decided by the call site alone: `updateTag` throws in a route handler                                                                                    | route/action split                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| I23  | **A toast is raised through `appToast`, never HeroUI's `toast` at a call site**                                                                                                                                                                    | `fl_frontend/src/shared/utils/appToast.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| I32  | **A form whose fields are React state submits through the shared `Form`, never through a function `action`.**                                                                                                                                      | `fl_frontend/src/shared/components/ui/Form.tsx :: Form`, whose props carry no `action`, typechecked by `fl_frontend/src/shared/components/ui/Form.test.ts`; a native `<form>` is linted out                                                                                                                                                                                                                                                                                         |
...
| I34  | **A refusal lands on the control its path names, or where none does, in the press's toast in that path's own message**                                                                                                                             | `fl_frontend/src/shared/hooks/useDraftFieldErrors.test.ts :: "a blocked press's announcement"` and `:: "a failed write's one announcement"`                                                                                                                                                                                                                                                                                                                                         |
| I35  | **Every `path` a field label or an editor's own lookup is given is a path its editor's descriptor table carries**                                                                                                                                  | `fl_frontend/src/shared/components/ui/DraftStatusContext.tsx :: FieldPath`, which types a label's `path` by that table's literals, under tsc; `fl_frontend/src/shared/utils/draftStatus.ts :: fieldStatus` throws at run time for one it lacks                                                                                                                                                                                                                                      |
...
| I37  | **Every two-press confirmation escalates through the shared two-press control**, the retirement dialog's and the sign-out's too; one spelling its own armed state is the violation                                                                 | `fl_frontend/src/shared/hooks/useTwoPressConfirm.ts`; review for a control spelling its own armed state                                                                                                                                                                                                                                                                                                                                                                             |
| I38  | **An entity editor's page chrome is `EditPageHeader`**, whose title, chip, Reaktivieren control and save notice are data slots rather than nodes                                                                                                   | `fl_frontend/src/shared/components/ui/EditPageHeader.tsx` and `fl_frontend/src/shared/components/ui/EditFormLayout.tsx :: EditFormLayout`, whose `header` prop admits no node                                                                                                                                                                                                                                                                                                       |
...
| I41  | **The locale is pinned once, at the root, to the language `<html lang>` declares**: an unpinned `DatePicker` reorders its segments at hydration, changing a birthdate                                                                              | `fl_frontend/src/core/providers/RootProviders.tsx`, which the root layout renders around every route; `fl_frontend/src/app/documentLocale.test.ts` holds a date field under that layout to its `<html lang>`                                                                                                                                                                                                                                                                        |
| I42  | **Where several sites share a toast title, the description is what tells them apart**, and one title is raised at one variant                                                                                                                      | review, against §1.12                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| I43  | **A link inside text takes its underline and colour from one recipe, whose base carries the underline**: colour alone is not a link (WCAG 1.4.1)                                                                                                   | `fl_frontend/src/shared/components/ui/textLink.ts :: textLink`, whose `base` carries the underline; `fl_frontend/eslint.config.mjs :: SOURCE_BANS` refuses a hand-spelled link decoration outside it, planted in `fl_frontend/eslint-plants/plants.tsx.txt`                                                                                                                                                                                                                         |
| I44  | **A heading names itself and nothing else**: a hint is rendered beside a panel's `<h2>` and the shell's `<h1>`, never inside either                                                                                                                | `fl_frontend/src/shared/components/ui/PanelHeading.tsx :: PanelHeading`; `fl_frontend/src/shared/components/ui/PanelHeading.test.ts`; `fl_frontend/src/features/admin/components/ui/AdminShell.test.ts`; the heading-hint entry of `fl_frontend/eslint.config.mjs :: SOURCE_BANS`                                                                                                                                                                                                   |
| I45  | **No module consumes an environment value at load that the builder stage leaves undefined**: `SKIP_ENV_VALIDATION=true` makes every name `undefined` for the whole build                                                                           | `fl_frontend/src/core/buildTimeEnv.test.ts`, whose AST reader tells a function body from a module's own statements; `fl_frontend/src/core/authBuild.test.ts` drives `fl_frontend/src/core/auth.ts` under the build's config                                                                                                                                                                                                                                                         |
...
| I47  | **A field sharing a row fixes no width of its own, and nothing in that row keeps its input's intrinsic width as a floor**                                                                                                                          | `fl_frontend/src/shared/components/ui/AddressFields.tsx :: AddressFields`'s two paired rows and `fl_frontend/src/features/teams/components/forms/WebsiteUrlField.tsx`, the whole of what `fl_frontend/src/shared/components/ui/formFieldWidths.test.ts` reads; review elsewhere                                                                                                                                                                                                     |
...
| I61  | **A badge on a picker trigger sits beside the truncating span**, and each child spaces itself with `ms-2` rather than a gap on `FIELD_TRIGGER_CLASSES`                                                                                             | `fl_frontend/src/shared/components/ui/formFieldStyles.ts :: FIELD_TRIGGER_CLASSES`, which is where the no-gap half is kept                                                                                                                                                                                                                                                                                                                                                          |
...
| I64  | **`useFieldStatus` answers `undefined` for a declared path whose descriptor does not apply to the draft**; an undeclared path throws (I35)                                                                                                         | `fl_frontend/src/shared/components/ui/DraftStatusContext.tsx :: useFieldStatus`, through `fl_frontend/src/shared/utils/draftStatus.ts :: fieldStatus`                                                                                                                                                                                                                                                                                                                               |
...
| I66  | **One reveal and one action row per panel, whatever it offers**: a second reveal would arm one operation while the row below confirmed the other                                                                                                   | `fl_frontend/src/shared/components/ui/confirmPanels.test.ts`, which arms each operation of every panel rendering the shared row or reveal and counts one of each                                                                                                                                                                                                                                                                                                                    |
...
| I71  | **Every form runs `validationBehavior="aria"` and gates its submit through the shared `guardSubmit`, which are one mechanism rather than two**                                                                                                     | `fl_frontend/src/shared/components/ui/Form.tsx :: Form`, which `fl_frontend/eslint.config.mjs :: HEROUI_FORM` and `:: DYNAMIC_LOADS` make HeroUI's form's one importer by a literal specifier; `fl_frontend/src/shared/hooks/useDraftFieldErrors.ts :: guardSubmit`; `fl_frontend/src/shared/hooks/draftFieldWiring.test.ts`                                                                                                                                                        |
...
| I368 | **A field is marked required exactly where its form's schema refuses its emptied value**                                                                                                                                                           | `fl_frontend/src/features/admin/formMarks.test.ts` over every form, `fl_frontend/src/shared/components/ui/requiredMarking.test.ts` over `fl_frontend/src/shared/components/ui/RequiredMarks.tsx :: requiredBy`; `fl_frontend/eslint.config.mjs :: HEROUI_MARKED_FIELDS` and its ban on any `isRequired` outside `:: MARKED_FIELD_WRAPPERS` and five named sites                                                                                                                     |
| I371 | **A field's hint is the description of the field it sits inside, which names it in its own `aria-describedby`**                                                                                                                                    | `fl_frontend/src/shared/components/ui/Hint.test.ts :: "describes the field's own input"`; a field hint outside a field is linted out                                                                                                                                                                                                                                                                                                                                                |
...
| I75  | **A second `I18nProvider` anywhere below is the same defect narrowed to a subtree**, react-aria reading the nearest one                                                                                                                            | `fl_frontend/eslint.config.mjs :: LOCALE_PROVIDER`, refusing its import outside `fl_frontend/src/core/providers/RootProviders.tsx`                                                                                                                                                                                                                                                                                                                                                  |
| I76  | **The one exception is structural: a confirmation or a spinner may be identified by the press that raised it**, `appToast.pending` taking no options                                                                                               | review                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| I79  | **The two link tones are a ranking rather than a palette, so they may not spell one class list**                                                                                                                                                   | `fl_frontend/src/shared/components/ui/textLink.test.ts`, which compares the two class lists and pins the token each carries                                                                                                                                                                                                                                                                                                                                                         |
| I80  | **One shared component renders every panel heading**, which is what makes the heading level a single decision                                                                                                                                      | `fl_frontend/eslint.config.mjs :: SOURCE_BANS` refuses a heading taking `heading()` outside the component, planted in `fl_frontend/eslint-plants/plants.tsx.txt`; a bare `<h2>` carries no marker parting it from any other                                                                                                                                                                                                                                                         |
...
| I91  | **Each half of I47 carries its own failure**: `min-w-0` removes the floor, and a `flex-<n>` share splits FREE space two fixed widths have already spent                                                                                            | `fl_frontend/src/shared/components/ui/formFieldWidths.test.ts`, which renders both fields, reads their classes, and floors each case before judging it                                                                                                                                                                                                                                                                                                                              |
...
| I120 | **The frontend opens exactly one `MongoClient`, in `fl_frontend/src/core/db.ts`, and the sign-in library is its only reader**: application data goes through FastAPI                                                                               | Unenforced — no check sweeps for a second client; `fl_frontend/src/core/db.ts` is the only production module constructing a `MongoClient`, and `fl_frontend/src/core/auth.ts` its only production importer                                                                                                                                                                                                                                                                          |
| I362 | **Every sign-in store operation ends within its `timeoutMS`, commits included, a cold start's automatic connect within `serverSelectionTimeoutMS`, and a reconnect's handshake within `connectTimeoutMS`**                                         | `fl_frontend/src/core/db.ts :: options`; `fl_frontend/src/core/db.db.test.ts` hangs each connect, a session read and a commit, holding the experimental option across upgrades; counterpart `docs/backend/spec.md :: I320`                                                                                                                                                                                                                                                          |
| I364 | **A sign-in store unreachable at its first connect is reconnected once it answers**, never by a restart or a second client; **a close is final**                                                                                                   | `fl_frontend/src/core/db.ts :: SignInStoreClient`; `fl_frontend/src/core/db.db.test.ts` reads again after a refused first connect, reopens after a hung one, and not after a close                                                                                                                                                                                                                                                                                                  |
| I366 | **A request's API and mail calls end by one deadline under nginx's cut; a write it cuts, or whose mail may have gone, answers unknown**                                                                                                            | `fl_frontend/src/core/requestScope.ts :: boundCall`; the spines `fl_frontend/src/shared/utils/adminMutation.ts :: runAdminMutation` and `fl_frontend/src/shared/utils/publicRoute.ts :: handlePublicRequest`; `fl_frontend/src/features/einladungen/deadline.test.ts` cuts a season-wide press; counterpart `docs/backend/spec.md :: I320`                                                                                                                                          |
| I372 | **A ban's notice the deadline cuts answers the ban saved and the notice unclear**: the ban is written before its notice                                                                                                                            | `fl_frontend/src/features/sperrliste/actions.ts :: benachrichtigen` through `fl_frontend/src/core/requestScope.ts :: runAnsweringOwnCut`; `fl_frontend/src/features/sperrliste/actions.test.ts` cuts one                                                                                                                                                                                                                                                                            |
| I121 | **Admin is `ALLOWED_ADMIN_EMAILS` rather than a stored role, and `fl_frontend/src/core/allowlist.ts :: isUserAdmin` decides it on every session read** (§4)                                                                                        | `fl_frontend/src/core/auth.test.ts` and `fl_frontend/src/proxy.test.ts` drive the session read; no check counts the call sites, `fl_frontend/src/core/config.ts` validating the allowlist's shape (§1.7)                                                                                                                                                                                                                                                                            |
| I122 | **`/bereich/admin` is guarded twice and rendering fails closed even if the matcher stops matching**: `fl_frontend/src/proxy.ts`, and `fl_frontend/src/features/admin/components/providers/AdminAuthGuard.tsx` inside the admin layout's `Suspense` | Unenforced — no check compares the two; `fl_frontend/src/app/bereich/admin/layout.tsx` places the second, above the page segment                                                                                                                                                                                                                                                                                                                                                    |
| I377 | **Every person and team page reads its subject through `requireSubjectSession`**, which sends a lapsed session to sign in                                                                                                                          | `fl_frontend/src/features/funktionen/guard.test.ts :: "every person page's subject"`; `fl_frontend/src/features/funktionen/team.test.ts :: "the guard over a team's area"`                                                                                                                                                                                                                                                                                                          |
| I394 | **A `/bereich/admin` render never runs the person guard**: its switcher reads the admin session's own records through the one lookup                                                                                                               | `fl_frontend/src/features/funktionen/guard.test.ts :: "an admin render's subject reads"`; `fl_frontend/src/features/funktionen/switcher.test.ts :: "the administrator's switcher"`                                                                                                                                                                                                                                                                                                  |
| I395 | **Every signed-in shell but an area boundary's heads its sidemenu with the Funktion switcher**, shown where the Funktionen lead to two or more places                                                                                              | `fl_frontend/src/shared/components/layout/shell/AppShell.tsx :: funktionSwitcher`; `fl_frontend/src/features/funktionen/switcher.test.ts`; `fl_frontend/src/shared/components/layout/sidemenu/FunktionSwitcher.test.ts :: "when the switcher shows"`                                                                                                                                                                                                                                |
| I396 | **The switcher lists the landing's places under the landing's labels**, and marks the one the address stands in                                                                                                                                    | `fl_frontend/src/features/funktionen/utils.ts :: funktionOrteOf`; `fl_frontend/src/features/funktionen/switcher.test.ts :: "the places the switcher lists"`; `fl_frontend/src/shared/components/layout/sidemenu/FunktionSwitcher.test.ts :: "what the switcher lists"`                                                                                                                                                                                                              |
| I397 | **„Zu Deinem Bereich“ ends the switcher in the team and admin shells**, never in the person shell, whose own landing is `/bereich`                                                                                                                 | `fl_frontend/src/shared/components/layout/sidemenu/FunktionSwitcher.test.ts :: "what the switcher lists"`                                                                                                                                                                                                                                                                                                                                                                           |
| I378 | **The landing tells a confirmation still pending apart from no record at all**, and sends Funktionen sharing one address straight there                                                                                                            | `fl_frontend/src/features/funktionen/guard.test.ts :: "where the landing takes a person"`                                                                                                                                                                                                                                                                                                                                                                                           |
| I385 | **A read failing in an area layout is answered by its own panel in its shell**, the admin switcher's lookup excepted; redirects and 404s pass                                                                                                      | `fl_frontend/src/app/bereich/layoutErrors.test.ts`, over `PersonAreaBoundary`, `TeamAreaBoundary` and `AdminAreaBoundary`                                                                                                                                                                                                                                                                                                                                                           |
| I409 | **A failing switcher lookup under `/bereich/admin` renders no switcher** and leaves the page standing: the administration needs no backend answer to be reached                                                                                    | `fl_frontend/src/features/funktionen/components/ui/AdminFunktionSwitcher.tsx`; `fl_frontend/src/app/bereich/layoutErrors.test.ts :: "the administrator's switcher failing its lookup"`                                                                                                                                                                                                                                                                                              |
| I386 | **An allowlisted address whose administrator verdict lapsed is sent from `/bereich` to `/bereich/admin`**, whose proxy takes it through the step it owes                                                                                           | `fl_frontend/src/app/bereich/(persoenlich)/page.tsx`; `fl_frontend/src/features/funktionen/guard.test.ts :: "where the landing takes a person"`                                                                                                                                                                                                                                                                                                                                     |
| I380 | **A team's address opens only on a seat held on that team in that season**: any other, a `past` season's included, answers the forbidden panel                                                                                                     | `fl_frontend/src/features/funktionen/resolvers.ts :: requireTeamSeats`, swept by `fl_frontend/src/features/funktionen/team.test.ts :: "what a person meets at an address they hold no seat on"`; `fl_frontend/src/app/bereich/team/[team_id]/[saison_id]/layout.tsx :: TeamChrome`                                                                                                                                                                                                  |
| I381 | **The forbidden panel names nothing of the address it refuses**: not who holds a seat there, nor whether the team or the season exists                                                                                                             | `fl_frontend/src/features/funktionen/components/ui/TeamForbiddenPanel.tsx`; `fl_frontend/src/features/funktionen/team.test.ts`                                                                                                                                                                                                                                                                                                                                                      |
| I392 | **A refused person reaches every Funktion they hold from the forbidden panel**: each seat's address, plus `/bereich` where another Funktion or none is held                                                                                        | `fl_frontend/src/features/funktionen/components/ui/TeamForbiddenPanel.test.ts :: "where the forbidden panel sends the person"`                                                                                                                                                                                                                                                                                                                                                      |
| I393 | **Over the forbidden panel the team bar's hint says the person is not entered**, never that the address belongs to no page                                                                                                                         | `fl_frontend/src/features/funktionen/components/ui/TeamShell.tsx`; `fl_frontend/src/features/funktionen/components/ui/TeamForbiddenPanel.test.ts :: "what the team shell's bar says over an address"`                                                                                                                                                                                                                                                                               |
| I410 | **Over the team area's crash panel the bar keeps the area's own hint**, never the refusal's: a failed read says nothing of the seats held                                                                                                          | `fl_frontend/src/features/funktionen/components/ui/TeamShell.tsx :: isRefused`; `fl_frontend/src/app/bereich/layoutErrors.test.ts :: "the bar over the team area's crash panel"`                                                                                                                                                                                                                                                                                                    |
| I382 | **Every seat reaches every team-area entry**: `teamStructureFor` selects `TEAM_SIDEMENU_ENTRIES` whole, one row per existing page and no seat rule                                                                                                 | `fl_frontend/src/features/funktionen/teamStructure.ts :: teamStructureFor`; `fl_frontend/src/features/funktionen/structure.test.ts`                                                                                                                                                                                                                                                                                                                                                 |
| I390 | **The allowlist is read before the backend call, so an administrator is offered a sign-in while the backend is down**                                                                                                                              | `fl_frontend/src/core/signInGate.ts :: mayReceiveSignIn`; `fl_frontend/src/core/auth.test.ts`, whose allowlisted case answers both reads with a throw                                                                                                                                                                                                                                                                                                                               |
| I391 | **Any other address is offered a sign-in only past the ban, holding a Funktion or only pending records; a failed read offers none**                                                                                                                | `fl_frontend/src/core/signInGate.ts :: mayReceiveSignIn`; `fl_frontend/src/core/auth.test.ts` and `fl_frontend/src/features/auth/signInSideEffects.test.ts`, a case per branch                                                                                                                                                                                                                                                                                                      |
| I359 | **Every admin page but a season-bearing record's reads the season its render's header shows**: the address's, else the running one, else the first, never omitted                                                                                  | `fl_frontend/src/features/saisons/resolvers.ts :: resolveAdminSaison` and `:: selectSaison`, which `fl_frontend/src/features/saisons/components/ui/SaisonMetadataDisplay.tsx` hands the header; `fl_frontend/src/app/bereich/admin/omittedSaison.test.ts` renders both in each league                                                                                                                                                                                               |
| I363 | **A page taking its season from `requireAdminSaison` sends a league holding none to `/bereich/admin/saisons`**, the page that creates one, rather than to its error page                                                                           | `fl_frontend/src/features/saisons/resolvers.ts :: requireAdminSaison`; `fl_frontend/src/app/bereich/admin/omittedSaison.test.ts`'s empty league                                                                                                                                                                                                                                                                                                                                     |
| I135 | **Every session carries an idle window and an absolute cap, compared per request rather than configured**: dropping either as redundant restores a far longer window                                                                               | `fl_frontend/src/core/auth.ts :: withinLifetime`, over `fl_frontend/src/core/sessionLifetimes.ts :: ADMIN_LIFETIME` and `:: PERSON_LIFETIME`; `fl_frontend/src/core/auth.test.ts` and `fl_frontend/src/core/subject.test.ts` drive both kinds past each bound                                                                                                                                                                                                                       |
...
| I139 | **An unreadable kit-colour read degrades to the EMPTY set**: narrowing would withhold a colour nobody holds                                                                                                                                        | `fl_frontend/src/app/(public)/bewerbung/[saison_id]/page.tsx :: Degraded to the EMPTY set`; `fl_frontend/src/features/bewerbungen/routes.test.ts :: offers every colour where the assignments could not be read`                                                                                                                                                                                                                                                                    |
| I147 | **Every confirmation page renders each standing paragraph from the version it stamps**; the age warning, the armed alert and the answered states are its own                                                                                       | `fl_frontend/src/core/einwilligung.ts :: LIGA_KENNTNISNAHMEN`, one entry per page, the contact page's rendered by `fl_frontend/src/features/bewerbungen/components/views/BestaetigungHinweise.tsx`; `fl_frontend/src/features/bewerbungen/publicRoutes.test.ts :: renders every paragraph the version holds`                                                                                                                                                                        |
| I148 | **Only the running build's label is newly stored**: a confirmation, application seat, admin reseat or new contacts seat naming another is refused                                                                                                  | `fl_frontend/src/features/bewerbungen/utils.ts :: nenntLaufendeFassung`, an application's by `fl_backend/app/api/bewerbungen/services.py :: find_veraltete_fassung_refusal` after its replay lookup; `fl_frontend/src/app/api/bestaetigung/kontakt/route.test.ts`, `fl_backend/tests/api/test_bewerbung_submission_execution.py`, `fl_frontend/src/features/bewerbungen/kontaktSitz.test.ts`, `fl_frontend/src/features/kontakte/kontakteFassung.test.ts`                           |
...
| I162 | **A hover is a declared token, never an alpha**: an alpha composites against its ground and lands differently on every surface                                                                                                                     | `fl_frontend/eslint.config.mjs :: SOURCE_BANS` refuses an opacity or an alpha hover, planted in `fl_frontend/eslint-plants/plants.tsx.txt`; `fl_frontend/src/app/globals.css :: --color-hover` names the tokens                                                                                                                                                                                                                                                                     |
...
| I166 | **The corner ladder steps down one rung per nesting, and `rounded-3xl` is a page's outermost box alone**                                                                                                                                           | review — `fl_frontend/src/shared/components/ui/badges.ts :: PILL_RADIUS_CLASSES` is the bottom rung                                                                                                                                                                                                                                                                                                                                                                                 |
...
| I170 | **A label pill names its tone from `fl_frontend/src/shared/components/ui/badges.ts :: PillTone`, a closed set with no neutral member**                                                                                                             | The type, at every `labelBadge` call; `fl_frontend/eslint.config.mjs :: SOURCE_BANS` for the HeroUI `Chip` route, which no type reaches, planted in `fl_frontend/eslint-plants/plants.tsx.txt`                                                                                                                                                                                                                                                                                      |
...
| I186 | **A message's links stand on `AUTH_URL`'s origin, never `fl_frontend/src/core/brand.ts :: SITE_URL`**: one variable behind both puts an environment-read origin before a crawler                                                                   | `fl_frontend/src/core/emailShell.ts :: mailOrigin`; `fl_frontend/src/core/emailShell.test.ts` sweeps every builder's close, and each link minter's own test its link's; `fl_frontend/eslint.config.mjs :: SITE_ORIGIN` refuses the published one                                                                                                                                                                                                                                    |
...
| I228 | **A deployment that is not `production` mails nothing, and is demanded no key to mail with**                                                                                                                                                       | `fl_frontend/src/core/config.ts :: APP_ENV` declares it and `createFinalSchema` demands the key under it; `fl_frontend/src/core/mail.ts :: sendMail` withholds and raises `MailWithheldError`; `fl_frontend/src/core/config.test.ts` and `fl_frontend/src/core/mail.test.ts` drive both halves                                                                                                                                                                                      |
| I229 | **A pill sits on `surface` or `background`, or takes its tone's opaque pair**: a tint composites against any other fill and misses 4.5:1                                                                                                           | `fl_frontend/src/shared/components/ui/badges.ts :: trackCountBadge`, `:: trackLabelBadge`, typed to `:: FeedbackTone`; `scripts/checks/docs_gate/scheme.py :: PAIRS`; review elsewhere                                                                                                                                                                                                                                                                                              |
...
| I232 | **A 404 wears the chrome of the area whose address was asked for**, which only a segment matching that address can do                                                                                                                              | `fl_frontend/src/app/notFound.test.ts`; `fl_frontend/src/shared/components/layout/publicShell.test.ts` for the shell the root-mounted area's answer wears                                                                                                                                                                                                                                                                                                                           |
| I233 | **An admin write's success or unknown outcome refreshes the page**, whatever tags move: `runAdminMutation` calls `refresh()`; an action, for a refusal behind its landed write                                                                     | `fl_frontend/src/shared/utils/adminMutation.test.ts :: "the refresh an admin write owes the page"`, and I7's sweep for every action running inside it                                                                                                                                                                                                                                                                                                                               |
| I234 | **Every `gap-*` is one of `0.5 1 2 3 4 6 8 12`, `gap-0` aside, and a responsive pair is two adjacent rungs**                                                                                                                                       | `fl_frontend/eslint.config.mjs :: SOURCE_BANS` refuses a gap off the rungs; a pair skipping one, and an interpolated value, are review's                                                                                                                                                                                                                                                                                                                                            |
| I235 | **A collection row's hover fill is spelled on the row itself**: only a utilities-layer class outranks HeroUI's components-layer `:hover`, which paints its stock `--default`                                                                       | review — `fl_frontend/src/shared/components/ui/RowActions.tsx :: RowActionMenuItem`, `fl_frontend/src/shared/components/layout/topnav/TopNav.tsx`                                                                                                                                                                                                                                                                                                                                   |
| I236 | Every hand-mirrored length and regular expression agrees at both tiers, or the register records what puts the pair out of reach                                                                                                                    | `fl_backend/tests/shared/test_frontend_mirrors.py :: MIRRORED_BOUNDS`, `:: MIRRORED_PATTERNS` and `:: UNPAIRABLE_PATTERNS`, closed against both populations by `:: test_every_bound_this_package_declares_is_paired_or_named_unmirrored` and `:: test_every_pattern_this_package_declares_is_paired_or_named_unmirrored`                                                                                                                                                            |
...
| I238 | **A control outside prose takes its brand grade from one recipe, without the underline that names a link**: a re-spelled grade loses its dark arm                                                                                                  | `fl_frontend/src/shared/components/ui/textLink.ts :: BRAND_INK_OUTSIDE_PROSE_CLASSES`; `fl_frontend/eslint.config.mjs :: SOURCE_BANS` refuses a hand-spelled `hover:text-brand-solid` outside that module, planted in `fl_frontend/eslint-plants/plants.tsx.txt`                                                                                                                                                                                                                    |
| I239 | **Every admin CRUD region reserves the box its own route's placeholder draws, and one variable releases that box and the cover together**                                                                                                          | `fl_frontend/src/shared/components/ui/AdminCrudView.test.ts :: the box an admin CRUD region holds`, `:: every admin CRUD route`                                                                                                                                                                                                                                                                                                                                                     |
| I240 | **The gap is the whole of the distance between two siblings**: no margin on that axis between them, and no padding on a boxless child                                                                                                              | Review                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| I241 | **A region's hold is written by an animation above it**: every admin CRUD route returns the shell that runs it                                                                                                                                     | `fl_frontend/src/shared/components/ui/AdminCrudView.test.ts :: the hold every admin CRUD region reads`, `:: every admin CRUD route`                                                                                                                                                                                                                                                                                                                                                 |
| I242 | **A matched 404 streams a 200 and an unmatched one answers 404**, so every catch-all sits under an area's own url prefix                                                                                                                           | `fl_frontend/src/app/notFound.test.ts :: "puts every catch-all at a prefixed area's root"`; unenforced for the status, which only a request against a built stack reads                                                                                                                                                                                                                                                                                                             |
| I243 | **Every `/bereich/admin/:path*` arrival takes the session check, a server action's POST included**: a cookie-writing action makes Next render the admin layout into its answer                                                                     | `fl_frontend/src/proxy.test.ts`, which drives a signed-out GET, POST and every other method carrying `next-action`, and an allowlisted administrator's own action POST                                                                                                                                                                                                                                                                                                              |
...
| I379 | **Every shell declares whether its season rides the query string**: one keeping none puts no `saison_id` on a sidemenu entry or its 404's way out                                                                                                  | `fl_frontend/src/shared/components/layout/shell/AppShell.tsx :: keepsSaisonQuery`; `fl_frontend/src/app/notFound.test.ts`; `fl_frontend/src/shared/components/layout/sidemenu/Sidemenu.test.ts`                                                                                                                                                                                                                                                                                     |
| I246 | **An icon's size is a spacing-scale class on one of eight rungs**, never a `width` or `height` prop                                                                                                                                                | `fl_frontend/src/shared/components/ui/iconLadder.test.ts` for the `width` and `height` props, an icon reached through a dictionary or an `icon` prop included; review for the rungs                                                                                                                                                                                                                                                                                                 |
| I247 | **Every icon carries `aria-hidden="true"`**, or the name that makes it the thing being named                                                                                                                                                       | Review                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| I248 | **A hover arm matches its host**: `data-hovered:` where react-aria writes it, `hover:` where nothing does; the wrong arm latches or never paints                                                                                                   | Review                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| I249 | **A running label is a plain literal saying what the press itself is doing**: third person, three dots, `Speichert...`                                                                                                                             | review                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| I250 | **A failure toast names the thing rather than the act**: its success title negated — `Adresse nicht kopiert` — bar an unknown outcome (I325)                                                                                                       | review                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| I255 | **A save confirmation opens only over a draft every schema accepts**, never ahead of the block, where one accepted would be asked again                                                                                                            | `fl_frontend/src/shared/hooks/useDraftFieldErrors.ts :: guardSubmit`; `fl_frontend/src/shared/hooks/useDraftFieldErrors.test.ts :: "marks and announces a draft the schemas refuse, and raises no dialog over it"`; `fl_frontend/src/features/admin/editorWiring.test.ts`, per editor                                                                                                                                                                                               |
| I260 | **The factor is what made the SESSION, never what the account holds**: setting a passkey up ends the code's session, never promotes it                                                                                                             | `fl_frontend/src/core/auth.ts :: isAdminSession` over the server-owned `authFactor`; `fl_frontend/src/core/auth.test.ts` drives a code-borne session against a full assertion ceremony                                                                                                                                                                                                                                                                                              |
| I261 | **A passkey is added only within five minutes of a sign-in**: by either factor for a person, by passkey for an administrator bar their first                                                                                                       | `fl_frontend/src/core/auth.ts :: refuseEnrolment`, raised in `hooks.before` and in `registration.afterVerification`; `fl_frontend/src/core/auth.test.ts :: "which sessions may enrol a passkey, and how many rows they may leave"`                                                                                                                                                                                                                                                  |
| I263 | A squad row whose name the publication gate withheld reads „anonym“, avatar included, and never an empty cell                                                                                                                                      | `fl_frontend/src/features/spieler/constants.ts :: spielerAnzeigename` and `:: spielerInitialen` over `:: SPIELER_ANONYM_LABEL`, read by `fl_frontend/src/features/spieler/components/views/TeamSpielerView.tsx`; `fl_frontend/src/features/spieler/personCopy.test.ts` and `fl_frontend/src/features/spieler/components/views/TeamSpielerView.test.ts`                                                                                                                              |
...
| I316 | **Every `ALLOWED_ADMIN_EMAILS` entry is one the sign-in library itself accepts**, judged at boot: an entry it refuses is an administrator the neutral answer never mails                                                                           | `fl_frontend/src/core/emailAddress.ts :: isSignInLibraryAddress`, piped into `fl_frontend/src/core/config.ts :: ADMIN_EMAIL_ALLOWLIST`; `fl_frontend/src/core/config.test.ts :: "the sign-in library's own rule"` holds the predicate to the plugin's own request schema                                                                                                                                                                                                            |
| I317 | **`AUTH_SECRET` holds at least 32 characters**, the sign-in library's own floor, which it warns below and never refuses                                                                                                                            | `fl_frontend/src/core/config.ts :: server`; `fl_frontend/src/core/config.test.ts :: "the value an admin session is signed with"`, whose two cases sit either side of the floor                                                                                                                                                                                                                                                                                                      |
| I311 | **An administrator holds at most five passkeys**: nothing else bounds a session that has passed the assertion, and a planted row hides among real ones                                                                                             | `fl_frontend/src/core/auth.ts :: PASSKEY_LIMIT`, judged in `:: refuseEnrolment` and again in `fl_frontend/src/features/passkeys/actions.ts :: readPasskeysAction`, which closes the control                                                                                                                                                                                                                                                                                         |
| I312 | **A removal takes a fresh sign-in and never the last row**: a cookie alone could otherwise swap the administrator's authenticator for its own                                                                                                      | `fl_frontend/src/features/passkeys/actions.ts :: removePasskeyAction`, over `fl_frontend/src/core/auth.ts :: isFreshlySignedIn` and `:: removePasskey`; `fl_frontend/src/features/passkeys/actions.test.ts :: "what a removal costs, and what it refuses"`, `fl_frontend/src/features/passkeys/actions.db.test.ts`, `fl_frontend/eslint.config.mjs :: PASSKEY_DELETION`                                                                                                             |
| I313 | **Removing a passkey ends that administrator's other sessions in its own transaction**: otherwise the removed authenticator's sessions outlive it, and a refused removal ends none                                                                 | `fl_frontend/src/core/auth.ts :: removePasskey`; `fl_frontend/src/features/passkeys/actions.db.test.ts` drives an ended and a refused removal against a real replica set, `fl_frontend/src/features/passkeys/actions.test.ts` a second session's cookie                                                                                                                                                                                                                             |
| I314 | **Every passkey change mails that administrator the event and the time, and never the row**                                                                                                                                                        | `fl_frontend/src/core/passkeyEmail.ts`, sent from `fl_frontend/src/core/auth.ts :: notify` and `:: notifyPasskeyRemoved`; `fl_frontend/src/core/passkeyEmail.test.ts`                                                                                                                                                                                                                                                                                                               |
| I318 | **Every message leaves with an ASCII domain and its local part untouched**, a domain with no ASCII form being refused before any request is drawn                                                                                                  | `fl_frontend/src/core/emailAddress.ts :: withAsciiDomain`, applied where `fl_frontend/src/core/mail.ts :: sendMail` builds the request body; `fl_frontend/src/core/mail.test.ts :: "the recipient the provider is handed"`                                                                                                                                                                                                                                                          |
| I315 | **The list projects each row to what the dialog draws**: `publicKey` and `credentialID` never leave the server                                                                                                                                     | `fl_frontend/src/features/passkeys/actions.ts :: readPasskeysAction`; `fl_frontend/src/features/passkeys/actions.test.ts :: "what the list hands the dialog"`                                                                                                                                                                                                                                                                                                                       |
| I319 | **Both passkey ceremonies ask the authenticator to verify the user**, and both refuse an answer that did not                                                                                                                                       | `fl_frontend/src/core/auth.ts :: USER_VERIFICATION` and `:: refuseUnverified`, with `fl_frontend/patches/@better-auth__passkey@1.7.5.patch`; `fl_frontend/src/core/auth.test.ts :: "asks the authenticator to verify the user before it will take an assertion"`                                                                                                                                                                                                                    |
| I341 | **Two concurrent changes to one administrator's passkeys never both stand**: a count before the write admits both, so the database refuses one                                                                                                     | `fl_frontend/src/core/auth.ts :: claimAccount`, inside the transaction `fl_frontend/patches/@better-auth__passkey@1.7.5.patch` opens and the one `fl_frontend/src/core/auth.ts :: removePasskey` opens; `fl_frontend/src/core/auth.db.test.ts`, `fl_frontend/src/features/passkeys/actions.db.test.ts`                                                                                                                                                                              |
| I339 | **The media switch renders only for a birthdate reaching the served `medien_mindestalter`**, and the press sends `false` wherever it does not render                                                                                               | `fl_frontend/src/features/registrierungen/components/views/SpielerBestaetigungView.tsx` and `fl_frontend/src/features/schiedsrichter/components/views/SchiedsrichterBestaetigungView.tsx`, each `medienAngeboten`; `fl_frontend/src/features/registrierungen/publicRoutes.test.ts` and `fl_frontend/src/features/schiedsrichter/components/views/SchiedsrichterBestaetigungView.test.ts`, each `:: "the media switch, offered from the media age alone"`                            |
| I325 | **An action's failure is raised through `appToast.failure`**, which titles a failure of unknown outcome „Unklar, ob es gespeichert wurde“ in place of the site's title                                                                             | `fl_frontend/src/shared/utils/appToast.test.ts`; `fl_frontend/src/shared/utils/actionError.test.ts` for the `outcome` marker `DB-FAIL-002` sets; `fl_frontend/eslint.config.mjs :: FAILURE_BY_HAND` refuses a failure titled at its site                                                                                                                                                                                                                                            |
| I326 | **A write of unknown outcome is titled unclear on every surface**, never „nicht gespeichert“: every answer after which it may stand, and a rejected action                                                                                         | `fl_frontend/src/shared/utils/actionError.ts :: toActionErrorResult` and `:: unansweredAction`, `fl_frontend/src/core/apiRequests.test.ts` for reads a writing method sends; public panels title it „Unklar, ob es bei uns angekommen ist“                                                                                                                                                                                                                                          |
| I343 | **A `REQ-VAL-001` naming a body path marks the control whose `name` spells it**, on every form, with one sentence rather than the form's own                                                                                                       | `fl_frontend/src/shared/utils/actionError.test.ts`; `fl_frontend/src/features/schiedsrichter/addressRefusal.test.ts` renders it on the address box                                                                                                                                                                                                                                                                                                                                  |
| I344 | **A `REQ-VAL-001` no rendered control takes is announced in one toast, in the answer's own sentence**, never the generic retry, which resends the refused body                                                                                     | `fl_frontend/src/shared/hooks/useServerFieldErrors.ts :: useServerFieldErrors`; one case per admin editor in `fl_frontend/src/features/admin/editorWiring.test.ts`, and per public panel in `fl_frontend/src/features/bewerbungen/form.test.ts`, `fl_frontend/src/features/bewerbungen/publicRoutes.test.ts`, `fl_frontend/src/features/registrierungen/publicRoutes.test.ts` and `fl_frontend/src/features/schiedsrichter/components/views/SchiedsrichterBestaetigungView.test.ts` |
...
| I398 | **Every session is stamped by the endpoint that minted it, and an unlisted path mints none**: a sign-in route a release adds fails closed                                                                                                          | `fl_frontend/src/core/auth.ts :: SESSION_FACTOR_BY_PATH`, read in `databaseHooks.session.create.before`; `fl_frontend/src/core/auth.test.ts :: "what a session records about the sign-in that made it"`                                                                                                                                                                                                                                                                             |
| I399 | **A new sign-in ends the session the browser held, and a refused one ends nothing**: every step-up otherwise leaves the session it replaced alive                                                                                                  | `fl_frontend/src/core/auth.ts :: endReplacedSession`, run in `databaseHooks.session.create.after` once the sign-in commits; `fl_frontend/src/core/auth.test.ts :: "which session a new sign-in replaces"`                                                                                                                                                                                                                                                                           |
| I400 | **A passkey's session carries the credential the ceremony proved, never one its caller names**: removing that passkey ends exactly its sessions                                                                                                    | `fl_frontend/src/core/auth.ts :: ceremonyCredentialId`, the registration's posted id held to the attested one in `registration.afterVerification`; `fl_frontend/src/core/auth.test.ts`                                                                                                                                                                                                                                                                                              |
| I401 | **Over https the session cookie is `__Host-`-prefixed**, so no sibling subdomain can plant or overwrite it; over local http it keeps the library's name                                                                                            | `fl_frontend/src/core/auth.ts` `advanced`; `fl_frontend/src/core/authCookie.test.ts`, and `fl_frontend/src/core/auth.test.ts` for the http arm                                                                                                                                                                                                                                                                                                                                      |
| I402 | **A ban ends every session of the account its address folds to, keeping the account and passkeys**: a failure is told, never undoing the ban                                                                                                       | `fl_frontend/src/core/auth.ts :: endSessionsOfAddress`, from `fl_frontend/src/features/sperrliste/actions.ts :: postSperreAction`; `fl_frontend/src/core/auth.test.ts`, `fl_frontend/src/core/auth.db.test.ts`, `fl_frontend/src/features/sperrliste/actions.test.ts`                                                                                                                                                                                                               |
| I403 | **A session is minted only for an address the sign-in gate admits**: a code, a passkey and a set-up that signs in are judged alike                                                                                                                 | `fl_frontend/src/core/auth.ts :: refuseUnadmitted`, in `databaseHooks.session.create.before`; `fl_frontend/src/core/auth.test.ts`, `fl_frontend/src/core/auth.db.test.ts`                                                                                                                                                                                                                                                                                                           |
| I404 | **A person signed in by code, holding no passkey, is offered one inside the enrolment window**, never past it, where adding one is refused                                                                                                         | `fl_frontend/src/core/auth.ts :: passkeyStepOf`, answering both `getSignInDestination` and `getPasskeyStep`; `fl_frontend/src/core/auth.test.ts :: "the second factor, judged at the same guard"`                                                                                                                                                                                                                                                                                   |
| I405 | **The address field's passkey offer is armed only where the browser supports it, re-armed only after a ceremony the reader started**: else a refusal loops                                                                                         | `fl_frontend/src/features/auth/components/forms/PasskeySignIn.tsx :: PasskeySignIn`; `fl_frontend/src/features/auth/components/forms/PasskeySignIn.test.ts`                                                                                                                                                                                                                                                                                                                         |
...
| I412 | **„Später“ on the passkey offer reaches the person's own page**, and nothing there leads back to the offer, which would make it required in all but name                                                                                           | `fl_frontend/src/features/auth/passkeyOfferLater.test.ts`, over the real `fl_frontend/src/core/auth.ts :: getSignInDestination`, passkey page and person landing                                                                                                                                                                                                                                                                                                                    |

## 3. Violation → remedy

| Symptom                                                                                                                            | Cause                                                                                                                                                                                   | Remedy                                                                                                                                                                   |
| ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| An admin write saves and toasts, and the list keeps the old rows until a manual reload                                             | The action runs outside `runAdminMutation`, or writes past the API and mail clients unrecorded, so its response carries no re-rendered payload                                          | I233 — run it inside the spine, recording any other client's write at its call; a tag's re-render is Next's bookkeeping, never the action's guarantee                    |
...
| A toast renders unstyled or misaligned after a HeroUI upgrade                                                                      | HeroUI's upgraded stylesheet spells a vendored class differently from the toast rules in `globals.css`, so those rules match nothing                                                    | Re-read `@heroui/styles/dist/components/toast.css` against the case `fl_frontend/src/app/toastStack.test.ts` fails                                                       |
...
| A failure toast says only `Änderung nicht gespeichert`, and nothing says which save                                                | The call site raised a shared title with no description, so the only distinguishing half is missing                                                                                     | I42 — give it a description, or a title no other site raises                                                                                                             |
...
| A field reads as a plain box, or its fill changes colour when the pointer crosses it                                               | The call site spells its own border rather than taking `FIELD_INPUT_CLASSES`, `FIELD_GROUP_CLASSES` or `FIELD_TEXTAREA_CLASSES`, or it carries a hover fill                             | §1.17 — a field takes `control`, and its hover moves the border and never the fill                                                                                       |
...
| A session read the store never answers reads as no session, so an outage sends an administrator to sign-in and refuses every action (I362)           | Accepted — it fails closed, and `FE-AUTH-003` names the store's error; closing it needs Better Auth's `customSession` to let an adapter failure throw                                       |
