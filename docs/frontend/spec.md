...
- **A link inside text is `textLink`** (I43, I78, I79). A standalone action is a `ctaButton` link
  ...
  (`fl_frontend/src/shared/components/ui/brandInk.ts :: BRAND_INK`); a whole box is pressable only as
...
| I238 | **A control outside prose takes its brand grade from one recipe, without the underline that names a link**: a re-spelled grade loses its dark arm                                                                                          | `fl_frontend/src/shared/components/ui/brandInk.ts :: BRAND_INK`; `fl_frontend/src/shared/components/ui/brandInk.test.ts` sweeps every `.ts` and `.tsx` under `src` for a hand-spelled `hover:text-brand-solid`, sparing the two recipes by name                                                                          |
