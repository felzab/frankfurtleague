...
**Verified against:** `624dfc5`, 2026-08-11\
...
| I29 | **A picker trigger's readout never replays a list row that carries a badge, and a badge on the trigger is a sibling of the truncating span.** Such a readout comes from the prop, or from a `Value` render prop returning a string; a trigger carrying a badge gives its span `flex-1 min-w-0 truncate`, so the trailing controls hold their place with or without the badge; and the separation between two children is each child's own `ms-2`, never a gap on `FIELD_TRIGGER` | `fl_frontend/src/features/spiele/components/forms/AdminEditSpielDataForm/FormTeamPicker.tsx :: FormTeamPicker`, the trigger that carries a badge; review at every other `Autocomplete.Trigger` and `Select.Trigger` in the tree                                    | Independent failures, any one sufficient: `Autocomplete.Value` and `Select.Value` render the chosen list ITEM's children verbatim and drop the item's own className, so a row built as a name plus an `ml-auto` chip replays inside the trigger and draws the chip over the name it annotates; moving the badge inside the truncating span makes the one piece of state a closed trigger has to carry the first thing the ellipsis eats; parking the free space on the badge with `ml-auto` rather than on the span drags the clear button to the far edge only while a team is disqualified, so one control sits in two places; and a gap on `FIELD_TRIGGER` cannot supply the separation, because `Autocomplete.Indicator` wraps its absolutely positioned icon in an empty in-flow `<button>` that takes the gap instead, while `Select.Indicator` is that icon unwrapped and is no flex item at all. `pe-9` covers none of it — it keeps the row clear of the indicator, which is a different edge. None of them is visible to `tsc`, ESLint or the build |
