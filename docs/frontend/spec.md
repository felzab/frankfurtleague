...
| I162 | **A hover is a declared token, never an alpha at a call site**: an alpha composites against its ground and lands differently on every surface                                                                                              | `fl_frontend/src/shared/components/ui/hoverToken.test.ts` sweeps every `.tsx` under `src`; `fl_frontend/src/app/globals.css :: --color-hover` names the tokens, and review holds a recipe module                                                                                                                         |
