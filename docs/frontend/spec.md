...
| I43  | **A link inside text takes its underline and colour from one recipe, whose base carries the underline**: colour alone is not a link (WCAG 1.4.1)                                                                                           | `fl_frontend/src/shared/components/ui/textLink.ts :: textLink`, whose `base` carries the underline; `fl_frontend/src/shared/components/ui/textLink.test.ts` reads every module under `src` for a hand-spelled underline, that module spared by name                                                                                                                                                                                                                                     |
...
| I162 | **A hover is a declared token, never an alpha**: an alpha composites against its ground and lands differently on every surface                                                                                                             | `fl_frontend/src/shared/components/ui/hoverToken.test.ts` sweeps every `.ts` and `.tsx` under `src`, recipe modules included; `fl_frontend/src/app/globals.css :: --color-hover` names the tokens                                                                                                                                                                                                                                                                                       |
