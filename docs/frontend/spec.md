...
| A toast renders unstyled or misaligned after a HeroUI upgrade                                                                                                                    | A vendored selector the toast block in `globals.css` keys on was renamed — every class and attribute there that `AppToaster` does not write itself is HeroUI's own             | Re-read `@heroui/styles/dist/components/toast.css`; the rules say which version they were written against (ADR-0043)                                                                                                                                         |
