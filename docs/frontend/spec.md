...
| OPS-89 | I34's sweep does not reach a form dispatched from a route handler | Open — `fl_frontend/src/core/refusalPaths.test.ts` recognises a subject only at `features/*/actions.ts`, so the public application form — parsed in its route handler and mapped in `fl_frontend/src/features/bewerbungen/utils.ts` — is paired with its inputs by review rather than by the sweep. Tracked in [`docs/_roadmap/tooling-items.md`](../_roadmap/tooling-items.md)                                                                                                  |
