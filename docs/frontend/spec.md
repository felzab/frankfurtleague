...
- **Every dialog's footer puts the action first and the way back second** — left in a row, top in a
  ...
  which renders every component spelling `fl_frontend/src/shared/components/ui/formButtons.ts :: MODAL_FOOTER_CLASSES`.
...
| I229 | **A pill sits on `surface` or `background`, or takes its tone's opaque pair**: a tint composites against any other fill and misses 4.5:1                                                                                                   | `fl_frontend/src/shared/components/ui/badges.ts :: trackCountBadge`, `:: trackLabelBadge`, typed to `:: FeedbackTone`; `fl_frontend/src/shared/components/ui/pillGround.test.ts` sweeps option and badge slots, class or call; `scripts/checks/docs_gate/scheme.py :: PAIRS`; review elsewhere                                                                                                                                                                                          |
...
| I234 | **Every `gap-*` is one of `0.5 1 2 3 4 6 8 12`, `gap-0` aside, and a responsive pair is two adjacent rungs**                                                                                                                               | `fl_frontend/eslint.config.mjs :: SOURCE_BANS` refuses a gap off the rungs and `fl_frontend/src/shared/components/ui/gapLadder.test.ts` a pair skipping one; an interpolated value is read by neither                                                                                                                                                                                                                                                                                   |
...
| I240 | **The gap is the whole of the distance between two siblings**: no margin on that axis between them, and no padding on a boxless child                                                                                                      | `fl_frontend/src/shared/components/ui/gapLadder.test.ts`, which resolves gaps and margins through the min-width cascade; a `.map()`-rendered sibling, a computed `className` and padding on an interpolated child go unread                                                                                                                                                                                                                                                             |
