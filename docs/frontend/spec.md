...
| I233 | **Every admin write action calls `refresh()` on its success path**, whatever tags it also moves                                                                                                                                            | `fl_frontend/src/core/refusalPaths.test.ts :: "every slice's admin writes, and the sweep each one owes"`, which fences each slice's own `actions.test.ts` sweep                                                                                                                                                          |
