...
carrying one link for every seat that mailbox holds (`:: seatsByMailbox`), so two people on a
school inbox get one message and a person holding two seats gets one control per link rather than
per seat. The footer line saying who a message reached is split with them. Every one tells a reader
...
| `BEWERBUNG_SWEEP`                              | `on` \| `off`, case-normalised, `on` where the server sets nothing |
...
| I147 | **The confirmation page renders every paragraph of its standing text from the version it stamps**: a paragraph spelled in the component escapes the stamp                                                                                  | `fl_frontend/src/core/einwilligung.ts :: BESTAETIGUNG_ABSAETZE`; `fl_frontend/src/features/bewerbungen/publicRoutes.test.ts :: renders no paragraph of its own beside the ones the version holds`, and the reverse case beside it                                   |
