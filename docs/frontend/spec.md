...
- **`fl_frontend/src/shared/hooks/useServerFieldErrors.ts :: UNHANDLED_FIELD_REFUSAL` speaks only
  where the answer brings no sentence**: its retry resends the body just refused.
...
linter can express is held, `fl_frontend/src/core/refusalPaths.test.ts` (I34) among them.
