...
| I29 | **The document root is never a containing block** — `<html>` and `<body>` carry no `position` other than `static`, and no `transform`, `filter`, `contain: paint` or backdrop filter                                                                                                                                                | review — `fl_frontend/src/app/layout.tsx` holds both class lists                                                                                                                                                                                                   | react-aria writes a top-placed overlay's `bottom` as the viewport height minus the trigger's document offset, and CSS resolves that `bottom` against the overlay's containing block — so the two agree only while the containing block is viewport-tall, which is what an unpositioned root leaves. Make either root a containing block and the page becomes it, and every top-placed tooltip and popover opens a whole scroll height below its trigger. The signed-in routes hide it: `AppShell` is an `h-dvh` frame that scrolls inside `<main>`, so `<body>` is exactly viewport-tall under `/dashboard` and `/admin` and the two heights coincide. The public routes scroll the document, so they show it. Nothing fails on the way — the overlay is painted, opaque and on top, merely off screen |
...
| A tooltip opens a screen-height below the control it names, on a public route only   | `<html>` or `<body>` became a containing block, so react-aria measured a top-placed overlay's `bottom` against the viewport and CSS resolved it against the page               | I29 — take the `position`, `transform`, `filter` or `contain` off the root; the overlay's own geometry is correct and needs no offset                                                                                                                        |
