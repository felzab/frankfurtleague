...
| I47  | **A field sharing a row fixes no width of its own, and nothing in that row keeps its input's intrinsic width as a floor**                                                                                                                  | `fl_frontend/src/shared/components/ui/AddressFields.tsx :: AddressFields`, whose two paired rows and the website field are the whole of what `fl_frontend/src/shared/components/ui/formFieldWidths.test.ts` reads; review elsewhere                                                                                      |
...
| I229 | **A pill on `muted` or any hover fill is the tone's solid fill under its on-colour, never a tint: a composited tint misses 4.5:1**                                                                                                         | `fl_frontend/src/shared/components/ui/badges.ts :: trackCountBadge`, `:: trackLabelBadge`, typed to `:: FeedbackTone`; `fl_frontend/src/shared/components/ui/pillGround.test.ts` sweeps hovered options and badge slots; `scripts/checks/docs_gate/scheme.py :: PAIRS`; review for a tint on `muted`                     |
...
| I233 | **Every admin write action calls `refresh()` on its success path**, whatever tags it also moves                                                                                                                                            | `fl_frontend/src/core/adminWriteRefresh.test.ts :: "every slice's admin writes, and the sweep each one owes"`, which fences each slice's own `actions.test.ts` sweep                                                                                                                                                     |
...
| A count or label pill reads faint on a tab strip, or on a row the pointer is over                                                  | A tint on `muted` or on the hover fill under it: composited, its light ink measures under 4.5:1                                                                                         | I229 — `trackCountBadge` or `trackLabelBadge`                                                                                                                            |
