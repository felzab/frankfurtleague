...
| I216 | **An erased referee's editor route answers a read-only readout, never the editor**: `EditPageHeader` is the editor's chrome (I38) and this page carries its own                                                                            | review — `fl_frontend/src/features/schiedsrichter/components/views/AdminSchiedsrichterEditView.tsx` branches on the erasure stamp alone, and `fl_frontend/src/features/schiedsrichter/components/views/AdminSchiedsrichterGeloeschtView.tsx` is the readout                                                              |
...
| I234 | **Every `gap-*` is one of `0.5 1 2 3 4 6 8 12`, `gap-0` aside, and a responsive pair is two adjacent rungs**                                                                                                                               | `fl_frontend/src/shared/components/ui/gapLadder.test.ts`, which declares the rungs and reads every string literal; a recipe-built class list, and a sibling a `.map()` renders, go unread                                                                                                                                |
...
| I240 | **The gap is the whole of the distance between two siblings**: no margin on that axis between them, and no padding on a boxless child                                                                                                      | `fl_frontend/src/shared/components/ui/gapLadder.test.ts`, which resolves each breakpoint's gap and margin through the min-width cascade                                                                                                                                                                                  |
...
| I243 | **The proxy exempts a server action's POST alone**: exempting a GET carrying the same header serves the admin shell unauthenticated                                                                                                        | `fl_frontend/src/proxy.test.ts`, which drives the exemption against a GET, a POST and every other method                                                                                                                                                                                                                 |
...
| A server action fails with "An unexpected response was received from the server"                                                   | Something answered its POST with a redirect, so the client read HTML where an RSC payload belongs                                                                                       | `fl_frontend/src/proxy.ts` exempts the action's POST alone; the action's own `getAdminSession()` refuses it instead                                                      |
...
| A matched address answers 200 on its 404 (I242), which is the price of the area chrome I232 asks for                                                 | Accepted — `cacheComponents` flushes a prerendered shell before any page runs, and a `proxy.ts` rewrite's status is dropped; only a response it builds carries one                          |
