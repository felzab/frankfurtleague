...
the session it would otherwise check. Both are in the table because they are the slice's whole
surface (§1.1) and because a reader looking for a server action looks here; both invalidate
nothing, mutating no application data.
...
once and every editor gets it. `fl_frontend/src/shared/utils/undoDispatch.ts` is the press half: it
builds the offer the toast carries, `fetch`es the route rather than dispatching an action — the
editor is unmounted by the time the press lands — and turns each outcome into the sentence the
admin reads. `fl_frontend/src/shared/utils/undoRoute.ts :: handleUndoRequest` is the route half,
and it is where `getAdminSession()` and `runAdminMutation` are actually reached: each of the eight
`route.ts` files supplies a name, a schema and the replay, and nothing else. **Both answer 200 with
the outcome in the body for every reportable case**, a non-2xx landing in the dispatch's rejection
arm, which blames the transport and sends the admin to check a connection that is fine.
...
`fl_frontend/src/shared/hooks/useDraftFieldErrors.test.ts` enumerates forms by where `<Form` sits
and requires that listing to be `deepEqual` to a second one reached by eliminating a named
prose-only allowlist from every file mentioning `<Form` at all, so narrowing either route breaks the
equality rather than shrinking the sweep (I71). `fl_frontend/src/core/refusalPaths.test.ts` finds
its mappers twice over — every module whose RETURN TYPE declares the shape, and every one assigning
it — and resolves a `name` a component builds through a local helper rather than skipping it, an
unbound helper failing the sweep rather than disappearing from it; what an assignment's value is
made of decides whether it can be read at all, so `toFieldErrors(…)`, an object literal and a
forwarded `x.fieldErrors` are each handled and anything else fails (I34, I63).
`fl_frontend/src/core/toastTitles.test.ts` resolves a title expression to literals **or fails** — a
value arriving from a prop resolves only while every call site passes a literal — and reads a callee
aliased to a conditional pair branch beside branch, so a title conditioned on the same flag keeps
the variant its own branch raises (I42). `fl_frontend/src/core/bewerbungEmail.test.ts` asserts that
no line of either mail OPENS with a forged fact shaped like a real row but written by no message, so
a hit is the forgery rather than a genuine line, and
`fl_frontend/src/features/bewerbungen/schemas.test.ts` beside it spells every refused character by
code point rather than as a literal, one pasted in being invisible in a diff and in a review (I87,
I88).
...
| I46 | **A decision mail's text branch sets one fact to the line, so every value it prints is normalised where it is printed**                                                                                                       | `fl_frontend/src/core/bewerbungEmail.ts :: einzeilig` and `:: eingerueckt`, the normalisers `:: renderText` applies to every fact it prints                                                                                      |
...
| I69 | **Addresses are collected DISTINCT and each is sent its own message, all of them settled**: one person can hold two of the application's slots                                                                                | `fl_frontend/src/features/bewerbungen/notifications.ts :: collectBewerbungEmpfaenger`, whose `Promise.allSettled` keeps one refusal to its own recipient                                                                         |
| I70 | **Composing runs inside that settling, in an `async` callback**: a message that cannot be built is one rejected promise rather than a throw escaping `.map()`                                                                 | `fl_frontend/src/features/bewerbungen/notifications.ts :: sendBewerbungMail`                                                                                                                                                     |
...
| A season is edited in Compass and the change never appears                                                                         | A hand edit invalidates nothing; only an action does                                                                                                                                    | I16 — accept the daily bound or recreate the containers; the backend's own ten-minute season cache means the frontend alone is not enough (`docs/ops/spec.md` §3)        |
