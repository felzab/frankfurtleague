...
**Verified against:** `d774a73`, 2026-08-06
...
**Uncached, deliberately: two.** `getAdminSpieleActionRequired` (admin) and `getSpiel` (spiele). Both
are admin-authorized data, which does not belong in a cache shared across requests. The first also
derives its `bracket_faults` per request over the stored bracket
...
**`getSpiel` is `GET /spiele/{spiel_id}` and is cached by nothing**
([ADR-0054](../_decisions/0054-the-match-editors-read-is-uncached.md)). A cached entry here could only
carry the base `spiele` tag — the match editor is addressed by match id with no season in the URL
([ADR-0050](../_decisions/0050-a-form-that-outgrows-a-dialog-becomes-a-page.md)), and a season tag would
be wrong even where one is available, because a match write resolves the whole bracket and rewrites
fixtures the request never named
([ADR-0042](../_decisions/0042-a-result-entry-resolves-the-whole-bracket.md)). That tag is invalidated by
every match write, and under `cacheComponents` revalidating this dynamic segment raises the I22
invariant and truncates the response of the server action that triggered it. With nothing tagged on the
route there is nothing to revalidate. It throws `APIBadStatusError` with `statusCode: 404` for an unknown
id, which the editor page catches to reach `notFound()`; every other error is rethrown.
...
| I22 | **A dynamic segment's page awaits `params` (and every runtime API) INSIDE a `<Suspense>` boundary, never at its own top level.** All three do: `/admin/spiele/[spiel_id]`, `/dashboard/teams/[team_id]`, `/dashboard/spieler/[team_id]` | The page component is synchronous and renders `<Suspense><…Content {...props} /></Suspense>`; `next build` prints an App Shell sub-entry under each of the three, which is the visible signal it worked      | `cacheComponents` builds an App Shell with FALLBACK params for a segment with no `generateStaticParams` (ADR-0011). A top-level await ties that shell to one URL, and Next then raises `Invariant: postponed state should not be provided when fallback params are provided`. The action's response is truncated, the client reports "An unexpected response was received from the server", and the write never reaches the backend. Nothing in the toolchain catches it: it type-checks, lints, builds and renders correctly on a direct visit. **This boundary is necessary and NOT sufficient** — the same invariant is reachable a second way, by a server action's `updateTag` revalidating the segment from a different route, which no page shape fixes. That route is closed per-read by not caching ([ADR-0054](../_decisions/0054-the-match-editors-read-is-uncached.md)), and it is still open on `/dashboard/teams/[team_id]` and `/dashboard/spieler/[team_id]`, whose `getTeam` is tagged `teams` and invalidated by every match write |
...
| A server action reports "An unexpected response was received from the server"    | Its `updateTag` revalidated a dynamic segment, which raised the I22 invariant and truncated the response mid-stream | I22 — the page shape does not cover this route. Stop caching the read that segment revalidates on (ADR-0054), or accept the crash |
