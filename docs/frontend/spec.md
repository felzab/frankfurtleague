...
| I359 | **Every admin page but a season-bearing record's reads the season its render's header shows**: the address's, else the running one, else the first, never omitted                                                                          | `fl_frontend/src/features/saisons/resolvers.ts :: resolveAdminSaison` and `:: selectSaison`, which `fl_frontend/src/features/saisons/components/ui/SaisonMetadataDisplay.tsx` hands the header; `fl_frontend/src/app/admin/omittedSaison.test.ts` renders both in each league                                                                                                                                                                                                       |
| I363 | **A page taking its season from `requireAdminSaison` sends a league holding none to `/admin/saisons`**, the page that creates one, rather than to its error page                                                                           | `fl_frontend/src/features/saisons/resolvers.ts :: requireAdminSaison`; `fl_frontend/src/app/admin/omittedSaison.test.ts`'s empty league                                                                                                                                                                                                                                                                                                                                             |
