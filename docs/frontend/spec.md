...
| `bewerbungen`    |   ✅    |    ✅     |   ✅    |   ✅    | Triage alone — an acceptance and a decline, both irreversible, and no other write; `constants.ts`, `facets.ts`, `notifications.ts` (the decisions' mail fan-out), `resolvers.ts`, `types.ts`, `utils.ts`, tests                                                                        |
...
**The route handlers are the eight page-owned editors' undos — `POST /api/admin/spiele/undo`,
...
`spielorte`, `schiedsrichter`, `saisons`, `spieltage` and `bewerbungen`, every one of them through a
page-owned editor, and `kontakte`, whose erasure stands as a panel on the list rather than in an
...
a second time (`REQ-BEWERBUNG-001`). The other absences are
