...
**Every admin action and admin undo runs under one request deadline** (I366),
`fl_frontend/src/core/requestScope.ts :: REQUEST_DEADLINE_MS`, set where the request's scope opens:
`fl_frontend/src/core/api.ts :: apiClient` and `fl_frontend/src/core/mail.ts :: sendMail` cap each
call's own bound to what is left of it and start none once it has passed. **A write the deadline cut
answers as of unknown outcome whatever the action made of the cut**, a mail fan-out settling it among
its refusals included: part of the write may stand, as it may where the backend's own deadline cuts
one (`docs/backend/spec.md :: I321`). The deadline sits under the edge's cut by what the same response
spends outside it — the proxy's session read before the action, and the page's re-render after it,
whose reads keep their own bounds — so the administrator is answered by this application rather than
by nginx's 504.
...
| I366 | **A request's API and mail calls all end by one deadline under nginx's cut, and an admin write it cuts answers as of unknown outcome**                                                                                                     | `fl_frontend/src/core/requestScope.ts :: boundCall` and `fl_frontend/src/shared/utils/adminMutation.ts :: runAdminMutation`; `fl_frontend/src/features/einladungen/deadline.test.ts` cuts a season-wide press; counterpart `docs/backend/spec.md :: I320`                                                                                                                                                                                                                               |
