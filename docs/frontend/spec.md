...
admits exactly the sessions `getAdminSession()` admits, so a link-borne session of an allowlisted
...
| I261 | **A further passkey is enrolled only from a session the passkey made minutes ago**, the plugin's own gate being freshness a link-borne session is inside                                                                                           | `fl_frontend/src/core/auth.ts :: refuseEnrolment`, raised in `hooks.before` and in `registration.afterVerification`; `fl_frontend/src/core/auth.test.ts :: "which sessions may enrol a passkey, and how many rows they may leave"`                                                                                                                                                                                                                                                  |
