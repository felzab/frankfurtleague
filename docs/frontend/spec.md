...
**Verified against:** `b5a7fa5`, 2026-08-10\
...
| I29 | **A picker trigger renders its own selected value, and a badge beside it is a sibling of the truncating span with a gap between them.** The label comes from the prop, never from `Autocomplete.Value` or `Select.Value`; the badge carries `ml-auto shrink-0` against the span's `min-w-0 truncate`; the separation comes from `FIELD_TRIGGER` | `fl_frontend/src/shared/components/ui/formFieldStyles.ts :: FIELD_TRIGGER`, and every `Autocomplete.Trigger` and `Select.Trigger` in the tree                                                                                                                      | Independent failures, any one sufficient: `Autocomplete.Value` renders the chosen list ITEM's children verbatim and drops the item's own className, so a row built as a name plus an `ml-auto` chip replays inside the trigger and draws the chip over the name it annotates; moving the badge inside the truncating span makes the one piece of state a closed trigger has to carry the first thing the ellipsis eats; and without the gap each pair of children meets at zero separation, measured 0.00px on the running stack against 8px in the listbox rows beside them. `pe-9` does not cover this — it keeps the row clear of the absolutely positioned indicator, which is a different edge. None of the three is visible to `tsc`, ESLint or the build |
