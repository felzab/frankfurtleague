...
| `schiedsrichter` |   ✅    |    ✅     |   ✅    |   ✅    | Full CRUD; `constants.ts`, `facets.ts`, `resolvers.ts`, `schiedsrichterDraftStatus.ts`, `types.ts`                                                 |
...
| A toast renders unstyled or misaligned after a HeroUI upgrade                                                                                                                    | One of the two vendored selectors `globals.css` still owns was renamed — `.toast` or `.toast__close-button`                                                                    | Re-read `@heroui/styles/dist/components/toast.css`; the rules say which version they were written against                                                                                                                                                                                                                                                                  |
...
| A picker trigger's clear button sits in a different place depending on whether the team is disqualified                                                                          | The free space is parked on the badge with `ml-auto` instead of on the name span                                                                                               | I30 — `flex-1 min-w-0 truncate` on the span; the badge and the clear button each carry their own `ms-2`                                                                                                                                                                                                                                                                    |
