...
**Is:** the four fields a bracket resolution can rewrite on a fixture nobody asked it about — each side's club and goals, the shoot-out record and the `sonderereignis` — declared once and reaching the wire twice, as the report of every fixture a save moved and as the body that puts one of them back.\
**In code:** `fl_backend/app/api/spiele/schemas.py :: _SpielPaarung`, extended by `:: FLSpielPriorPaarung` for the report `:: FLPatchSpielDataResponse` serves under `prior_paarungen` and by `:: FLPatchSpielPaarungPayload` for the body `fl_backend/app/api/spiele/admin_router.py :: patch_spiel_paarung` takes; the report is composed by `fl_backend/app/api/spiele/crud.py :: report_prior_paarungen` and sent back by `fl_frontend/src/features/spiele/mutations.ts :: patchAdminSpielPaarung`. The German an administrator reads is `fl_frontend/src/features/spiele/utils.ts :: formatSpielUpdateMessage`.\
**Trap:** the word names which two clubs meet and the shape carries what they PRODUCED as well, so putting a Paarung back puts the goals, the shoot-out record and the `sonderereignis` back with the occupants, and no `ergebnis` travels — the score is composed from the goals again at the far end. It is deliberately NARROWER than the fixture holding it: the date, the time, the venue, the referee, the note and both `Quelle`s are read off the stored document by `fl_backend/app/api/spiele/schemas.py :: FLPatchSpielPaarungPayload.completed_with` rather than taken from the request, which is what lets an edit made between the save and the undo survive — and which means no restore ever puts a cleared reference back, the bracket being resolved again over the wiring as it now stands. One entry stands per FIXTURE and never per rewrite, so a fixture both of a save's reports name is restored once, and the fixture the request itself named is left out of them because that caller already holds its before-state. And it is not an `Aufstellung`: the site stores a starting line-up too, and that is the squad a club fields rather than the pairing it plays in.\
**See:** backend spec I210 for what a restore names and what it reads instead, [backend spec §1.1](backend/spec.md#11-endpoint-inventory) for the route, `fl_backend/tests/api/test_spiel_paarung_execution.py :: TestAnUndoReplayPutsTheFixtureBackAsItStood` for the sequence driven against a database, and the `Quelle` entry below for the references a restore leaves where they stand.
