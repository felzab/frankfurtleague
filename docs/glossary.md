...
**Trap:** the id is a short fixed-length string rather than an ObjectId (`fl_backend/app/shared/schemas/bounds.py :: SAISON_ID_LENGTH`), and every model that ACCEPTS one AS DATA TO STORE holds it to that length. Backend spec I5 names the kinds that deliberately do not: a model echoing a stored id, because one refusing a stored row would answer 500 for the whole list it appears in, and a LIST FILTER, which matches rather than stores. So an id of the wrong length that reaches the database by a route holding it to nothing is echoed back without complaint, while every match and matchday carrying it fails to read. The one payload that MINTS an id is tighter than that length — four ASCII digits naming a year the league can play (backend spec I204) — so a create refuses an id the read models go on serving. The application window carries an asymmetry of its own: `_SaisonWritable` declares `bewerbung` with NO default, so a PATCH omitting the key is refused rather than silently closing a window somebody opened, while `FLSaison` re-declares it with one, every season stored before the field carrying neither key nor value.\
**See:** backend spec I5 for the length, I204 for the minted id, I18 for the single path to `status`.
...
**Trap:** only `vorname` is required and `nummer` is free text; the base tier reads a surname as an initial (`READ-PUPIL-001`) and is served no `stufe` (`READ-PUPIL-002`), no birthdate, no consent record and neither `is_nachgetragen` nor `rolle`, so no public squad list names a captain; and `FLSpieler` is one player against one season and carries no `saison_id`, so the admin list reads `GET /spieler/memberships` instead.\
...
**Trap:** deletion is soft and stays soft under a privacy request — every match embeds the referee's name and id, so an anonymisation nulls `name`, `schule` and the members of `kontakt`, on the row and on every match, where a pupil, whom no match embeds, is erased outright; it also RETIRES the row, so an erased referee takes no new fixture (`REQ-BOOKING-001`) and cannot be reactivated (`REQ-ANONYMISE-003`), while a retirement the row already carried keeps its own day; `default_payment` survives, being the league's rate rather than a fact about the person; a rename must fan out into the embedded copy on every match carrying it — every season's, unlike a club's, because a referee is not season-scoped, and that fan-out is one-way — a rename putting the name, the school or a contact detail back onto an anonymised referee is refused (`REQ-ANONYMISE-002`), the snapshot window being the only route back; and the collection's own read is admin-tier, a referee being a pupil whose `kontakt` and `schule` are private (`READ-CONTACT-001`) beside a `default_payment` that is money (`READ-MONEY-001`).\
...
**Is:** one school's application to play one season, stored as it arrived apart from what the confirmation flow moves: the triage moves `status`, `entscheidung` and `team_id`; each seat's own person fills their `geburtsdatum` and consent record, or answers with a `Widerspruch` and empties the slot; a re-send replaces the entry under `bestaetigungen` of every seat that person holds and restarts `bestaetigungsfrist`; and a contact erasure empties the slot naming the person who asked for it, with the entry beside it. The opponent it would like on the first Spieltag is a FREE STRING and never a reference: a school may name an applicant nobody has accepted yet, and a picker over the accepted ones would give a later applicant the longer list.\
**In code:** `fl_backend/app/api/bewerbungen/schemas.py :: FLBewerbung`, the stored shape; `POST /bewerbungen` in `fl_backend/app/api/bewerbungen/public_router.py` is what creates one, and the two decisions are `POST /bewerbungen/{bewerbung_id}/annehmen` and `.../ablehnen` in `fl_backend/app/api/bewerbungen/admin_router.py`. Three more endpoints write to a stored one: `POST /bewerbungen/einwilligung` in `fl_backend/app/api/bewerbungen/einwilligung_router.py`, the re-send `POST /bewerbungen/{bewerbung_id}/einwilligung/{seat}/erneut` beside the decisions, and `POST /kontakte/erasure` in `fl_backend/app/api/kontakte/admin_router.py`.\
**Trap:** **while the application still stands `eingereicht`**, exactly one of `team_id` and `schule` carries a value — the club the applicant picked, or the school they propose — and no validator of types and enums can say so, which is why acceptance judges it and refuses the rest (`REQ-BEWERBUNG-002`) rather than branching on an assumption. Acceptance is IRREVERSIBLE, `saison_teams` having no DELETE, so a second decision on either endpoint is refused (`REQ-BEWERBUNG-001`) rather than entering the club again; it writes the created club's id back into `team_id` and leaves `schule` where it stood, so a decided application always names a club and an accepted new school's names both. One base-tier route serves stored fields of an application: the confirmation view answers the seat a link opens — its state, the school, the season, the role, the holder's first name and the wording's version, and nothing a leaked link could act on (`READ-BEWERBUNG-002`). Everything else is admin-tier, unlike `spiele`: an application holds three people's email addresses and telephone numbers, a date of birth per seat that stays null until that person enters it, and says which schools asked and were turned down. The public form's create is base-tier too, and answers the deadline it wrote and one raw link per seat, never the hashes it stored (`READ-BEWERBUNG-001`).\
...
**Is:** the closed set `A` through `P` on the `saison_teams` junction; a season runs the first `rules.number_of_groups` of them.\
**In code:** `fl_backend/app/api/teams/schemas.py :: FLGruppenNames`, mirrored by `fl_frontend/src/features/teams/schemas.ts :: FLGruppenSchema`.\
**Trap:** the grouped response is seeded from `rules.number_of_groups` rather than from the teams present, so a group holding nobody keeps its key and a group the season does not run has none; and teams arrive already in standing order, so re-sorting a group anywhere is a second answer to who finished second.\
...
### `Paarung` — the two sides of one fixture and what they produced

**Is:** the four fields a bracket resolution can rewrite on a fixture nobody asked it about — each side's club and goals, the shoot-out record and the `sonderereignis` — declared once and reaching the wire twice, as the report of every fixture a save changed and as the body that puts one of them back.\
**In code:** `fl_backend/app/api/spiele/schemas.py :: _SpielPaarung`, which `:: _SpielRestore` joins to `:: FLSpielPriorOtherFields` for both wire shapes: `:: FLSpielPriorPaarung` for the report `:: FLPatchSpielDataResponse` serves under `prior_paarungen`, and `:: FLPatchSpielPaarungPayload` for the body `fl_backend/app/api/spiele/admin_router.py :: patch_spiel_paarung` takes; the report is composed by `fl_backend/app/api/spiele/crud.py :: report_prior_paarungen` and sent back by `fl_frontend/src/features/spiele/mutations.ts :: patchAdminSpielPaarung`. The German an administrator reads is `fl_frontend/src/features/spiele/utils.ts :: formatSpielUpdateMessage`.\
**Trap:** the word names which two clubs meet and the shape carries what they PRODUCED as well, so putting a Paarung back puts the goals, the shoot-out record and the `sonderereignis` back with the occupants, and no `ergebnis` travels — the score is composed from the goals again at the far end. Beyond those four it is narrower than the fixture holding it, and by no fixed set: `fl_backend/app/api/spiele/schemas.py :: FLPatchSpielPaarungPayload.completed_with` puts back the `:: FLSpielRestorableField` members `other_fields.replaced` names and reads every other off the stored document, so an edit somebody made between the save and the undo survives — while a `Quelle` that save itself cleared does come back. The block carries every one of those members whichever ones `replaced` names, so a consumer reading the block rather than that list reverts whoever moved a field after the save. One entry stands per FIXTURE and never per rewrite, so a fixture both of a save's reports name is restored once, and the fixture the request itself named leads them, in an order nothing downstream may re-sort. And it is not an `Aufstellung`: the site stores a starting line-up too, and that is the squad a club fields rather than the pairing it plays in.\
**See:** backend spec I210 for what a restore names and what it reads instead, I215 for the order and for the fields beyond the Paarung, [backend spec §1.1](backend/spec.md#11-endpoint-inventory) for the route, `fl_backend/tests/api/test_spiel_paarung_execution.py :: TestAnUndoReplayPutsTheFixtureBackAsItStood` for the sequence driven against a database, and the `Quelle` entry below for what a standing reference owns.
...
### `stufe` — which Oberstufe level a squad row records

**Is:** the Halbjahr of the Hessen Oberstufe a player is in, a closed set covering both phases, held on the `saison_spieler` junction and nullable, with each season declaring in its rules which of the members its squad form offers. Inside `oberstufengymnasium` the word is not this field at all, that being a `schulform` value describing the SCHOOL rather than anybody in it, and inside `stufengroesse` it is a whole Abi-Jahrgang rather than a level.\
**In code:** `fl_backend/app/api/spieler/schemas.py :: FLSpielerStufe`, declared on `:: _SaisonSpielerWritable`, which the squad payloads and the flattened read share, and again on `:: FLSaisonSpielerRow`; the season's narrowing is `fl_backend/app/api/saisons/schemas.py :: FLSaisonRules.erlaubte_stufen`, whose list every picker orders against `fl_frontend/src/features/spieler/constants.ts :: orderStufen`.\
**Trap:** `erlaubte_stufen` bounds what the squad FORM offers and never what a stored row holds, so narrowing it leaves the rows outside it readable rather than invalid; `E2` is in the closed set although the current season has nobody in it, a set stopping at what a season contains refusing an entry as the year turns; and the base tier is served no `stufe` at all (`READ-PUPIL-002`), so a public squad list carries none; and it is not an age — the person's own `geburtsdatum` answers that, and is optional (`fl_backend/app/core/domain.py :: UNENFORCED`).\
**See:** backend spec I35 for the closed sets, I16 for why the validator carries the enum and no range, and [`domain.md`](domain.md) for why a substring match attributes this term's facts to another concept.

### `stufengroesse` — how many pupils are in the Abi-Jahrgang the team comes from

**Is:** one number on a public application: the whole Abi-Jahrgang the team comes from, the pupils who will never play included, rather than the squad it expects to field. Stated once by the school and checked against nothing afterwards — no season rule bounds it, and nothing compares it to the squad that arrives at acceptance.\
**In code:** `fl_backend/app/api/bewerbungen/schemas.py :: FLBewerbung.stufengroesse`, required and bounded on the write alone by `:: FLPostBewerbungPayload` against `fl_backend/app/shared/schemas/bounds.py :: BEWERBUNG_STUFENGROESSE_MAX`; the stored shape is typed and left out of `required` in `fl_backend/app/core/constraints.py :: COLLECTION_VALIDATORS`; the label `Größe der Stufe` and the sentence under it that fixes which pupils are counted are `fl_frontend/src/features/bewerbungen/components/forms/BewerbungForm/FormSchuleSection.tsx`.\
...
**In code:** `fl_backend/app/shared/schemas/bounds.py :: TEAM_SHORTHAND_LENGTH` fixes the width and `fl_frontend/src/features/bewerbungen/constants.ts :: KUERZEL_LAENGE` mirrors it; `fl_backend/app/core/constraints.py :: uniq_shorthand` holds the club's copy unique; the availability read is `fl_backend/app/api/bewerbungen/public_router.py :: get_kuerzel`, answering `fl_backend/app/api/bewerbungen/schemas.py :: FLBewerbungKuerzelResponse`.\
**Trap:** the German and the English meet inside one URL, `/bewerbungen/kuerzel/{shorthand}`, so a search for either spelling finds none of the other's sites. Uniqueness is byte for byte: the upper-casing sits at the inputs that take a code and nowhere on the wire or in the database, so a write arriving by any other route stores `gg` beside `GG` as two clubs — which is why the triage's `Dublette` folds case where the index does not. A season read serves the JUNCTION's copy, rewritten only into seasons that are not `past` (backend spec I13), so a club recoded after a season finished still reads there under the code it played under and what is on screen is no key to look the club up by; `GET /teams/memberships` serves the club's own instead. At the unauthenticated write a code some club already holds is refused on the box that holds it (`REQ-BEWERBUNG-008`) and the availability read narrows that collision without closing it, while two applications proposing one FREE code are both accepted — uniqueness across applications would let a stranger lock a school out by typing its code — so the queue marks the pair a `Dublette` and the SECOND acceptance is the one `uniq_shorthand` refuses.\
**See:** the `Team` entry above for the club this identifies, the `Dublette` entry below for the fold the triage applies, and backend spec I20 for the create that never revives a retired holder.
...
### `Einwilligung` · `Kenntnisnahme` — one stored key over two records: a pupil's consent, and what a contact seat was told

**Is:** the block stored under the key `einwilligung` saying what it covers (`umfang`), when (`datum`), on whose word it is held — `erteilt_von` on a pupil, `erfasst_von` on a contact seat — and the day it was confirmed (`bestaetigt_am`, null while it is not); carried once on a pupil and once on each contact seat of a team's season, where it also cites the wording that person was shown (`text_version`, which the pupil block has no field for).\
**In code:** `fl_backend/app/api/spieler/schemas.py :: FLEinwilligung` for a pupil and `fl_backend/app/api/teams/schemas.py :: FLKontaktKenntnisnahme` for a contact seat, validated by `fl_backend/app/core/constraints.py :: _EINWILLIGUNG` and `fl_backend/app/core/constraints.py :: _KONTAKT_KENNTNISNAHME`; each half has its own Zod mirror, `fl_frontend/src/features/spieler/schemas.ts :: FLEinwilligungSchema` and `fl_frontend/src/features/teams/schemas.ts :: FLKontaktKenntnisnahmeSchema`, and `fl_frontend/src/core/einwilligung.ts :: LIGA_KENNTNISNAHMEN` is the registry every `text_version` names an entry of.\
**Trap:** a contact seat's block is no consent and only the model name says so — the stored key is `einwilligung` on both collections, so a grep for the key finds both vocabularies and a rename of the key would move every stored record. The published basis for holding a contact person's details is Art. 6(1)(b)/(f) and the confirmation page says so in as many words (`fl_frontend/src/core/einwilligung.ts :: BESTAETIGUNG_ABSAETZE`); the one consent on that block is the optional WhatsApp scope. The two models share every field name but the provenance one, and not one value. `umfang` is `kader_oeffentlich` or `intern` on a pupil and `kontaktdaten` or `kontaktdaten_whatsapp` on a contact seat; `erteilt_von` asks WHO agreed on a pupil — `erziehungsberechtigt`, `volljaehrig` or `bestandsuebernahme` — while `erfasst_von` asks WHETHER the person themselves answered on a contact seat, `person` against `administrativ`. `volljaehrig` names who spoke and pins no age; the league's is `fl_backend/app/shared/schemas/bounds.py :: BEWERBUNG_KONTAKT_MIN_AGE_YEARS`. A helper, a facet or a label written against one model is therefore wrong on the other, and no type refuses the reuse. `text_version` names a REGISTRY LABEL and never the words: it says which text the stamping person was shown, so an entry there is never reworded or removed and new wording is a new label — which is why the confirmation page carries one of its own, the applicant who ticked for three people having read a different page. The WhatsApp scope is on the read model alone, so no administrative write can transcribe it. And `bestaetigt_am` on a contact seat is stamped by that seat's own link and by nothing else, while on a pupil the field is served with no writer a person can reach (`docs/_roadmap/items.md :: 8wd7-ff49`).\
**See:** the `kontakte` entry above for why the two vocabularies stay unshared, and [`datenschutz.md`](datenschutz.md#2-consent-comes-from-the-person-from-16) for the rulings the contact half implements.

### `Bestätigung` — one contact seat's own answer to being entered on an application

**Is:** the bookkeeping beside each seat of a `Bewerbung` — the hash of the link that seat's person was mailed, the day it went, the day a reminder went and the day they exercised their `Widerspruch` (`abgelehnt_am`) — kept beside the slot rather than inside it, so a Widerspruch that empties the person's slot leaves the record of it standing. One link opens two doors: the person confirms their entry, or contradicts it, and the second empties the slot and tells the submitter.\
**In code:** `fl_backend/app/api/bewerbungen/schemas.py :: FLBewerbungBestaetigungen` over `fl_backend/app/api/bewerbungen/schemas.py :: FLBewerbungBestaetigung`, validated by `fl_backend/app/core/constraints.py :: _BEWERBUNG_BESTAETIGUNG`; written by `fl_backend/app/api/bewerbungen/services.py :: compose_bestaetigung` at submission, `fl_backend/app/api/bewerbungen/services.py :: compose_erinnerung_update` for a reminder and `fl_backend/app/api/bewerbungen/services.py :: compose_erneut_update` for an administrator's re-send; the link opens `fl_backend/app/api/bewerbungen/einwilligung_router.py`, and `fl_frontend/src/features/bewerbungen/components/views/BestaetigungView.tsx` is the page behind it.\
**Trap:** the raw token is the whole credential and exists only in the mail — the document keeps `token_hash`, and after a reminder `token_hash_zuvor` beside it, and no model declares either, so no admin read can serve one and a lost link is re-minted rather than recovered. A REMINDER and a RE-SEND are different acts: the reminder mints a fresh link and keeps the first hash live, so both messages work and `bestaetigungsfrist` does not move, while a re-send replaces the whole entry of every seat that person holds, which is what voids the earlier links and restarts the deadline. That deadline sits on the APPLICATION and not on the seat, so one seat's re-send moves it for all three, and a deadline that passes deletes the whole application with every contact record on it (`docs/backend/spec.md :: I151`). And the pending state has no member of `status`: an application nobody has confirmed reads `eingereicht` like any other, and it is acceptance that refuses while a seat is outstanding (`REQ-BEWERBUNG-013`). Outstanding is read INSIDE the slot and never from this block: `fl_backend/app/api/bewerbungen/services.py :: ausstehende_seats` asks each seat's `Kenntnisnahme` for its `bestaetigt_am`, so the day a link went, a reminder's day and an `abgelehnt_am` recorded here move acceptance not at all — which is why a Widerspruch counts as answered to the link that spends it (`fl_backend/app/api/bewerbungen/services.py :: seat_is_answered`) and stays outstanding to the acceptance writing `saison_teams`, one seat read by two questions. The second door is spelled two ways: the screen and every message call it Widerspruch (`fl_frontend/src/features/bewerbungen/components/views/BestaetigungFormPanel.tsx :: WIDERSPRUCH_SENDEN`) while the wire and the stamp say `abgelehnt`, so a sentence, a facet or a label taking the schema's word for the reader's puts a rejection in the mouth of the person who was entered rather than the league's — `Absage` being the league's own refusal of a whole application.\
**See:** the `Einwilligung` entry above for what a confirmation stamps inside the slot, the `Zustellstand` entry for the delivery state that sits INSIDE this block, so `POST /kontakte/erasure` takes it with the person and a re-send replaces it whole — a Widerspruch does not, leaving it beside the emptied slot as the record of the decline itself does — the `Bewerbung` entry for the application this hangs off, and backend spec I152 for what a reminder writes. Widerspruch has no entry of its own: it is one of this term's two doors, and splitting it out would put a seat's two answers on two pages.

### `Dublette` — two open applications the triage marks as colliding

**Is:** two or more `eingereicht` applications to one season that collide on a single key, marked side by side for an administrator to choose between. Nothing stores one: the read that serves the triage queue answers which keys collide, and the page marks every row holding one of them.\
**In code:** `fl_backend/app/api/bewerbungen/schemas.py :: FLBewerbungenListResponse.dubletten_schluessel` is the wire field, composed by `fl_backend/app/api/bewerbungen/services.py :: dubletten_schluessel_of` over `:: build_dubletten_pipeline`. The frontend module is spelled in English — `fl_frontend/src/features/bewerbungen/duplicates.ts`, where `:: markBewerbungDubletten` marks the served rows and `:: BEWERBUNG_DUBLETTE_LABEL` carries the two German chips — so a search for one spelling finds none of the other's sites.\
**Trap:** the key is the season and exactly ONE identity — the club id where an applicant picked one, the case-folded Kürzel where it proposes a new school — and never the school's NAME, so two schools of one name asking under different Kürzel are no Dublette, and one club applying in two seasons is two applications rather than one. That fold is deliberately stricter than every comparison the write path makes: `fl_backend/app/core/constraints.py :: uniq_shorthand`, the availability read `fl_backend/app/api/bewerbungen/public_router.py :: get_kuerzel` and the submission's own refusal all match byte for byte, so a pair parted only by case is one collision here and two clubs there (`fl_frontend/src/features/bewerbungen/duplicates.ts :: dublettenSchluessel`). It is SHOWN and never refused — no write path rejects a colliding application (`:: markBewerbungDubletten`). And only `eingereicht` counts at both ends, a decided application sharing a Kürzel neither making a collision nor being marked by one, which is why the queue reads the ROW's own status rather than the key's.\
**See:** backend spec I203 for the pass over the whole queue, and the `Bewerbung` entry above for the application this hangs off.
...
### `Vorgangsnummer` · `trace_id` — the id naming one write on the action log

**Is:** the trace id of the request that made a write, copied off an action-log row to find every row that one save produced (`Aktion` above). nginx mints it per request as thirty-two lowercase hex characters ([logging spec §1.1](logging/spec.md#11-the-trace-id)).\
**In code:** `fl_backend/app/core/recording.py :: record_write` stamps it, `fl_backend/app/api/aktionen/schemas.py :: FLAktion` serves it and `fl_backend/app/api/aktionen/admin_router.py :: get_aktionen` takes it as a search term. The German is screen copy alone: `fl_frontend/src/features/aktionen/components/collections/AdminAktionenTable.tsx` labels the row's copy control, `fl_frontend/src/features/aktionen/constants.ts` offers it in the search placeholder, `fl_frontend/src/features/admin/constants.ts` explains it in the page's hints, and `fl_frontend/src/core/toastTitles.test.ts` holds the toast title the copy raises.\
**Trap:** `-nummer` is the trap — the value is a hex string, so nothing about it sorts, increments or bounds a range, an input mask or a validation written for a number refuses the real thing, and one retyped from a screenshot does not survive a `0`/`O` confusion. The German appears in screen copy alone and `trace_id` in code alone, so a search for either spelling finds none of the other's sites. A row no request made carries the sentinel `SYSTEM` in its place (`fl_backend/app/core/logging.py :: NO_REQUEST_SENTINEL`), which gathers every system write rather than one. The same id names every log line of that request across all three surfaces, where the action log answers for recorded writes alone, so an administrator searching it and whoever reads the container logs are asking two different questions. And the rows it names are expired twelve months after the write they recorded (backend spec I119), so one quoted in a support request has a shelf life.\
**See:** [logging spec §1.1](logging/spec.md#11-the-trace-id) for who mints the id and what becomes of a client's own, and the `Aktion` entry for what one trace gathers.

### `Zustellstand` — what became of the last message to one contact seat

**Is:** the delivery state stored at `bestaetigungen.<seat>.zustellung` on a `Bewerbung` — the mail provider's own id for that message, one of six German states, the provider's token for a refusal, and the instant it reported. `angenommen` is the provider ACCEPTING the request, which is all a send ever learns; `zugestellt`, `verzoegert`, `unzustellbar`, `unterdrueckt` and `beschwerde` are what a delivery event reports.\
**In code:** `fl_backend/app/api/bewerbungen/schemas.py :: FLBewerbungZustellung` over `:: FLBewerbungZustellstand`, validated by `fl_backend/app/core/constraints.py :: _BEWERBUNG_ZUSTELLUNG`; written by `fl_backend/app/api/bewerbungen/zustellung_router.py` and read by `fl_backend/app/api/bewerbungen/services.py :: seat_is_unreachable`. The provider's report reaches it through `fl_frontend/src/app/api/mail/zustellung/route.ts`.\
**Trap:** it is not the seat's `Stand`, which is what the PERSON did — a seat reads `bestaetigt` with an earlier link that bounced, and folding the two would make that state unrepresentable. Three of the six are what the clocks read — `unzustellbar`, `unterdrueckt` and `beschwerde` — while `verzoegert` chases and sweeps exactly as `angenommen` does, a delayed message still arriving. `am` is an INSTANT where every other stamp on an application is a date, because two events about one message share a day and the ordering is the whole of the idempotency; it is normalised to one UTC spelling at a fixed width, so the comparison is a comparison of moments rather than of text. `grund` is the provider's own token and never its prose, whose bounce message quotes the recipient's address. And the block is REPLACED whole by a re-send or an address correction, so a refusal never outlives the address it was recorded against.\
**See:** the `Bestätigung` entry for the bookkeeping this sits in, and backend spec I195 for the two write conditions and I196 for the application a refusal holds.
