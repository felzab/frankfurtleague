...
**Trap:** deletion is soft and stays soft under a privacy request — every match embeds the referee's name and id, so an anonymisation nulls the members of `kontakt` and writes a neutral label over the name, on the row and on every match, where a pupil, whom no match embeds, is erased outright; a rename must fan out into the embedded copy on every match carrying it — every season's, unlike a club's, because a referee is not season-scoped, and that fan-out is one-way — a rename putting a name back onto an anonymised referee is refused (`REQ-ANONYMISE-002`), the snapshot window being the only route back; and the collection's own read is admin-tier, a referee being a pupil whose `kontakt` and `schule` are private (`READ-CONTACT-001`) beside a `default_payment` that is money (`READ-MONEY-001`).\
