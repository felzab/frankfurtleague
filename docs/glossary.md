...
**Is:** the block stored beside a person saying what they agreed to (`umfang`), when (`datum`) and on whose word it is held (`erteilt_von`) — carried once on a pupil, and once on each contact seat of a team's season, where it also cites the wording that person was shown (`text_version`) and the day they confirmed it themselves (`bestaetigt_am`).\
**In code:** `fl_backend/app/api/spieler/schemas.py :: FLEinwilligung` for a pupil and `fl_backend/app/api/teams/schemas.py :: FLKontaktEinwilligung` for a contact seat, validated by `fl_backend/app/core/constraints.py :: _EINWILLIGUNG` and `fl_backend/app/core/constraints.py :: _KONTAKT_EINWILLIGUNG`; the contact half alone has a Zod mirror, `fl_frontend/src/features/teams/schemas.ts :: FLKontaktEinwilligungSchema`, and `fl_frontend/src/core/einwilligung.ts :: LIGA_EINWILLIGUNGEN` is the registry every `text_version` names an entry of.\
...
**Is:** the bookkeeping beside each seat of a `Bewerbung` — the hash of the link that seat's person was mailed, the day it went, the day a reminder went and the day they declined — kept beside the slot rather than inside it, so a decline that empties the person's slot leaves the record that they declined standing.\
...
**Trap:** the raw token is the whole credential and exists only in the mail — the document keeps `token_hash`, and after a reminder `token_hash_zuvor` beside it, and no model declares either, so no admin read can serve one and a lost link is re-minted rather than recovered. A REMINDER and a RE-SEND are different acts: the reminder mints a fresh link and keeps the first hash live, so both messages work and `bestaetigungsfrist` does not move, while a re-send replaces the seat's whole entry, which is what voids the earlier link and restarts the deadline. That deadline sits on the APPLICATION and not on the seat, so one seat's re-send moves it for all three, and a deadline that passes deletes the whole application with every contact record on it (`docs/backend/spec.md :: I151`). And the pending state has no member of `status`: an application nobody has confirmed reads `eingereicht` like any other, and it is acceptance that refuses while a seat is outstanding (`REQ-BEWERBUNG-013`).\
**See:** the `Einwilligung` entry above for what a confirmation stamps inside the slot, the `Bewerbung` entry for the application this hangs off, and backend spec I152 for what a reminder writes.
