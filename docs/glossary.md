...
**Is:** one school's application to play one season, stored as it arrived apart from what the confirmation flow moves: the triage moves `status`, `entscheidung` and `team_id`; each seat's own person fills their `geburtsdatum` and consent record, or declines and empties the slot; a re-send replaces one seat's entry under `bestaetigungen` and restarts `bestaetigungsfrist`; and a contact erasure empties the slot naming the person who asked for it, with the entry beside it. The opponent it would like on the first Spieltag is a FREE STRING and never a reference: a school may name an applicant nobody has accepted yet, and a picker over the accepted ones would give a later applicant the longer list.\
