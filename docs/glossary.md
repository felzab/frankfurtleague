...
**Trap:** `erlaubte_stufen` bounds what the squad FORM offers and never what a stored row holds, so narrowing it leaves the rows outside it readable rather than invalid; `E2` is in the closed set although the current season has nobody in it, a set stopping at what a season contains refusing an entry as the year turns; and the base tier is served no `stufe` at all (`READ-PUPIL-002`), so a public squad list carries none; and it is not an age — it says which Halbjahr a pupil is in and never how old they are, the person's own `geburtsdatum` being the field that answers that and optional (`fl_backend/app/core/domain.py :: UNENFORCED`).\
...
### `Einwilligung` — the record that a person agreed, in two vocabularies that share no member

**Is:** the block stored beside a person saying what they agreed to (`umfang`), when (`datum`), on whose word it is held (`erteilt_von`) and the day it was confirmed (`bestaetigt_am`, null while it is not) — carried once on a pupil and once on each contact seat of a team's season, where it also cites the wording that person was shown (`text_version`, which the pupil block has no field for).\
**In code:** `fl_backend/app/api/spieler/schemas.py :: FLEinwilligung` for a pupil and `fl_backend/app/api/teams/schemas.py :: FLKontaktEinwilligung` for a contact seat, validated by `fl_backend/app/core/constraints.py :: _EINWILLIGUNG` and `fl_backend/app/core/constraints.py :: _KONTAKT_EINWILLIGUNG`; each half has its own Zod mirror, `fl_frontend/src/features/spieler/schemas.ts :: FLEinwilligungSchema` and `fl_frontend/src/features/teams/schemas.ts :: FLKontaktEinwilligungSchema`, and `fl_frontend/src/core/einwilligung.ts :: LIGA_EINWILLIGUNGEN` is the registry every `text_version` names an entry of.\
**Trap:** the two models share every field name and not one value. `umfang` is `kader_oeffentlich` or `intern` on a pupil and `kontaktdaten` or `kontaktdaten_whatsapp` on a contact seat; `erteilt_von` asks WHO agreed on a pupil — `erziehungsberechtigt`, `volljaehrig` or `bestandsuebernahme` — and WHETHER the person themselves did on a contact seat, `person` against `administrativ`. `volljaehrig` names who spoke and pins no age; the league's is `fl_backend/app/shared/schemas/bounds.py :: BEWERBUNG_KONTAKT_MIN_AGE_YEARS`. A helper, a facet or a label written against one model is therefore wrong on the other, and no type refuses the reuse. `text_version` names a REGISTRY LABEL and never the words: it says which text the stamping person was shown, so an entry there is never reworded or removed and new wording is a new label — which is why the confirmation page carries one of its own, the applicant who ticked for three people having read a different page. The WhatsApp scope is on the read model alone, so no administrative write can transcribe it. And `bestaetigt_am` on a contact seat is stamped by that seat's own link and by nothing else, while on a pupil the field is served with no writer a person can reach (`docs/_roadmap/items.md :: 8wd7-ff49`).\
**See:** the `kontakte` entry above for why the two vocabularies stay unshared, [`datenschutz.md`](datenschutz.md#2-consent-comes-from-the-person-from-16) for the rulings the contact half implements, and `docs/_roadmap/items.md :: duhh-xcsh` for the naming question the legal basis decides.
