...
Paste into a fresh session (or run via `/audit:pass frontend 3`).

---

Audit pass 3 of 6 on `./fl_frontend`. Lens: RSC AND CACHING SEMANTICS, TYPE AND DATA VALIDATION
...
the module list from a `"use server"` grep — never a hardcoded list): action | resource
mutated | tags invalidated | tags used by the queries reading that resource | GAP. Derive the
read side from every `use cache` function's `cacheTag` calls. Verify ADR-0001's invariants
hold: every granular tag has a matching `updateTag` in the same slice, and the base tags are
invalidated unconditionally. Report the full table.

A2. **`use cache` correctness.** Per cached function: `cacheTag`/`cacheLife` declared or silently
defaulted; request-scoped reads (cookies, headers, searchParams, `Date.now()`, random) that
would poison a shared entry; dynamically constructed tag strings unreachable by invalidation.
...
start with `await connection()` — ADR-0009 applies to it too), searchParams parsed not cast.

A6. **Hook correctness and hydration.** Effects doing derived state or server-side work; unstable
dependencies; index keys on reorderable lists (and keys unique only per-subset — a per-season
match number is not unique across sibling lists); state that should be URL state; hydration
mismatch sources (clock reads, locale-dependent formatting, storage reads before mount).
...
B3. **Schema/type drift.** Hand-maintained types duplicating a zod schema in the same slice:
...
narrowing, shape-assuming catch blocks. `noUncheckedIndexedAccess` is on — anything reported
here must be a genuine remaining hole, not something the compiler already rejects.

B5. **Env contract.** Every env var consumed in `src` is declared and validated in
`core/config.ts`; anything declared but never consumed (names only, never values; check
ADR-0014 before flagging the system key).
