...
3. **EXCESS — code that should not exist.** The candidates for this surface:
   ...
   - `.claude/rules/backend.md` bears directly on this check: the `$jsonSchema` validators duplicate the
     Pydantic models by hand on purpose, and `app/core/domain.py` is written by hand rather than
     generated. No write path may import `domain.py`, `app/main.py :: declared_refusals` being its one
     reader (`fl_backend/tests/core/test_domain.py` holds the importer set).
