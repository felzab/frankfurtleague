...
| `FE-SWEEP-001`  | A retention pass did not reach the backend, for one season or for the whole run (`fl_frontend/src/features/bewerbungen/sweep.ts :: runBewerbungSweep`). The remaining seasons still run, and the next hourly pass retries; the line carries the season and the error's name, never a person                                                                                                                                                                                                      |
