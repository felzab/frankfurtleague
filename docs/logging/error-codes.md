...
| `REQ-VAL-001`   | 422    | Request payload or parameters failed validation; at 400, with no `fields`, a body that is not JSON at all or could not be read                                                                                                                                                                                                                                                                                      |
...
| `REQ-ROUTE-003` | any    | The routing layer refused the request at a status no other code names, passed through as it was raised                                                                                                                                                                                                                                                                                                              |
...
| `FE-MAIL-008`   | The message telling somebody their address has been barred did not reach them (`fl_frontend/src/features/sperrliste/actions.ts :: benachrichtigen`). The ban itself stands and its administrator is told the person was not reached, or of an unclear outcome where the connection broke after the send; the address was typed for that one send and is stored nowhere, so nothing can re-send it and the line is the only record that a ban went out unannounced. It carries the error's NAME alone, never the address (`docs/logging/spec.md :: L9`)            |
