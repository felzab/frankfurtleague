...
| `FE-AUTH-006`   | A session a sign-in replaced was not ended, and it lives beside the new one until its own expiry (`fl_frontend/src/core/auth.ts :: endReplacedSession`): the sign-in stands, its own session having committed first. The line carries the event `auth.session_rotation_failed` and the error's NAME alone                                                                                                                                                                                                                                                            |
