...
| `REQ-ACTIVATE-004`    | 409    | A season with a matchday carrying no `beginn` was activated, which would take the league live without a calendar                                                                                                                                                             | `fl_frontend/src/features/saisons/actions.ts :: activateSaisonAction`        |
...
| `REQ-SPERRLISTE-001`  | 409    | An address the ban list already holds was banned again                                                                                                                                                                                                                       | `fl_frontend/src/features/sperrliste/refusals.ts :: mapAdresseRefusal`       |
| `REQ-SPERRLISTE-002`  | 409    | A ban was entered in a league that has run no season, so nothing bounds the five seasons it lapses after                                                                                                                                                                     | `fl_frontend/src/features/sperrliste/refusals.ts :: mapAdresseRefusal`       |
| `REQ-EINLADUNG-001`   | 409    | A registration link was minted for a team the season holds no junction row for                                                                                                                                                                                               | `fl_frontend/src/features/einladungen/actions.ts :: mapEinladungRefusal`     |
| `REQ-EINLADUNG-002`   | 409    | A registration link was minted, singly or for a whole season, after that season had ended                                                                                                                                                                                    | `fl_frontend/src/features/einladungen/actions.ts :: mapEinladungRefusal`     |
...
| `REQ-BOOKING-001`     | 409    | A venue or a referee NEWLY assigned to a fixture, or kept on one a save makes unplayed again, is unknown or retired                                                                                                                                                          | `fl_frontend/src/features/spiele/actions.ts :: mapSpielRefusal`              |
| `REQ-CLASH-001`       | 409    | A venue or referee claim the fixture did not already make, less than four hours from another fixture's claim                                                                                                                                                                 | `fl_frontend/src/features/spiele/actions.ts :: mapSpielRefusal`              |
...
| `DB-FAIL-001`         | 500    | A database operation crashed. The status is the RAISED path's: the code also logs on a request that answers 200, where `fl_backend/app/api/saisons/admin_router.py :: _mail_one_team` catches one team's failure and answers a row rather than losing the other teams' links | —                                                                            |
