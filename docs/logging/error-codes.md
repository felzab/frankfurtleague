...
| `FE-AUTH-006`   | Sessions that should have ended live on until their own expiry: the one a sign-in replaced (`fl_frontend/src/core/auth.ts :: endReplacedSession`), whose sign-in stands, or a barred address's, whose ban stands and whose administrator is told (`fl_frontend/src/features/sperrliste/actions.ts :: postSperreAction`). The line carries the event, `auth.session_rotation_failed` or `sperrliste.sessions_not_ended`, and the error's NAME alone                                                                                                                   |
