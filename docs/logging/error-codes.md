...
are, whether a route serves the request, and whether the body parses.**
...
**A 403 answers who the caller is and never the target's state, so a shut window on
...
| `REQ-AUTH-005`  | 400    | No usable `X-FL-Actor` header on an admin-tier write, or on any request a person's route serves                                                                                                                                                                                                                                                                                                                     |
...
| `REQ-ROUTE-002` | 405    | The path is served, but not with this method; the `Allow` header names the methods that are                                                                                                                                                                                                                                                                                                                         |
...
| `FE-NET-001`    | The network did not answer, timeout included (`APINetworkError`: `isTimeout` distinguishes, and `method` names the request); a write whose answer never arrived is answered as of unknown outcome, as `DB-FAIL-002` is; the mail transport raises it for a stalled send; a call the request deadline refused unsent logs it too, a write's as `ApiUnsentError`, which changed nothing                                                                                                                                                                                |
...
| `FE-AUTH-006`   | Sessions that should have ended live on until their own expiry: the one a sign-in replaced (`fl_frontend/src/core/auth.ts :: endReplacedSession`), whose sign-in stands, or a barred address's, whose ban stands, whose administrator is told (`fl_frontend/src/features/sperrliste/actions.ts :: postSperreAction`) and which no person page serves (`docs/frontend/spec.md :: I406`). The line carries the event, `auth.session_rotation_failed` or `sperrliste.sessions_not_ended`, and the error's NAME alone                                                    |
...
| `FE-MAIL-008`   | The message telling somebody their address has been barred did not reach them (`fl_frontend/src/features/sperrliste/actions.ts :: benachrichtigen`). The ban itself stands and its administrator is told the person was not reached, or, where the connection broke after the send, that whether they were is unclear; the address was typed for that one send and is stored nowhere, so nothing can re-send it and the line is the only record that a ban went out unannounced. It carries the error's NAME alone, never the address (`docs/logging/spec.md :: L9`) |
