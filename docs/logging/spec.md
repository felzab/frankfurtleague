...
([`docs/ops/spec.md`](../ops/spec.md) §1.3), so one that does not restate both headers passes a
client-sent `traceparent` or `X-FL-Actor` through as it arrived, nginx forwarding every request
header no `proxy_set_header` names. One location declares any, `nginx/shared/site.conf :: location = /api/v0/system/is_live`,
...
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| nginx    | Mints the trace id and its own span on every request, logs both, proxies the header upstream                                                | `nginx/shared/site.conf :: proxy_set_header traceparent`, `nginx/shared/http.conf :: map $request_id $edge_span` |
...
site that forgets one leaves an uncoded line rather than a defaulted one. A line the application did
not write takes its forwarding route's code instead, one route per surface
(`fl_frontend/src/core/consoleShim.ts :: installConsoleShim`,
`fl_backend/app/core/logging.py :: ForwardedFailureFilter`), and why each route carries a code of its
own is [`error-codes.md`](error-codes.md#4-forwarded-codes) §4. The frontend's environment refusal is an
...
| L4  | Every failure a handler answers is `{error_code, trace_id}`, the code the exception's own; a `REQ-VAL-001` adds where each refusal sits, never its value | `fl_backend/tests/api/test_error_responses.py`; the two middleware 400s answered before any handler: [`docs/backend/spec.md`](../backend/spec.md) §1.4                                                                                                                                                                                                                                                                                    |
...
| L7  | A visitor's `traceparent` is replaced by the edge's own — trace `$request_id`, span its first sixteen hex — on every path reaching application code      | `nginx/shared/site.conf :: proxy_set_header traceparent` at server level, inherited by every location declaring none; driven by `nginx/edge_test.sh` on every frontend location                                                                                                                                                                                                                                                           |
| L8  | Every uncached admin-tier read seeds the request's trace scope                                                                                           | `fl_frontend/src/shared/utils/adminRead.ts :: runAdminRead`, held over every slice's queries by `fl_frontend/src/shared/utils/adminReadSpine.test.ts`; the class is defined in [`docs/frontend/spec.md`](../frontend/spec.md#12-cached-reads) §1.2                                                                                                                                                                                        |
| L9  | A log line names a REJECTED FIELD, never the value submitted for it                                                                                      | `fl_backend/tests/api/test_error_responses.py :: TestValidationLoggingWithholdsTheValue`; `fl_backend/app/core/logging.py :: STRUCTURED_EXTRAS` bounds what travels as a field; `fl_frontend/src/core/mail.test.ts`, `fl_frontend/eslint.config.mjs :: LOGGED_ERROR`, `fl_frontend/src/core/auth.test.ts`, `fl_frontend/src/core/config.test.ts` and `fl_frontend/src/features/bewerbungen/notifications.test.ts` hold the frontend to it |
| L10 | The `X-FL-Actor` a visitor sends is cleared on every proxied path that reaches a write                                                                   | `nginx/shared/site.conf :: proxy_set_header X-FL-Actor` at server level, inherited by every location declaring none and restated by the liveness location; driven by `nginx/edge_test.sh`                                                                                                                                                                                                                                                 |
| L11 | A credential a URL carries never reaches the edge's access line, on the path field or on the referer (section 4)                                         | `nginx/shared/http.conf :: map $uri $logged_uri`, `:: map $request_uri $credential_free_uri` and `:: map $http_referer $logged_referer`, which both edges include; driven by `nginx/edge_test.sh`                                                                                                                                                                                                                                         |
...
| A request was slow and no line says where the time went             | Each hop meters its own span, and nothing joins the figures (1.2, `nginx/shared/http.conf :: log_format fl_json`)                            | An edge `duration_s` with an empty `upstream_duration_s` is nginx or the network; a large backend `duration_ms` is the application                                                                                |
...
| A sign-in or confirmation link's token appears in nginx's error log | Every line nginx writes about a request repeats the request line, query string and `Referer` — L11 governs the access line alone (section 4) | Accepted: it answers once, until `fl_frontend/src/core/auth.ts :: LINK_VALIDITY_SECONDS` or its flow's `fl_backend/app/shared/schemas/bounds.py` deadline; the host alone holds the log, eight days at most (1.2) |
| An invitation link's token appears in nginx's error log             | The same lines as the row above; every pupil of a team registers through its one invitation, so the link is multi-use by design              | Accepted: it stays live whenever its season's registration window runs, until revoked (`fl_backend/app/api/einladungen/services.py :: find_live_einladung_filter`); the host alone holds the log                  |
...
| Raising the error log to `crit` would shed the tokens                                  | Refused — it sheds every upstream-failure, timeout and 413 line with them, and the published notice's sentence on a failed request's entry goes untrue                       |
| Per-path error levels with a landing page's `Referrer-Policy` would keep its token out | Refused — it closes one route and misses the next minter's landing, which is why access lines redact by parameter, not path (L11)                                            |
| A redacting receiver behind `error_log syslog:` would rewrite each line                | Refused — a new component would stand between nginx and its only failure record                                                                                              |
