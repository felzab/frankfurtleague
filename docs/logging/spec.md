...
- **nginx:** the `fl_json` `log_format` with `escape=json`, set on every serving block. `path` and
  `message` read `$logged_uri` rather than `$request_uri`: the two are the same string everywhere
  but the Auth.js callback prefix, where a fixed literal stands in because the query string carries
  a live sign-in token and the administrator's address (L11). The **error
...
| L11 | A credential a URL carries never reaches the edge's access line — the Auth.js callback's query string holds the sign-in token and the address it was mailed to, and the line records a fixed literal in place of the whole URI. The match is on `$uri`, the decoded and normalised path nginx routes on, so a request reaching the callback by another spelling — a doubled slash, a percent-encoded letter, a `.` segment — is redacted too; and the literal is written out rather than echoing `$uri`, because a `%3F` in the path decodes to a `?` and puts the query string back inside `$uri` | `nginx/prod.conf :: map $uri $logged_uri` with `nginx/local.conf :: map $uri $logged_uri`, read by `fl_json`'s `path` and `message` (1.2). Unenforced by the gate, as L7 and L10 are: `nginx -t` parses a file without reading what it logs, and nothing parses `nginx/local.conf` at all ([`docs/ops/spec.md`](../ops/spec.md) §4). Observed 2026-09-01 against the pinned image driving `nginx/local.conf` itself: six spellings of the callback, one carrying a token and an address, each logged as the literal, while `/signin?error=Verification` and `/api/bewerbung/kuerzel?q=ABC` kept their own query strings |
...
| nginx's error log is plain text at its default level       | Accepted — the format is not configurable (1.2). It repeats the whole request line, so a callback request that fails writes the query string L11 keeps out of the access line; one that succeeds raises no error line |
