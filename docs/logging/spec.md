...
| L10 | The `X-FL-Actor` a visitor sends is cleared on every proxied path that reaches a write | `nginx/prod.conf :: proxy_set_header X-FL-Actor`, set at server level and inherited by every location declaring no header of its own — and `nginx/prod.conf :: location = /api/v0/system/is_live` is the only backend path the edge carries, so a visitor's header reaches FastAPI on no path at all |
...
| A total backend outage reports HTTP 200               | The error boundary streams after headers are sent, so status is no health signal    | Monitor `GET /api/v0/system/is_live` through the edge — `nginx/prod.conf :: location = /api/v0/system/is_live` is the one backend path it carries (`fl_backend/app/api/system/router.py :: check_is_live`) |
