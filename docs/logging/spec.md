...
| A percent-encoded parameter name defeats both request-line arms     | Accepted — `$request_uri` is never decoded, and both minters (@auth/core and `fl_frontend/src/app/api/bewerbung/route.ts`) spell `token=` literally, so no such spelling carries another user's credential (L11) |
