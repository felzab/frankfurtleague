...
`scripts/ops/deploy.sh` runs on every deploy. nginx is not in that set and carries its own log across a
...
| L7  | A visitor's `X-Correlation-ID` is replaced by the edge's own `$request_id` on every path that reaches application code | `nginx/prod.conf :: proxy_set_header X-Correlation-ID` at server level, which every location declaring no `proxy_set_header` of its own inherits; the locations that declare one are 1.1's                                                                                                                                                                           |
...
| A URI spelling `token=` or `email=` loses its whole logged query    | Accepted — over-redaction is the safe direction, and no route here produces such a URI (L11, `nginx/prod.conf :: map $request_uri $credential_free_uri`)                          |
| A percent-encoded parameter name defeats both request-line arms     | Accepted — `$request_uri` is never decoded, and @auth/core mints `token=` literally, so no such spelling carries another user's credential (L11)                                  |
