...
| A percent-encoded parameter name defeats both request-line arms     | Accepted — `$request_uri` is never decoded, and both minters (`fl_frontend/src/core/anmeldeLink.ts` and `fl_frontend/src/features/bewerbungen/bestaetigungLink.ts`) spell `token=` literally, so no such spelling carries a credential (L11) |
