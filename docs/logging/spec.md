...
- **nginx:** the `fl_json` `log_format` with `escape=json`, set on every serving block. `path` and
  ...
  but the Auth.js callback prefix, where the query string is dropped because it carries a live
  sign-in token and the administrator's address (L11). The **error
...
| L11 | A credential a URL carries never reaches the edge's access line — the Auth.js callback's query string holds the sign-in token and the address it was mailed to, and the line records the path alone | `nginx/prod.conf :: map $request_uri $logged_uri` with `nginx/local.conf :: map $request_uri $logged_uri`, read by `fl_json`'s `path` and `message` (1.2). Unenforced by the gate, as L7 and L10 are: `nginx -t` parses a file without reading what it logs, and nothing parses `nginx/local.conf` at all ([`docs/ops/spec.md`](../ops/spec.md) §4). Observed 2026-09-01 against the pinned image, the callback line carrying no query string and every other path carrying its own                                      |
