...
| L10 | The `X-FL-Actor` a visitor sends is cleared on every proxied path that reaches a write                                                              | `nginx/shared/site.conf :: proxy_set_header X-FL-Actor` at server level, which every location declaring no `proxy_set_header` of its own inherits and the liveness location restates (1.1)                                                                                                                                                                                                                                                                                                  |
